...
> **Migrated from `AGENTS.md`** (now a short agent-instructions file). This is the
> project's session log. The undated "Project Summary" sections below predate the
> per-session log; dated work appears under `## Session YYYY-MM-DD` headings.
>
> **Adding new work:** append a `## Session YYYY-MM-DD — <title>` section at the end of
> this file (or write `docs/sessions/YYYY-MM-DD.md` for a long session), with a short
> **Files changed** / **Verification** block and links to the relevant commits where
> known. **Do not** append to `AGENTS.md`.

## Project Summary (historical, undated)
...
## Session 2026-09-05 — X86_TSX validation + tsx_sim SGL fix + optimized gem5 sweep

### tsx_sim SGL lost-update fix (root cause: tid-0 == free-sentinel)
`runtime/tsx_sim/src/lib.rs` encoded the SGL fallback lock as
`SGL_OWNER == 0 → free`, but native thread IDs are 0-based
(`NATIVE_TID_COUNTER` starts at 0). Thread 0's
`compare_exchange(0, 0, ...)` is a no-op "acquisition" that leaves the owner
at 0 (free), so a second thread's CAS also succeeded → two threads in SGL mode
simultaneously → write-through RMWs clobbered each other and in-flight TSX
commits. Symptom: `fuzz_counter -t 8` FAILed with ~20 stale publishes
(negative-delta writes) per 16k tx; ring traces showed orphaned SGL
begin/read/write/end events (e.g. a tid=4 SGL end with owner=1).
**Fix**: real `SGL_HELD: AtomicBool` mutex (CAS acquire/release) +
diagnostic-only `SGL_OWNER`; all checks (`try_begin_single` LOCK_BUSY, TSX
commit OWNER_CHANGED, `tm_abort` release, `sim::try_begin`/`force_sgl`) now
use `SGL_HELD`. **Verified**: 12/12 fuzz t{1,2,4,8}×3 seeds PASS with a ring
validator showing zero protocol violations and zero stale writes; fuzz_bank
t{2,4,8} PASS.

### gem5 X86_TSX: XBEGIN ABI bug found & fixed
`XBeginInst::completeAcc` (`gem5_sim/gem5/src/arch/x86/insts/htmruby.cc`)
returned `0` on success; real binaries check `status == _XBEGIN_STARTED`
(-1). Consequence: unmodified TSXSGL binaries never entered TSX under gem5
(silent SGL fallback for the whole run — invisible to PASS checks). Fixed to
write `0xFFFFFFFF` (Intel ABI). After rebuild, `bank_gem5_tsxsgl` t1 shows
`HTM_Start=HTM_Commit=1001` (TSX genuinely exercised).

### gem5 TSX semantics validated (all PASS)
- CPUID RTM=1; XBEGIN decodes (C7/3) + htmStart; in-tx read/write sets
  exact (1/1 for minimal probe); XEND commits with write visible.
- Minimal-tx cycle budget: 183c sample = 5c body + 178c commit; total
  CPU-visible 60+body+178 = 238c fixed — matches calibrated Broadwell-EP
  (`machine_profiles/broadwell_ep_v4.json`; knobs are
  `Sequencer.py htm_start_latency=60 / htm_commit_latency=178` + 40c
  coherence-probe term in `HTMSequencer::rubyHtmCallback`).
- Conflict probe (2 cores, 2×50 RMW on one line): final=100/100, 1 abort,
  retry loop works → no lost updates.
- Probe rules: no syscalls inside a tx (SE `syscall_fault` → abort
  cause=exception); accept both 0/`_XBEGIN_STARTED` conventions in probes.

### Optimized gem5 benchmark flow (new tooling)
- `gem5_sim/scripts/run_gem5_sweep.py` — 16-run matrix (bank/fuzz_counter/
  intruder × NOREC/TSXSGL × t{1,2,4}), 12-way parallel, per-run timeout,
  auto-parse. 16 runs ≈ 5 min wall on this 32-core host (per-run 3.6–142 s).
- `gem5_sim/scripts/parse_gem5_stats.py` — last-occurrence-wins over repeated
  stats sections (ROI dump is last), per-core `l1_controllers*` HTM
  aggregation, guest verdicts for bank/fuzz/intruder.
- `configs/x86-se-bank.py` gained `--raw-args` for non-bank guest argv.
- Results: **16/16 PASS** (money conserved / counter sum / 300-of-300
  flows). TSXSGL abort rate climbs with contention (t4 bank: 1611/1617
  aborts, SGL fallback carries the load — expected design behaviour).
  `m5out/sweep-<ts>/{results.csv,results.json}`.

### Best path: gem5 TSX vs real hardware on all benchmarks (3 tiers)
1. **Decision equivalence (all benchmarks, native speed)**: same workload
   natively via tsx_sim feature + C++ NOREC vs gem5 ROI runs — invariants,
   commit/abort breakdowns. t4 gap is model difference (gem5 = in-flight
   line-granularity coherence + TSXSGL `sgl_owner`-in-read-set amplification;
   tsx_sim = commit-time WR/WW), not a bug.
2. **Cycle-budget fidelity (per tx)**: gem5 fixed cost 238c reproduces the
   calibrated Broadwell-EP profile; compare `m_htm_transaction_cycles` vs
   tsx_sim virtual cycles vs `machine_profiles` cost model, flag >10%.
3. **Real RTM anchor**: `patches/profile/tsx/run_workflow.sh` +
   `run_tsx_profiling.py` on Broadwell/Skylake hardware regenerate
   `machine_profile.json`; re-tune the two sequencer latency knobs + 40c
   coherence term.

### Files changed
- `explicit_api/rust/workspace/runtime/tsx_sim/src/lib.rs` — SGL_HELD mutex fix
- `gem5_sim/gem5/src/arch/x86/insts/htmruby.cc` — XBEGIN success = 0xFFFFFFFF
- `gem5_sim/configs/x86-se-bank.py` — `--raw-args`
...
DX/CI/docs hygiene items S10–S15.

### S01–S06 — correctness (see TODO.md + CORRECTNESS_FIXES.md §9–12)
- **S01** NOrec/NOrec-BF plugin-mode bypass: migrated read/write/commit to the
  `isOnCurrentThreadStack` pattern. `bank_norec -a 128 -t 4` conserves; social
  invariant PASS.
- **S03** TinySTM invariants: `Lock::unlock()` owner re-check
  (`g_unlock_owner_mismatch` counter + debug `TM_ASSERT`); clock-wrap reset
  verified by `tests/expli-api/test_tinystm_clock_wrap.cpp` (11/11).
- **S04** NOrec RO→RW promotion: documented that reads are always recorded (no
  abort-restart needed); regression `tests/expli-api/test_norec_ro2rw.cpp`.
- **S05** NOrec globals on `tm_exit`: verified NO-OP (all static storage; OS
  reclaims; valgrind clean).
- **S06** single entry point + `make check-fast` (~40 s smoke: plugin + 3
  C++ backends + Rust sim + workspace); `.DEFAULT_GOAL := help`.

### S10 — docs index + README Known Issues
- New `docs/README.md`: index of all 24 docs by category + "when to read this".
- `README.md` Known Issues refreshed (stale LEFTRIGHT 29/114 removed; JVSTM
  `peek()` + Calvin 4T added; links `TODO.md`).
- `docs/DEVELOPER_GUIDE.md`: `check-fast`/`help` + docs-index pointer.

### S11 — freeze AGENTS.md → CHANGELOG.md
- `CHANGELOG.md` holds all session history. `AGENTS.md` rewritten as an
  85-line instruction file (≤150) with doc-map + conventions; "do not append
  sessions here". `docs/AGENTS.md` is a symlink to it. New
  `docs/sessions/README.md` (long-session format).

### S12 — CONTRIBUTING.md + issue/PR templates
- New `CONTRIBUTING.md` (add-a-backend 6-step C++/Rust walkthrough from
  `DEVELOPER_GUIDE.md`, `make check-fast`/`make fmt-check` requirements, commit
  style, PR checklist).
- New `.github/ISSUE_TEMPLATE/{bug_report,feature_request,backend_proposal}.md`
  + `.github/PULL_REQUEST_TEMPLATE.md`.
- Linked `CONTRIBUTING.md` from `README.md` and `docs/README.md` (acceptance).
- Fixed a stale `scripts/check-requirements.sh` → `tools/check-requirements.sh`
  reference in `AGENTS.md` (and used the correct path in `CONTRIBUTING.md`).

### S14 — gate PRs on cross-backend + TLC smoke
- `.github/workflows/ci.yml`: `cross-backend` runs on PRs (3 core backends,
  `exit 1` on fail) + push:main (all 10). New blocking `tla-smoke` job downloads
  tla2tools.jar and runs `make -C docs/proofs smoke-check`.
- `docs/proofs/Makefile` gained a `smoke-check` target (3 small models,
  `-deadlock`, 60 s each).

### S15 — format/lint gate
- `Makefile`: `fmt` (apply) + `fmt-check` (verify) targets. C++ via
  `clang-format-22 --style=file` over the C++ tree; Rust via `cargo fmt --check`
  + `cargo clippy -- -D warnings` (tm w/ wbctl, simulator).
- `make fmt` reformatted the C++ tree (now 263/263 sources clean) + all Rust.
  Fixed a pre-existing `plugin/clang-tm --print-tool cxx` bug (missing
  `--gcc-install-dir` → stdlib not found on this box).
- Fixed the one non-stabilizable C++ file
  (`backends/tm_impl/common/tm_opaque_safe.hpp`) by wrapping the
  `TM_OPAQUE_SAFE_TABLE` macro in `// clang-format off/on`.
- Fixed all pre-existing clippy warnings: addrspace (3× `not_unsafe_ptr_arg_deref`
  allows on FFI allocators + 2 redundant casts), tm (`TmPtr` canonical
  `Clone`/`Copy`, `# Safety` docs, `TmCell` doc reorder), simulator (14: match
  guards, `is_multiple_of`/`div_ceil`, `Default` impls, `enumerate`, collapsible
  if/match).
- `make fmt-check` + `make check-fast` pass; simulator 98 tests + workspace green.
