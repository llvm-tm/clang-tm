...
static void collectLocalVariables(Function &F, SmallPtrSet<const Value *, 32> &LocalVars)
{
	for (auto &BB : F) {
		for (auto &I : BB) {
	...
	TM_DEBUG("Total local variables in %s: %d",
	         F.getName().str().c_str(),
	         (int)LocalVars.size());
}
		...
		for (User *U : V->users()) {
			...
			if (auto *Call = dyn_cast<CallBase>(U)) {
				if (tm_platform::isHeapAllocationCall(Call))
					continue;
				if (tm_platform::isDeallocationCall(Call))
					continue;
			...
			if (isa<PHINode>(U) || isa<GEPOperator>(U) || isa<BitCastInst>(U) ||
			    isa<SelectInst>(U) || isa<PtrToIntInst>(U) || isa<IntToPtrInst>(U)) {
                                ...
                                int Depth = 0)
{
	...
	if (auto *Load = dyn_cast<LoadInst>(Ptr)) {
		return originatesFromLocal(Load->getPointerOperand(),
		                           LocalVars,
		                           CurrentFunc,
		                           Depth + 1);
	}
	...
	if (auto *Select = dyn_cast<SelectInst>(Ptr)) {
		return originatesFromLocal(Select->getTrueValue(),
		                           LocalVars,
		                           CurrentFunc,
		                           Depth + 1) &&
		       originatesFromLocal(Select->getFalseValue(),
		                           LocalVars,
		                           CurrentFunc,
		                           Depth + 1);
	}
	...
	if (auto *IToP = dyn_cast<IntToPtrInst>(Ptr)) {
		return originatesFromLocal(IToP->getOperand(0),
		                           LocalVars,
		                           CurrentFunc,
		                           Depth + 1);
	}
	...
	if (auto *Call = dyn_cast<CallBase>(Ptr)) {
		...
		if (CurrentFunc && tm_platform::isHeapAllocationCall(Call) &&
		...
		for (auto &Arg : Call->args()) {
			if (Arg->getType()->isPointerTy()) {
				if (originatesFromLocal(const_cast<Value *>(&*Arg),
				                        LocalVars,
				                        CurrentFunc,
				                        Depth + 1))
					return true;
...
// If OutGV is non-null and a matching GlobalVariable is found, sets *OutGV
// so callers can retrieve the global name without re-tracing the chain.
static bool tracesFromTMGlobal(
    Value *V,
    Module &M,
    SmallPtrSetImpl<const AllocaInst *> *VisitedAllocas = nullptr,
    SmallPtrSetImpl<const PHINode *> *VisitedPHIs = nullptr,
    int Depth = 0,
    GlobalVariable **OutGV = nullptr)
{
	if (Depth > 15)
		return false;
	if (!V)
	...
	if (auto *GV = dyn_cast<GlobalVariable>(V)) {
		if (isTMAnnotatedGlobal(GV, M)) {
			if (OutGV) *OutGV = const_cast<GlobalVariable *>(GV);
			return true;
		}
		return false;
	}
		...
		if (Function *Callee = Call->getCalledFunction()) {
			if (Callee->getName().starts_with("tm_")) {
				...
				if (Callee->getName() == "tm_read_ptr" ||
				    ...
				    Callee->getName() == "tm_calloc" || Callee->getName() == "tm_realloc")
					return true;
				return false;
			}
		}
		for (unsigned i = 0; i < Call->arg_size(); i++)
			if (tracesFromTMGlobal(Call->getArgOperand(i),
			                       M,
			                       VisitedAllocas,
			                       VisitedPHIs,
			                       Depth + 1, OutGV))
	...
	if (auto *Phi = dyn_cast<PHINode>(V)) {
		...
		for (Value *Inc : Phi->incoming_values())
			if (tracesFromTMGlobal(Inc, M, VisitedAllocas, VisitedPHIs, Depth + 1, OutGV))
				return true;
		return false;
	}
	...
	if (auto *Sel = dyn_cast<SelectInst>(V))
		return tracesFromTMGlobal(Sel->getTrueValue(),
		                          M,
		                          VisitedAllocas,
		                          VisitedPHIs,
		                          Depth + 1, OutGV) ||
		       tracesFromTMGlobal(Sel->getFalseValue(),
		                          M,
		                          VisitedAllocas,
		                          VisitedPHIs,
		                          Depth + 1, OutGV);
	...
	if (auto *PTI = dyn_cast<PtrToIntInst>(V))
		return tracesFromTMGlobal(PTI->getPointerOperand(),
		                          M,
		                          VisitedAllocas,
		                          VisitedPHIs,
		                          Depth + 1, OutGV);
	...
	if (auto *EV = dyn_cast<ExtractValueInst>(V))
		return tracesFromTMGlobal(EV->getAggregateOperand(),
		                          M,
		                          VisitedAllocas,
		                          VisitedPHIs,
		                          Depth + 1, OutGV);
	...
	if (auto *IV = dyn_cast<InsertValueInst>(V)) {
		if (tracesFromTMGlobal(IV->getAggregateOperand(),
		                       M,
		                       VisitedAllocas,
		                       VisitedPHIs,
		                       Depth + 1, OutGV))
			return true;
		return tracesFromTMGlobal(IV->getInsertedValueOperand(),
		                          M,
		                          VisitedAllocas,
		                          VisitedPHIs,
		                          Depth + 1, OutGV);
	...
	if (auto *Arg = dyn_cast<Argument>(V)) {
		...
		for (User *U : Parent->users()) {
			...
			if (!ActualArg)
				continue;
			if (tracesFromTMGlobal(ActualArg, M, VisitedAllocas, VisitedPHIs, Depth + 1, OutGV))
				return true;
		}
		return false;
	}
	...
	if (auto *AI = dyn_cast<AllocaInst>(V)) {
		...
		while (!AllocaExpr.empty()) {
			...
			for (User *U : Cur->users()) {
				if (!VisitedExpr.insert(U).second)
					continue;
				if (auto *Store = dyn_cast<StoreInst>(U)) {
					...
					if (StorePtr == AI || getBaseObject(StorePtr) == AI) {
						...
						if (isa<AllocaInst>(
						        getBaseObjectNoLoad(Store->getValueOperand())))
							continue;
						if (tracesFromTMGlobal(Store->getValueOperand(),
						                       M,
						                       VisitedAllocas,
						                       VisitedPHIs,
						                       Depth + 1, OutGV))
...
static bool tracesFromAlloca(Value *V, Module &M, int Depth = 0)
{
	...
	if (auto *GEPOp = dyn_cast<GEPOperator>(V))
		return tracesFromAlloca(const_cast<Value *>(GEPOp->getPointerOperand()),
		                        M,
		                        Depth + 1);
...
static void collectTMLocalAllocas(Function &F,
                                  Module &M,
