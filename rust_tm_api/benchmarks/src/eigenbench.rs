// ── EigenBench: STM Orthogonal Characteristics Microbenchmark ──────────
//
// Based on: S. Hong et al., IISWC 2010.
// Tests 8 orthogonal TM characteristics via configurable R1/W1/R2/W2/A1/A2/A3.
...
struct SharedArrays {
    a1: Vec<TmCell<i64>>,         // shared, size A1
    a2: Vec<Vec<TmCell<i64>>>,    // per-thread, each size A2
    a3: Vec<Vec<TmCell<i64>>>,    // per-thread, each size A3
    config: Config,
}
...
fn gen_indices(count: usize, size: usize, tid: usize, param: f64, spread: bool) -> Vec<usize> {
    let mut rng = Rng::new(tid as u64 * 12345 + 42);
    let max = if spread { size } else { size.min(10) };
    (0..count).map(|i| {
        if param > 0.8 {
            tid % max
        } else if param > 0.5 {
            let off = rng.range(0, if spread { size as u64 / 10 } else { max as u64 }) as usize;
            tid * size / config_threads.load(Ordering::Relaxed) + off % size
        } else {
            rng.range(0, size as u64) as usize
        }
    }).collect()
}

static config_threads: std::sync::atomic::AtomicUsize = std::sync::atomic::AtomicUsize::new(1);
...
fn main() {
    ...
    config_threads.store(config.threads, Ordering::Relaxed);
