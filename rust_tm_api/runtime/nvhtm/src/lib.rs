// ── NV-HTM Rust backend ─────────────────────────────────
// Commit-time locking (WBCTL-style write-back) with:
//   - Lock table (hash-based, 2^20 entries)
//   - Global clock for versioning
//   - Write-set buffer (HashMap), no read-set
//   - Null-address guard on writes
//   - Durability: SeqCst fence after write-back
//   - Abort counter
//
// In the C++ version, NV-HTM uses Intel RTM for the
// transaction body and a redo log for NVM durability.
// The Rust variant uses software TM (write-back commit-time
// locking) which provides the same correctness guarantees
// without requiring RTM hardware.  The durability API is
// stubbed with atomic fences.
...
impl Lock {
    ...
    fn unlock_exclusive(&self) {
        let cur = self.data.load(Ordering::Relaxed);
        let next = ((cur >> 1) + 1) << 1; // clear lock bit, increment version
        self.data.store(next, Ordering::Release);
    }

    fn version(&self) -> u64 {
        let v = self.data.load(Ordering::Acquire);
        v >> 1 // version is everything except the lock bit
    }
...
struct TxState {
    ...
    read_set: Vec<(usize, u64)>, // (address, version_at_read_time)
    snapshot: u64,
    aborted: bool,
}
...
fn read_word<T: Primitive>(addr: usize) -> T {
    ...
    loop {
        ...
        with_tx(|tx| {
            // Abort if version changed since snapshot
...
pub fn tm_commit() -> bool {
    ...
    // Phase 4: write-back all entries (safe — WriteBack::apply() encapsulates the unsafe)
    for wb in tx.write_backs {
        wb.apply();
    }

    // Durability: flush cachelines (SeqCst fence as approximation)
    fence(Ordering::SeqCst);
