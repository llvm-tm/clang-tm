...
__device__ uintptr_t*   csmv_gpu_free_list_addr();
__device__ uintptr_t*   csmv_gpu_retire_list_addr();
...
struct CSMVWarpState {
    ...
    uint64_t batch_snapshot;  // clock at batch launch; versions older than
                              // this become reclaimable once superseded
    int      num_reads;
    int      num_writes;
...
// csmv_gpu_commit GC) are recycled at the next batch boundary instead of
// leaking one malloc per committed version.
__device__ inline CSMVVersionNode* csmv_gpu_alloc_node() {
    uintptr_t *fl = csmv_gpu_free_list_addr();
    uintptr_t head = atomicAdd((unsigned long long*)fl, 0ULL);
    while (head != 0) {
        CSMVVersionNode *n = (CSMVVersionNode*)head;
        uintptr_t nxt = (uintptr_t)(uintptr_t)n->next;
        if (atomicCAS((unsigned long long*)fl, (unsigned long long)head,
                      (unsigned long long)nxt) == (unsigned long long)head)
            return n;
        head = atomicAdd((unsigned long long*)fl, 0ULL);
    }
    return (CSMVVersionNode*)malloc(sizeof(CSMVVersionNode));
}

// Push a node onto the retire list.  Memory stays valid until the drain
// kernel between batches moves it to the free list, so lock-free readers
// that already followed a link into it still see intact data.
__device__ inline void csmv_gpu_retire_node(CSMVVersionNode *n) {
    uintptr_t *rl = csmv_gpu_retire_list_addr();
    uintptr_t prev = atomicExch((unsigned long long*)rl, (unsigned long long)(uintptr_t)n);
    n->next = (CSMVVersionNode*)(uintptr_t)prev;
}
...
__device__ inline uint64_t csmv_gpu_commit(CSMVWarpState *ws) {
    ...
    if (lane_id == 0) {
        ...
        for (int i = 0; i < num_writes; i++) {
            CSMVGpuEntry *entry = &csmv_gpu_table()[ws->writes[i].entry_idx];
            CSMVVersionNode *node = csmv_gpu_alloc_node();
            node->timestamp = commit_ts;
            node->value = ws->writes[i].val;
            node->next = csmv_gpu_load_head(entry);
            csmv_gpu_store_head(entry, node);

            // GC: every reader in this batch (and all future batches) has
            // start_clock >= batch_snapshot, so once the chain contains a
            // node visible to them (ts <= batch_snapshot), everything older
            // is unreachable.  Retire it; the drain kernel between batches
            // recycles the memory.  We hold the entry lock, so no other
            // committer is restructuring the chain; lock-free readers are
            // safe because retired memory stays valid until the batch ends.
            CSMVVersionNode *p = node;
            while (p && p->timestamp > ws->batch_snapshot)
                p = p->next;
            if (p) {
                CSMVVersionNode *dead = p->next;
                p->next = nullptr;
                while (dead) {
                    CSMVVersionNode *nx = dead->next;
                    csmv_gpu_retire_node(dead);
                    dead = nx;
                }
            }
