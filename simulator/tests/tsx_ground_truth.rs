...
fn ev(ts: u64, tid: u32, seq: u64, kind: EventKind) -> Event {
...
fn tsx_rr_both_commit() {
    ...
    assert_eq!(
        eng.stats.commits, 2,
        "RR: both should commit (got {} commits, {} aborts)",
        eng.stats.commits, eng.stats.aborts
    );
...
fn tsx_rw_reader_aborts() {
    ...
    assert_eq!(
        eng.stats.commits, 1,
        "RW writer-first commit: 1 commit (writer wins)"
    );
    assert_eq!(
        eng.stats.aborts, 1,
        "RW writer-first commit: 1 abort (reader)"
    );
}
...
fn tsx_ww_first_committer_wins() {
    ...
    assert_eq!(
        eng.stats.aborts, 1,
        "WW first committer wins: 1 abort (other writer)"
    );
}
...
fn tsx_spurious_rate_sanity() {
    ...
    for _ in 0..10000 {
        seq += 1;
        events.push(ev(ts, 0, seq, EventKind::TxBegin));
        ts += 1;
        seq += 1;
        events.push(ev(
            ts,
        ...
        ts += 1;
        seq += 1;
        events.push(ev(ts, 0, seq, EventKind::TxEnd));
        ts += 1;
    }
    let eng = run(Backend::TsxSim, &events);
    assert!(
        eng.stats.aborts <= 10,
        "spurious: {} aborts in 10k single-thread (expected ~0-1 at 6e-06)",
        eng.stats.aborts
    );
