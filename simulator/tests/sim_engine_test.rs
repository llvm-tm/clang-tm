...
fn test_abort_detected_outside_tx_read() {
    for b in [Backend::Norec, Backend::Tl2] {
        ...
        let engine = run_events(b, &events);
        assert_eq!(
            engine.stats.reads_outside_tx,
            1,
            "{}: outside read counted",
            b.name()
        );
    }
}
...
fn test_abort_detected_outside_tx_write() {
    for b in [Backend::Norec, Backend::Tl2] {
        ...
        let engine = run_events(b, &events);
        assert_eq!(
            engine.stats.writes_outside_tx,
            1,
            "{}: outside write counted",
            b.name()
        );
    }
}
...
fn test_idempotent_write_conflict_norec() {
    ...
    assert_eq!(
        engine.stats.commits, 2,
        "NOrec: idempotent writes → no conflict"
    );
...
fn test_idempotent_write_conflict_tl2() {
    ...
    assert_eq!(
        ...
        "TL2: idempotent writes → version conflict → abort"
    );
...
fn test_tinystm_write_only_conflict() {
    ...
    assert_eq!(
        engine.stats.commits, 1,
        "TinySTM: write-only address conflict → 1 commit"
    );
    assert_eq!(
        engine.stats.aborts, 1,
        "TinySTM: T0 aborts due to B's version change"
    );
    assert!(
        engine.verifier.violations.is_empty(),
...
fn test_checkpoint_roundtrip() {
    ...
    for b in [
        ...
        while let Some(pos) = remaining
            .iter()
            .position(|e| matches!(e.kind, EventKind::Checkpoint))
        {
        ...
        // Compare
        assert_eq!(
            engine_baseline.stats.commits,
            engine_cp.stats.commits,
            "{}: commits match after checkpoint roundtrip",
            b.name()
        );
        assert_eq!(
            engine_baseline.stats.aborts,
            engine_cp.stats.aborts,
            "{}: aborts match after checkpoint roundtrip",
            b.name()
        );
        assert_eq!(
            engine_baseline.verifier.violations.len(),
            engine_cp.verifier.violations.len(),
            "{}: violation count matches after checkpoint roundtrip",
            b.name()
        );
    }
}
