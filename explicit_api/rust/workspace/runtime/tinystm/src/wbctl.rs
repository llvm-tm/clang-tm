...
fn write_word<T: Primitive>(addr: usize, val: T) {
    ...
    if with_tx(|tx| ws_contains(&tx.write_set, addr)) {
        with_tx(|tx| {
            if let Some((_, e)) = tx.write_set.iter_mut().find(|(a, _)| *a == addr) {
                e.value = tv.clone();
            }
        });
        return;
...
pub fn tm_commit() -> bool {
    ...
    if tx.write_set.is_empty() {
        update_read_write_stats(tx.read_set.len(), 0);
        return true;
    }
    ...
    for (addr, entry) in &tx.write_set {
        apply_typed_value(*addr, &entry.value);
    }
    fence(Ordering::SeqCst);
    unlock_indices(&idxs);
