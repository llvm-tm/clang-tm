...
fn bench_single(
    ...
    BenchResult {
        ...
        sum,
        secs,
        allocs_per_sec: if secs > 0.0 {
            (n as f64 / secs) as i64
        } else {
            0
        },
    }
...
fn main() {
    ...
    println!(
        "Linked list: {} nodes/thread, {} threads\n",
        n_nodes, n_threads
    );
    ...
    println!(
        "  {:<14} {:>10} {:>14}",
        r_tm.name,
        (r_tm.secs * 1000.0) as i64,
        r_tm.allocs_per_sec
    );
    println!(
        "  {:<14} {:>10} {:>14}",
        r_mal.name,
        (r_mal.secs * 1000.0) as i64,
        r_mal.allocs_per_sec
    );
    ...
    println!(
        "\nPhase 4: Multi-threaded throughput ({} threads)",
        n_threads
    );
    println!(
        "  {:<14} {:>10} {:>14}",
    ...
    println!(
        "  {:<14} {:>10} {:>14}",
        r_tm_mt.name,
        (r_tm_mt.secs * 1000.0) as i64,
        r_tm_mt.allocs_per_sec
    );
    println!(
        "  {:<14} {:>10} {:>14}",
        r_mal_mt.name,
        (r_mal_mt.secs * 1000.0) as i64,
        r_mal_mt.allocs_per_sec
    );
    ...
    let speedup = if ratio_st > ratio_mt {
        ratio_st
    } else {
        ratio_mt
    };
