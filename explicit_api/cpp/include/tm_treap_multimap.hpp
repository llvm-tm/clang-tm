...
template <typename K, typename V> class tm_treap_multimap
{
	struct Node {
		K key;
		V val;
	...
	{
		if (in_tx())
			return TM<K>::read_at(&n->key);
		return n->key;
	}
	static void wr_key(Node *n, const K &v)
	{
		if (in_tx())
			TM<K>::write_at(&n->key, v);
		else
			n->key = v;
	}
	...
	{
		if (in_tx())
			return TM<V>::read_at(&n->val);
		return n->val;
	}
	static void wr_val(Node *n, const V &v)
	{
		if (in_tx())
			TM<V>::write_at(&n->val, v);
		else
			n->val = v;
	}
	...
	{
		if (in_tx())
			tm_write_ptr(reinterpret_cast<void **>(&n->left), v);
		else
			n->left = v;
	}
	...
	{
		if (in_tx())
			tm_write_ptr(reinterpret_cast<void **>(&n->right), v);
		else
			n->right = v;
	}
	...
	static Node *rd_parent(Node *n)
	{
		if (in_tx())
			return static_cast<Node *>(
			    tm_read_ptr(reinterpret_cast<void **>(&n->parent)));
		return n->parent;
	}
	static void wr_parent(Node *n, Node *v)
	{
		if (in_tx())
			tm_write_ptr(reinterpret_cast<void **>(&n->parent), v);
		else
			n->parent = v;
	}
...
public:
	...
	Iterator begin() const { return Iterator(min_node(m_root)); }
	...
	void insert(const K &k, const V &v)
	{
		...
		while (rd_parent(z) && rd_prio(z) > rd_prio(rd_parent(z))) {
			...
			if (rd_left(p) == z) {
				...
				if (rd_right(z))
					wr_parent(rd_right(z), p);
				wr_right(z, p);
			} else {
				// Left rotation at p
				wr_right(p, rd_left(z));
				if (rd_left(z))
					wr_parent(rd_left(z), p);
				wr_left(z, p);
			}
			wr_parent(p, z);
			wr_parent(z, g);
