...
public:
	...
	// begin() approach).
	template <typename F> static void transaction(F &&body)
	{
	...
	T read() const { return tm_type_traits<T>::read(const_cast<T *>(ptr())); }
	void write(const T &v) { tm_type_traits<T>::write(ptr(), v); }
	...
	static void write_at(T *addr, const T &v) { tm_type_traits<T>::write(addr, v); }
...
template <typename T> class TM<T *>
{
	T **value_ = nullptr;
...
public:
	...
	T *read() { return (T *)tm_read_ptr((void **)ptr()); }
	void write(T *v) { tm_write_ptr((void **)ptr(), (void *)v); }
