...
template <bool UseTM> struct MemoryAccess;

template <> struct MemoryAccess<true> {
	template <typename T> static T load(const T *addr)
	{
	...
	template <typename T> static void store(T *addr, const T &val)
	{
		if constexpr (std::is_pointer_v<T>) {
...
template <> struct MemoryAccess<false> {
	template <typename T> static T load(const T *addr) { return *addr; }
	template <typename T> static void store(T *addr, const T &val) { *addr = val; }
};
