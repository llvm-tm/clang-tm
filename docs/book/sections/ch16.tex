...
\section{Exercises}
\begin{enumerate}
    \item In Intel RTM, a transaction that exceeds the L1 cache capacity always aborts. Propose a software fallback path and discuss its performance implications. If you do not have RTM hardware, ground the design in the companion's TSX-SGL backend (\texttt{backends/tm\_impl/tsx\_sgl}) and the phased-hybrid model of \S\ref{sec:hybrid-example}.
    \item Using a conceptual pipeline diagram, show why a transaction that triggers a TLB miss is likely to abort.
    \item Configure gem5 (\S\ref{sec:htm-simulation}) to run a simple transactional program and measure the abort rate as the number of cache lines touched increases. Explain the trend. If gem5 is not installed, run the same footprint sweep against the companion's TSX simulation backend (\texttt{simulator/}, \S\ref{sec:htm-simulation}) and report the trend it produces.
...
Exercise~4 is checked with the abort counters in
\S\ref{sec:rtm-bank-transfer}; and Exercise~7 runs the capacity probe from
\S\ref{sec:rtm-bank-transfer} or, without RTM, the simulator described in
\S\ref{sec:htm-simulation}.  The \emph{[verification]} exercises are
