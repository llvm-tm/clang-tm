...
\begin{table}[htbp]
\centering
\begin{tabular}{M{0.18\textwidth}M{0.25\textwidth}M{0.25\textwidth}M{0.22\textwidth}}
\hline
\thb{0.18\textwidth}{\textbf{Mechanism}} & \thb{0.25\textwidth}{\textbf{Tracking}} & \thb{0.25\textwidth}{\textbf{Abort sources}} & \thb{0.22\textwidth}{\textbf{Software obligation}} \\
\hline
...
gem5 Ruby/HTM model & Simulated cache hierarchy and coherence protocol (\texttt{MESI\_Three\_\allowbreak Level\_HTM}); ISA-agnostic HTM sequencer with calibrated begin/commit latencies. L0 cache tracks read/write sets with configurable line limits (128 read, 32 write) and triggers \texttt{LD\_FAIL}/\texttt{ST\_FAIL} on conflict or capacity aborts. & Commit path and abort callbacks are modeled; the x86 fault path maps cache failure reasons (\texttt{FAIL\_SELF}~$\to$~capacity, \texttt{FAIL\_REMOTE}~$\to$~conflict) to the EAX status word via \texttt{GenericHtmFailureFault}, exercising the same fallback code paths as real hardware (see \S\ref{sec:htm-simulation}). & Use simulation to separate effects that hardware counters collapse; Ruby coherence traffic and L0 set-associativity effects become directly observable.\\
\hline
\end{tabular}
...
Simulation answers questions that a single counter or the EAX status word on real hardware often blurs. The hardware does convey the coarse reason --- a conflict (bit~2), a capacity overflow (bit~3), or an explicit abort (bit~0) --- which is enough for a fallback policy. But it does not say \emph{which} coherence event caused a conflict (remote read versus remote write, Snoop versus directory), \emph{which} line was evicted on a capacity abort, or how the abort rate scales as the L1 associativity or line size changes, because those structures are not externally visible. A simulator exposes the cache hierarchy, coherence protocol, and transactional metadata directly, so each of these causes can be isolated and varied.

\subsection{SE vs FS: two levels of full-system fidelity}
\label{sec:se-vs-fs}

gem5 emulates the \emph{entire CPU and memory system} in both modes; the difference is how much of the operating system is emulated.

\begin{table}[htbp]
\centering
\caption{gem5 SE vs FS for TM: what is simulated and when to use which.}
\label{tab:se-vs-fs}
\begin{tabular}{M{0.18\textwidth}M{0.38\textwidth}M{0.36\textwidth}}
\hline
\thb{0.18\textwidth}{Mode} & \thb{0.38\textwidth}{What runs in the simulator} & \thb{0.36\textwidth}{Use for tiny benchmarks} \\
\hline
SE (syscall emulation) & CPU pipeline, Ruby caches, coherence, \texttt{MESI\_Three\_Level\_HTM}, DRAM. Syscalls (\texttt{mmap}, \texttt{futex}, \texttt{clone}) are \emph{emulated} by gem5, not passed to the host OS. $>99\%$ of a bank/STAMP run is cycle-accurate. & Default. Fast ($\sim$0.15\,s host for 2k transactions, \texttt{x86-se-bank.py}). No disk image, no \texttt{/dev/kvm}. The \texttt{FS tiny} bank in \S\ref{sec:htm-simulation} is SE. \\
FS (full system) & Above plus boot of \texttt{x86-ubuntu-24.04}, devices, interrupt controller, full Linux kernel. With \texttt{KVM} the boot is fast-forwarded, then switched to \texttt{Timing}. & Only for kernel interactions (page faults, interrupts, \texttt{rseq}). Slower ($\sim$30\,s for the same tiny workload, \texttt{x86-fs-tiny.py}). Requires disk image and, for speed, \texttt{/dev/kvm}. \\
\hline
\end{tabular}
\end{table}

For the throughput numbers of Chapter~14 the two modes give the same answer: the STAMP and bank tiny configurations (\texttt{-m 10 -n 10}, \texttt{-g 256}, \texttt{-t 1--2}, 2\,k transfers) spend no time in the kernel beyond thread start and memory mapping. SE mode is therefore a representative measurement of the region of interest for such user-space microbenchmarks, bracketed by \texttt{m5} reset/dump markers; it is not a full-system measurement in the sense of running a real kernel and its interrupts, page faults, and scheduler --- that is FS mode's job. Move to FS only when the experiment itself is about the kernel.



The companion gem5 fork (the \textsc{TM-SIM} repository, gem5 v25.1~\cite{binkert11gem5} plus an in-tree x86 TSX implementation) is the concrete vehicle. Upstream gem5 contains \emph{no} x86 TSX support; the fork adds the decoder and instruction semantics for \texttt{XBEGIN}/\texttt{XEND}/\texttt{XABORT}/\texttt{XTEST} and drives them through Ruby's \texttt{MESI\_Three\_Level\_HTM} protocol, whose ISA-agnostic \texttt{HTMSequencer} is calibrated with the measured Broadwell-EP costs of Chapter~14's machine profiles (\texttt{htm\_start\_\allowbreak latency}\,\allowbreak${}=60$ and \texttt{htm\_commit\_\allowbreak latency}\,\allowbreak${}=178$ cycles, matching the RDTSC-profiled XBEGIN/XEND costs from real hardware).

The key idea is that the simulator executes the same \texttt{x86-64 ELF} binary that would run on real hardware, but substitutes a simulated CPU pipeline and cache hierarchy for the physical one. This means the compiler-generated fallback code --- the same code path that handles RTM aborts on real silicon --- is exercised identically in simulation. A benchmark compiled against TSX-SGL runs inside gem5 without modification; when the simulated L0 cache detects a conflict or capacity limit, the fault reaches the same instruction semantics as a real \texttt{XBEGIN} abort. Cross-compilation is necessary because the host (often macOS or ARM) differs from the simulated ISA (x86-64), and syscall-emulation (SE) mode is sufficient for microbenchmarks because the transactional workload never enters the kernel --- the \texttt{m5} reset/dump markers bracket the region of interest so that only the transactional phase is measured.
...
\paragraph{Why the abort fault mapping matters.} On real hardware, when \texttt{\_xbegin} fails, the processor writes a status word into EAX that tells the software \emph{why}: bit~2 for a data conflict, bit~3 for a capacity overflow, bit~0 for an explicit \texttt{\_xabort}. The compiler-generated fallback path tests these bits to decide whether to retry, acquire the global lock, or give up. For the simulation to exercise this same fallback code, the simulated cache hierarchy must detect conflicts and capacity limits and route them through the same instruction semantics as real x86 TSX. Without this mapping, the fallback path would never fire in simulation, and the abort-handling logic would remain untested.

\paragraph{How the mapping works.} When the L0 cache detects a conflict during a transactional load, it sends an \texttt{LD\_FAIL} signal to the sequencer with a Ruby failure reason: \texttt{FAIL\_REMOTE} (another core touched the line) for a coherence conflict, or \texttt{FAIL\_SELF} (the line was evicted from L0) for a capacity abort. A transactional store that encounters the same conditions triggers \texttt{ST\_FAIL}. The sequencer converts the Ruby failure reason to an \texttt{HtmFailureFaultCause} --- \texttt{MEMORY} for conflict, \texttt{SIZE} for capacity --- and delivers it through \texttt{GenericHtmFailureFault}. The fault handler restores the pre-\texttt{XBEGIN} checkpoint and writes the Intel RTM status word into EAX: bit~2 for memory aborts, bit~3 for size aborts, or bit~0 plus the abort code for \texttt{XABORT}.

The same fault mechanism fires for both loads and stores, so write-write conflicts (\texttt{ST\_FAIL} from a coherence invalidation of a transactionally-written line) are handled identically to read-write conflicts (\texttt{LD\_FAIL} from a remote exclusive request hitting a transactionally-read line). The compiler-generated fallback path then tests the EAX bits and enters the software lock, exactly as it would on real hardware. This is what makes gem5 fundamentally different from a trace-driven simulator: the same binary, the same code path, the same fallback logic --- only the timing and the conflict-detection mechanism are simulated.
...
A simulation is a framework for testing whether a model is consistent with observations. The hypothesis here is that the gem5 fork's \texttt{MESI\_Three\_Level\_HTM} protocol, calibrated with measured Broadwell-EP latencies, reproduces the abort-frequency and throughput behaviour of real RTM hardware on the same workload. Running the same benchmark on real hardware and in simulation, and comparing the abort-rate breakdown and per-transaction cycle count, tests that hypothesis. When the numbers agree, the model is validated for that workload class; when they disagree, the divergence points to a mechanism the model is missing --- TLB events, interrupt masking, microarchitectural replay --- and that divergence is itself the useful result.

The companion project uses three instruments at different fidelities. Real-hardware profiling (the RDTSC patch of \texttt{patches/profile/tsx}) provides ground truth on the last widely available RTM parts. The gem5 fork provides a mechanistic view: Ruby coherence traffic, L0 set-associativity effects, and abort-fault routing are directly observable. The Rust cost-model simulator (\texttt{tsx\_sim}) provides speed: it replays \texttt{TM\_TRACE} \texttt{JSONL} traces through a calibrated cycle-estimation model to explore large parameter sweeps quickly. Crucially, \texttt{tsx\_sim} is \emph{not} inside gem5 --- gem5 executes the \texttt{x86-64 ELF} directly via its own \texttt{HTMSequencer} and Ruby coherence, while \texttt{tsx\_sim} is a separate DES backend (\texttt{runtime/tsx\_sim/src/lib.rs}) that replays traces outside gem5. They share the same Broadwell calibration but are independent simulators at different fidelities.
...
The model below represents one thread attempting a hardware transaction and a second thread that may interfere. Three variables capture the state: \texttt{bal} holds the two account balances, \texttt{pc} is the process state (\texttt{"idle"} or \texttt{"tx"}), and \texttt{snap} is the snapshot taken at \texttt{XBEGIN} --- the value of \texttt{bal} that the transaction believes it is working against. The actions model the RTM contract: \texttt{Begin} takes a snapshot and enters the transactional state; \texttt{Commit} checks that the snapshot is still valid (the abstract coherence check) and, if so, applies the transfer; \texttt{Abort} models a hardware abort that rolls back with no effect; \texttt{InterfereAbort} models another thread committing successfully while our transaction is mid-flight, which forces our transaction to abort; and \texttt{Fallback} executes the same transfer serially from the \texttt{idle} state, modelling the global-lock path. The model deliberately omits caches, lines, and invalidations --- the proof boundary is the software contract (snapshot validity implies atomicity), not the hardware mechanism that enforces it.
