...
State clearly whether the result changes if the two validation phases are correlated.
    ...
    \item \emph{[theory]} A backend guarantees virtual world consistency but not opacity. Describe a concrete bank scenario in which an aborted transaction's illegal observation is harmless, and one in which the same observation becomes harmful once the transaction's \emph{compensating action} (a plain, non-transactional write) executes after the abort.
\end{enumerate}
