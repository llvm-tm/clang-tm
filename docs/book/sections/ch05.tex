...
The useful mental model is separation of concerns. A DFA answers whether the next bytes form a valid token. A grammar answers whether the token stream has valid structure. Neither proves that a transaction is serializable. Those semantic properties belong to transition systems and invariants: the correctness conditions that this book actually checks --- serializability, strict serializability, and opacity, with the serializable-but-not-opaque history of Listing~\ref{lst:serializable-not-opaque} as the canonical separating example --- are introduced in Chapter~2 and exercised as checked invariants in Chapter~6; the models there are precisely the transition systems this paragraph points at.
...
\section{Histories, Legality, and Opacity}
\label{sec:histories-legality-opacity}
\index{history!transactional}
\index{legality}
\index{read-from relation}
\index{strict serializability}
\index{opacity!formal definition}
\index{virtual world consistency}
\index{TMS1}
\index{TMS2}
\index{obstruction-freedom}
\index{lock-freedom}
\index{wait-freedom}
\index{progress condition}

Chapter~2 introduced opacity informally through the mixed-snapshot history of Listing~\ref{lst:serializable-not-opaque}. Every invariant checked in Part~III is ultimately a statement about histories, so this section gives the definitions their formal clothing. The vocabulary is small --- one data type and four predicates --- and it is reused verbatim when Chapter~6 turns correctness into checkable invariants.

A \emph{history} $H$ is a finite sequence of events. Each event belongs to a transaction $T$ and takes one of five forms: $\mathrm{begin}(T)$, a read $r_T(x)\!\to\!v$ returning value $v$ from location $x$, a write $w_T(x)\!\leftarrow\!v$, and the two terminal events $\mathrm{commit}(T)$ and $\mathrm{abort}(T)$. A history is \emph{well-formed} if every transaction begins before it reads or writes, acts only between its begin and its terminal event, and has at most one terminal event. Real executions are always prefixes of well-formed histories: a thread may be mid-transaction when we stop observing.

Two derived sequences matter. The \emph{committed projection} $\mathrm{comm}(H)$ keeps only the events of transactions that committed. The \emph{real-time order} $<_{RT}$ relates two transactions when every event of the first precedes every event of the second; overlapping transactions are unrelated. Both are mechanical to compute from a trace, which is why they reappear as assertions in test harnesses.

A history becomes meaningful once we choose a \emph{serialization}: a total order of a set of transactions consistent with their event order inside $H$. Given a serialization $S$, each read event has a \emph{read-from} source: the latest write to the same location by an earlier transaction in $S$, or the initial value if none exists. $S$ is \emph{legal} if every read returns exactly its read-from value under ordinary sequential semantics. Legality is the entire content of ``the transaction saw a state that could really have existed.''

With these pieces, the correctness ladder has three rungs:

\begin{itemize}
\item \textbf{Serializability} (of commits): $\mathrm{comm}(H)$ admits a legal serialization. Aborted transactions are irrelevant --- this is the database-classic condition of Papadimitriou \cite{papadimitriou79serializability}.
\item \textbf{Strict serializability}: the serialization must additionally preserve $<_{RT}$. This is what a global-clock backend such as TL2 or TSC-TM buys you and what a coarse lock trivially guarantees.
\item \textbf{Opacity} \cite{guerraoui08correctness}: \emph{every finite prefix} of $H$ admits a legal serialization of \emph{all} transactions live or finished in that prefix, preserving $<_{RT}$ where it is defined. Aborted transactions must observe legal states too.
\end{itemize}

Opacity is strictly stronger than serializability, and the gap is exactly one aborted transaction's illegal observation. Recast Listing~\ref{lst:serializable-not-opaque} in the new vocabulary. The history contains $r_{T_1}(x)\!\to\!0$, then $w_{T_2}(x)\!\leftarrow\!1$, $w_{T_2}(y)\!\leftarrow\!1$, $\mathrm{commit}(T_2)$, then $r_{T_1}(y)\!\to\!1$, then $\mathrm{abort}(T_1)$. Try any serialization $S$ of $\{T_1, T_2\}$. If $T_2$ precedes $T_1$, the read-from source of $r_{T_1}(x)$ is $w_{T_2}(x)$, so legality demands the read return $1$ --- it returned $0$. If $T_1$ precedes $T_2$, the read-from source of $r_{T_1}(y)$ is the initial value, so legality demands $0$ --- it returned $1$. No legal total order exists: the history is not opaque. Yet $\mathrm{comm}(H) = \{T_2\}$ serializes trivially: the history is serializable. One predicate separates them, and only the aborted transaction feels the difference.

Opacity is expensive to guarantee for aborted transactions that read many locations. \emph{Virtual world consistency} \cite{imbs12vwc} weakens it: committed transactions must serialize legally as before, but an aborted transaction needs only consistency with its own causal past. The weakening permits invisible-read protocols that never validate aborted readers, at the price of letting a doomed transaction see states no serial execution produced. Multi-version designs sit naturally near this end of the spectrum; value-validated designs such as NOrec pay the opacity price on every read.

The definitions descend from the original STM papers. Shavit and Touitou proposed the abstraction \cite{shavit95stm}; TMS1 \cite{herlihy03tms1} supplied structural details --- nesting, composition, the locator-based object model that JVSTM's VBox and OFG-STM's locators inherit --- and TMS2 \cite{herlihy03tms2} made dynamic-sized data structures transactional. All three treated only committed transactions; opacity arrived later precisely because implementations kept failing on aborted ones.

Safety says nothing about whether a transaction ever commits. That is the domain of \emph{progress conditions}. An implementation is \emph{wait-free} if every operation completes in a bounded number of its own steps; \emph{lock-free} if some operation completes in finitely many steps system-wide; and \emph{obstruction-free} \cite{herlihy03obstructionfree} if a transaction completes when it runs alone long enough. Obstruction-freedom is the classic TM bargain: optimistic validation gives it cheaply, but under sustained contention the system may livelock unless a contention manager intervenes. Lock-based designs (the SGL family) offer only deadlock-freedom and can starve individual transactions. This distinction is why the liveness columns in the companion's proof suite distinguish ``passes'' from ``starvation possible'' --- most optimistic backends admit schedules where one thread retries forever, and weak fairness on the retry step does not rule them out.

Finally, note what makes these predicates tractable for a model checker. Opacity quantifies over all finite prefixes and all serializations --- apparently unbounded. Bounding the universe (two accounts, one amount, at most $k$ transactions) bounds both quantities, and the read-from legality condition collapses into per-state invariants of the form ``every committed slot's recorded reads match the values a replay would produce,'' which is literally the shape of the strongest invariant in the companion's multi-version models. What TLC then establishes is opacity for the bounded fragment; the transfer of trust to the unbounded system rests on the modeling discipline of keeping the invariant structure identical.


\begin{table}[htbp]
\centering
\begin{tabular}{M{0.16\linewidth}M{0.18\linewidth}M{0.30\linewidth}M{0.26\linewidth}}
\hline
\thb{0.16\textwidth}{Tool} & \thb{0.18\textwidth}{Object modeled} & \thb{0.30\textwidth}{Question answered} & \thb{0.26\textwidth}{Typical TM use} \\
\hline
...
History analysis & Transactional executions & Is this execution correct? & Opacity and serializability arguments \\
...
\begin{table}[htbp]
\centering
\begin{tabular}{M{0.20\linewidth}M{0.25\linewidth}M{0.25\linewidth}M{0.20\linewidth}}
\hline
\thb{0.20\textwidth}{Model} & \thb{0.25\textwidth}{Keeps} & \thb{0.25\textwidth}{Abstracts away} & \thb{0.20\textwidth}{Best use} \\
\hline
...
History + serialization & Read-from relations & Timing within transactions & Correctness definitions \\
...
\section{Progress and the Family of Correctness Conditions}
\label{sec:progress-semantics}
\index{progress condition}
\index{obstruction-freedom}
\index{progressiveness}
\index{conflict-freedom}
\index{disjoint-access parallelism}
\index{contention manager}

Opacity says what a transaction may observe; it says nothing about whether
the transaction ever finishes. Two orthogonal questions complete the
specification of a TM: how much \emph{progress} is guaranteed, and how much
of opacity's safety can be relaxed before the abstraction stops protecting
the programmer. Both questions have sharp answers, and both shaped which
backends exist in the companion repository.

\subsection{The Progress Ladder and the Fall of Obstruction-Freedom}
\label{sec:progress-ladder}

Progress conditions form a ladder, strongest first
\cite{herlihy91,guerraoui10principles}:

\begin{itemize}
\item \textbf{Wait-freedom}: every operation completes in a bounded number
of its \emph{own} steps, regardless of what the other threads do. The gold
standard, and the rarest.
\item \textbf{Lock-freedom}: some operation completes in finitely many
steps system-wide; individual operations may starve.
\item \textbf{Obstruction-freedom} \cite{herlihy03obstructionfree}: a
transaction completes if it eventually \emph{runs alone} long enough ---
that is, if all other threads stop taking steps. Nothing is promised under
sustained contention: two conflicting obstruction-free transactions may
abort each other forever (livelock).
\item \textbf{Progressiveness} \cite{guerraoui10principles}: a transaction
may be aborted \emph{only} when it conflicts with a concurrent
transaction --- they access the same object and at least one access is a
write. \emph{Weak} progressiveness permits aborting any conflicting pair;
\emph{strong} progressiveness additionally guarantees that every
transaction commits when it runs conflict-free (no concurrent transaction
touches an object it accesses). Conflict-freedom is the corresponding
execution property: no two concurrent transactions conflict.
\end{itemize}

Obstruction-freedom and progressiveness answer different questions ---
obstruction-freedom bounds aborts by \emph{step contention}, progressiveness
bounds them by \emph{data conflicts} --- and the early TM literature bet on
the former. The first dynamic-size STMs were obstruction-free precisely
because optimistic validation gives it almost for free: never take a lock,
validate, abort on doubt. The bet failed in practice, and
Kuznetsov and Ravi explained why it \emph{had} to fail
\cite{kuznetsov15obstructionfree}. They proved a cluster of lower bounds
for obstruction-free TMs: with only read and write primitives, an
obstruction-free TM cannot even provide wait-free operation termination;
it cannot provide strict disjoint-access parallelism (transactions touching
disjoint objects must not contend on the same base object); it cannot have
invisible reads; and its reads cannot avoid incurring a linear number of
memory stalls or expensive read-after-write primitives. Then they
exhibited a simple \emph{lock-based}, progressive, opaque TM that violates
every one of those bounds at once. The conclusion is a linear complexity
separation between non-blocking and blocking TMs: the obstruction-free
class is not merely unfashionable, it is provably weaker in the currency
that matters.

This is the theoretical vindication of a design decision made throughout
the companion: \emph{every} backend in the repository is blocking by
choice. The SGL family takes a single mutex (deadlock-free trivially ---
one lock cannot form a cycle --- but individual transactions may starve).
TL2, NOrec, TinySTM, SwissTM, and MVLog take per-stripe
or commit locks: they are weakly progressive (an abort always traces to a
data conflict or a failed validation) and deadlock-free, but the liveness
columns of the companion's proof suite mark them ``starvation possible''
because a retrying transaction can lose repeatedly to fresh competitors.
The HTM paths (TSX-SGL, SPHT, POWER8) add aborts the program cannot
attribute to any conflict --- capacity and microarchitectural events ---
and buy their progress with the fallback lock. None of the backends is
obstruction-free, and after Kuznetsov and Ravi, none should be.

\begin{table}[htbp]
\centering
\caption{Progress guarantees across the companion's backend families.
``Blocking'' means locks are held across transaction steps; all rows are
deadlock-free.}
\label{tab:progress-taxonomy}
\renewcommand{\arraystretch}{1.3}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{M{0.22\linewidth} M{0.24\linewidth} M{0.36\linewidth}}
\hline
\thb{0.22\textwidth}{\textbf{Backend family}} &
\thb{0.24\textwidth}{\textbf{Progress guarantee}} &
\thb{0.36\textwidth}{\textbf{Aborts permitted when}} \\
\hline
SGL, PersistentSGL, DistributedSGL &
blocking; starvation depends on lock fairness &
never abort --- they serialize \\
TL2, TSC-TM, TinySTM (WBCTL/WT), SwissTM &
blocking, weakly progressive &
validation fails: a data conflict with a committed transaction \\
NOrec, NOrec-BF, MVLog &
blocking, weakly progressive &
value re-read differs: a committed write intervened \\
JVSTM, Calvin (execute phase) &
blocking; read-only transactions never abort &
read-set validation fails against a newer version \\
TSX-SGL, SPHT, POWER8-HTM &
blocking via fallback lock; hardware path abort-driven &
conflict, capacity, or explicit \texttt{xabort} --- not attributable to a data conflict \\
Queue executor (serializers) &
blocking; per-worker serial &
never abort inside a worker --- serialization replaces speculation \\
\hline
\end{tabular}
\end{table}

\subsection{Safety Beyond Opacity: VWC and the TMS Specifications}
\label{sec:safety-family}

Opacity is one point in a family of safety conditions, and the family is
best understood as four answers to one question: \emph{how many executions
may be invoked to justify what the transactions saw?} Opacity demands one
execution justify everyone, including aborted transactions. The other
members relax that demand in different directions.

\textbf{TMS1 and TMS2} descend from the structural specifications of
Herlihy, Luchangco, Moir, and Scherer \cite{herlihy03tms1,herlihy03tms2}.
TMS1 requires the \emph{committed} transactions of a history to be
justified by a single legal execution consistent with real-time order;
each \emph{live or aborted} transaction may appeal to its own, different
execution. TMS1 is essentially strict serializability restricted to
transactions on a single object. TMS2 strengthens this for multi-object
transactions: all reads must be justified by a \emph{single sequence of
memory states} --- the states produced by committed transactions in
commit order --- where a writing transaction's reads must match the latest
state at the time its operations take effect, while a read-only
transaction may read from any state that existed during its lifetime.
TMS2 is thus stricter than TMS1 but still more permissive than opacity.
Doherty, Groves, Luchangco, and Moir made both conditions fully formal by
specifying them as I/O automata, proved that TMS2 implements TMS1 by a
simulation argument, exhibited histories separating the two, and framed
the pragmatic question of which condition proofs should target
\cite{doherty13tms}.

\textbf{Virtual world consistency} \cite{imbs09vwc,imbs12vwc} relaxes
opacity from the multi-version side. Committed transactions must serialize
legally exactly as before, but a live or aborted transaction need only be
consistent with its own \emph{causal past}: each of its reads must return
a value from some global state that existed in some serial execution of
the committed prefix --- its ``virtual world'' --- and different reads may
come from different worlds. No single execution need justify a doomed
transaction's whole snapshot. Every opaque history is virtually
consistent, and the inclusion is strict: multi-version designs (JVSTM's
VBox walks are the canonical example) can serve a transaction from stale
versions that no single serialization would assemble, satisfy VWC, and
still fail opacity.

The relationships are not folklore --- they have been machine-checked.
Lesani, Luchangco, and Moir proved with PVS that \emph{TMS2 satisfies
opacity} and \emph{opacity satisfies TMS1} \cite{lesani12putting},
yielding the chain
\[
\text{TMS2} \;\Longrightarrow\; \text{opacity} \;\Longrightarrow\; \text{TMS1},
\]
with each implication strict, and opacity also strictly implying VWC.
Figure~\ref{fig:condition-lattice} arranges the family by the set of
histories each condition permits: TMS2 is the tightest contract, TMS1 and
VWC are two incomparable weakenings of opacity, and plain serializability
of commits sits at the permissive end where aborted transactions are
...
\begin{tikzpicture}[
font=\small,
cond/.style={draw,rectangle,rounded corners,align=center,
minimum width=4.3cm,minimum height=0.85cm},
edge/.style={-{Stealth},thick},
lab/.style={font=\footnotesize,fill=white,inner sep=1.5pt}
]
\node[cond] (tms2)  at (0,0)     {\textbf{TMS2}\\one memory-state sequence};
\node[cond] (opac)  at (0,-1.9)  {\textbf{opacity}\\one execution for everyone};
\node[cond] (tms1)  at (-4.1,-3.8) {\textbf{TMS1}\\one execution for commits};
\node[cond] (vwc)   at (4.1,-3.8)  {\textbf{VWC}\\one world per causal past};
\node[cond] (serial) at (0,-5.7) {\textbf{serializability}\\commits only};
\draw[edge] (tms2) -- node[lab,right=2pt] {strict} (opac);
\draw[edge] (opac) -- node[lab,above left=0pt and -4pt] {strict} (tms1);
\draw[edge] (opac) -- node[lab,above right=0pt and -4pt] {strict} (vwc);
\draw[edge] (tms1.south) |- ($(serial.west)+(-0.5,0)$) -- (serial.west);
\draw[edge] (vwc.south)  |- ($(serial.east)+(0.5,0)$)  -- (serial.east);
\node[font=\footnotesize, anchor=north, align=center] at (0,-6.6)
  {arrows point from the stronger condition to the weaker one:\\
   every history satisfying the source also satisfies the target};
\end{tikzpicture}
\caption{The correctness-condition family as a lattice of permitted
histories. TMS2 $\Rightarrow$ opacity $\Rightarrow$ TMS1 is machine-checked
\cite{lesani12putting}; opacity $\Rightarrow$ VWC is strict
\cite{imbs12vwc}. TMS1 and VWC weaken opacity along different axes and are
incomparable.}
\label{fig:condition-lattice}
\end{figure}

Which condition should a backend target? The answer depends on what
aborted transactions are allowed to do with what they saw. If transaction
bodies are pure --- reads feed only local computation, and an abort
discards everything --- the weaker conditions cost nothing and buy
performance: invisible reads, stale multi-version snapshots, no
read-set validation. The moment a transaction's observations leak into
non-transactional state (a compensating write after an abort, an error
message derived from an illegal read), only opacity guarantees the leak
is harmless, and opacity is what every backend in Part~IV is tested
against. The companion's fuzz harness enforces the money invariant on
committed state for all backends, and the opacity-sensitive tests
(``\S\ref{sec:histories-legality-opacity}'') fail a backend whose aborted
transactions can poison it. Exercise~\ref{ex:vwc-vs-opacity} asks you to
construct the poisoning scenario by hand.
...
Formal languages describe valid strings and support compiler construction. Automata make these descriptions executable. Markov chains turn retry loops into cost estimates when the memoryless assumption is acceptable. Queueing theory, especially Little's Law, relates committed throughput, residence time, and concurrency. Histories with legality, serializability, and opacity define correctness itself: serializability orders committed transactions, strict serializability preserves real time, and opacity extends the obligation to aborted transactions --- the condition whose violation Listing~\ref{lst:serializable-not-opaque} exhibits and whose bounded-fragment verification Part~III mechanizes. Progress conditions (obstruction-freedom, lock-freedom, progressiveness) are the liveness counterparts of these safety properties, and the Kuznetsov--Ravi separation (\S\ref{sec:progress-ladder}) explains why every backend in the companion is blocking by design. The safety family itself forms a lattice --- TMS2 $\Rightarrow$ opacity $\Rightarrow$ TMS1, with VWC weakening opacity along a second axis (\S\ref{sec:safety-family}) --- and choosing a rung on that lattice is choosing how much an aborted transaction may be trusted. Model checking complements all of these models by exploring reachable states instead of averaging them away.

In the bank example, this division is sharp. The retry model predicts attempts. Little's Law predicts in-flight work. The C++ simulation estimates the multiplier. The TLA+ model checks that committed states preserve money conservation and the account floor --- a bounded-fragment stand-in for the opacity argument of \S\ref{sec:histories-legality-opacity}. Formal languages underpin compiler construction; Markov chains and queueing theory provide first-order performance models for TM. Analytical models predict, model checking proves, and the two tools answer different questions.
...
State clearly whether the result changes if the two validation phases are correlated.
    ...
    \item \emph{[theory]} Consider the history $H$ over locations $x$ and $y$, both initially $0$: $r_{T_1}(x)\!\to\!0$; $w_{T_2}(x)\!\leftarrow\!5$; $\mathrm{commit}(T_2)$; $w_{T_3}(y)\!\leftarrow\!7$; $\mathrm{commit}(T_3)$; $r_{T_1}(y)\!\to\!7$; $\mathrm{abort}(T_1)$. Prove $H$ is serializable by exhibiting a legal serialization of its committed projection, then prove it is not opaque by showing that no legal serialization of all three transactions exists.
    \item \emph{[theory]}\label{ex:vwc-vs-opacity} A backend guarantees virtual world consistency but not opacity. Describe a concrete bank scenario in which an aborted transaction's illegal observation is harmless, and one in which the same observation becomes harmful once the transaction's \emph{compensating action} (a plain, non-transactional write) executes after the abort.
\end{enumerate}
...
\emph{[theory]} exercises are graded against \S\ref{sec:math-example}, with
Exercises~10--11 additionally graded against the definitions of
\S\ref{sec:histories-legality-opacity} (Exercise~10's expected answer:
$\mathrm{comm}(H)$ serializes as $T_2 < T_3$, while every placement of $T_1$
in a full serialization makes one of its two reads illegal).
...
\index{committed projection}
\index{real-time order}
\index{serialization}
\index{contention manager}
\index{deadlock-freedom}
