...
if (lane == 0) {
    ...
    if (aborted == WARP_SIZE)
        *retry_budget -= 1;          /* all lost: fall back or back off */
}
\end{lstlisting}
...
\section[Commit at GPU Scale]{Commit at GPU Scale: From Serial Locks to Batched Multi-Versioning}
\label{sec:commit}
...
\begin{itemize}
    \item \textbf{No commit locks.} A global commit lock that serialises
          ...
          losers. GUST reports that, at 8960 threads on an RTX~6000~Ada,
          the hardware sustains roughly \textbf{100$\times$ more concurrent
          AtomicINCs than CASes} \cite{nunes23gust} --- an atomic-append
          throughput comparison, not a commit-rate measurement, but it is
          exactly the operation a commit-log append performs. The design
...
\begin{itemize}
    ...
    \item GUST reports the underlying hardware asymmetry quantified:
          roughly $100\times$ more concurrent AtomicINCs than CASes at
          8960 threads (RTX~6000~Ada) \cite{nunes23gust} --- the ``no
          commit locks'' bullet, measured.
...
\begin{table}[htbp]
\centering
\begin{tabular}{M{0.14\linewidth}M{0.15\linewidth}M{0.13\linewidth}M{0.16\linewidth}M{0.26\linewidth}}
\hline
\thb{0.14\textwidth}{\textbf{Commit style}} & \thb{0.15\textwidth}{\textbf{Allocator}} & \thb{0.13\textwidth}{\textbf{Waiting}} & \thb{0.16\textwidth}{\textbf{Reader rule}} & \thb{0.26\textwidth}{\textbf{GPU consequence}} \\
\hline
...
silicon. Three of the software rows are realised, at some level, in the
...
and GUST in \texttt{gpu\slash{}backends\slash{}gpu\_gust}. The repository
additionally implements GPUTX
(\texttt{backends\slash{}tm\_impl\slash{}gputx}), which is not a table row;
the remaining literature rows (GPU-STM, the Holey--Zhai lightweight STMs,
OFG-STM) have no companion implementation.

\begin{table}[ht]
\centering
\begin{tabular}{lllM{9cm}}
\hline
\textbf{System} & \textbf{Year} & \textbf{Type} & \thb{9cm}{\textbf{Key idea}} \\
\hline
...
to replace contended commit-time synchronisation with an AtomicINC. It is
\emph{one design point} in the GPU commit-protocol space, not the canonical
answer: the rest of the chapter positions it against the global-lock,
CAS-append, client--server, and read-only MVCC alternatives of
Table~\ref{tab:gpu-commit-strategies} (Section ``Commit at GPU Scale'').
Several of
...
\texttt{GPU\_SIMT.tla}, \texttt{GPU\_JVSTM.tla}, \texttt{GPU\_CSMV.tla},
\texttt{GPU\_GUST.tla} --- make the warp the unit of concurrency and encode the SIMT lock step as a
...
The models caught real bugs --- most notably the \texttt{startTS} vs.\
\texttt{startTS+1} snapshot hole of Section~\ref{sec:gust}, found first in
the model (via \texttt{InvNoMissedConflict}) and then confirmed in the
kernel. The \texttt{\_\_ballot\_sync} divergence class of
Section~\ref{sec:ballot}, by contrast, was caught by on-target testing and
review, not by the models: the all-lanes ballot contract is a hardware
convention that sits below the models' abstraction, and no amount of TLC
state exploration would have found it. That division of labour is the
lesson: TLA+ says \emph{what} must be true; only the hardware says
\emph{how} it will let you express it.
...
\section{Exercises}
\begin{enumerate}
    \item \emph{[theory]} A warp of 32 lanes runs a transaction that spins on a lock held by
    ...
    \item \emph{[theory]} GPU-STM sorts locks to avoid deadlock while PR-STM steals locks by
          priority. Contrast the two contention policies under (a) uniformly
          random and (b) skewed (hot-spot) access patterns.
    \item \emph{[theory]} GUST uses \texttt{snapshot = startTS}, not \texttt{startTS + 1}.
    ...
    \item \emph{[theory]} Using the ballot pattern of Section~\ref{sec:ballot}, compute the
          ...
          lane-guarded branch.
    \item \emph{[theory]} CSMV dedicates server warps to committing client warps' writes.
    ...
    \item \emph{[verification]} Using the TLA+ workflow of Part~III, sketch an invariant for a
...
\item \emph{[literature]} Read the KILO TM paper \cite{fung11kilotm} and the
          GUST technical report \cite{nunes23gust}.  KILO TM validates with
          hardware-managed read/write logs and Bloom filters; GUST with
          CCT/MRV over a commit log.  Compare the two \emph{validation
          units} --- where the validation state lives, what it is checked
          against, and what an abort costs in each.

\item \emph{[open]} Every GPU TM in this chapter is either a simulation
          proposal (KILO TM, WarpTM, snapshot-isolation HTM) or a research
          system (CSMV, GUST, PR-STM); none ships in a commercial SDK.
          Argue a position: will GPU TM ever be productized, or should GPUs
          expose transaction-like higher-level APIs (e.g.\ atomic
          task/stream abstractions) instead, with TM confined to research?
          State the hardware or software change your position depends on.
\end{enumerate}

\textbf{How to check your work.} The \emph{[implementation]} exercise is
verified with the companion: extend the kernel of
\S\ref{sec:gpu-bank-transfer-example} with the bounded per-lane retry
counter, keeping the ballot of \S\ref{sec:ballot} outside the lane-guarded
branch.  The \emph{[verification]} exercises are checked with TLC against
\S\ref{sec:gpu-bank-tla-example} (conservation, floor, and type invariants;
see Appendix~\ref{chap:pluscal-quick-ref}).  The \emph{[theory]} exercises
are graded against \S\ref{sec:ballot}, \S\ref{sec:gust}, and
Table~\ref{tab:gpu-commit-strategies}.  The \emph{[literature]} exercise is
graded on whether your ``validation unit'' contrast is drawn accurately
for both papers.  The \emph{[open]} exercise has no single right answer;
it is graded on whether you state the hardware or software change your
position depends on and defend it against the strongest objection.
