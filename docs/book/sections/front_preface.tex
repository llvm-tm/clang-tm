...
\begin{itemize}
    ...
    \item A single running example --- a two-account \texttt{transfer()} --- threads through the book: it appears as locked code in Chapter~1, as an API call in Chapter~7, as the test case for every runtime in Chapter~9, and again as the invariant-bearing workload for the HTM, GPU, persistent, and distributed runtimes of Chapters~16--19.
\end{itemize}
...
\begin{itemize}
    ...
    \item Exercises are tagged \emph{[implementation]} (write code), \emph{[verification]} (write a TLA+ model), \emph{[theory]} (prove or derive), \emph{[literature]} (read and critique a named paper), or \emph{[open]} (argue a position with no single right answer) so instructors can mix them.
