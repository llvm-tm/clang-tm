...
\S\ref{sec:verify-workflow} maps representative runtime backends in this book
to their model file, key invariant, and TLC outcome; the full set of models
(about three dozen) lives in \texttt{docs/proofs}.
...
process Tx \in 1..2
variables snap = 0, r1 = 0, r2 = 0, ok = TRUE;
begin
  BeginTx:  snap := clock;
  Read:     r1 := bal[1]; r2 := bal[2];
  Validate: if clock /= snap then
              ok := FALSE;        \* abort: the caller retries
            end if;
  Commit:   if ok then
...
\begin{itemize}
    ...
    \item \textbf{Nested records break TLC's fingerprinting.}  Plain
          function-valued variables such as
          \texttt{bal = [a $\in$ 1..2 |-> 10]} fingerprint fine, but a
          record-of-records value such as
          \texttt{log = [s |-> [state: ..., ws: ..., rs: ...]]} makes
          TLC~2.14 fail with \emph{``can't enumerate the value of the
          `state' field''}.  The working idiom (used by the repository's
          \texttt{MVLog.tla}) is parallel arrays indexed by slot:
          \texttt{logState[s]}, \texttt{logWs[s]}, \texttt{logRs[s]}.
    \item \textbf{Nested record-field assignment confuses the PlusCal
          translator.}  Consecutive statements such as
          \texttt{log[slot].state := Committed;} followed by
          \texttt{log[slot].ws := ...} trigger \emph{``Missing labels''}
          errors; assign the whole element at once instead:
          \texttt{log[slot] := [state |-> Committed, ws |-> ...]}.
    \item \textbf{\texttt{==} followed by \texttt{=} is a precedence
          trap.}  A definition like \texttt{Init == log = [..]} can raise a
          parser precedence conflict; parenthesize the right-hand side:
          \texttt{Init == (log = [..])}.
