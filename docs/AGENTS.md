...
Created `backends/stubs/tm_stubs.cpp` — stubs implementation of all `extern "C"` TM API functions:
...
Added `stubs` target to `expli_instr/cpp/Makefile` with `BUILD_STUBS_RULE` that links against `tm_stubs.cpp` instead of the TM runtime. Runner builds stubs on demand in `run_uninstrumented()`. All 11 benchmarks (STAMP×8 + tpcc + ycsb + stmbench7) run successfully.
...
| File | Line | Fix |
|------|------|-----|
| `plugin/tm_pipeline.mk` | 129 | Added `-I$(BACKENDS_DIR)` to `TM_INCLUDES_singlelock` (was empty) |
| `expli_instr/cpp/Makefile` | 47 | `single_global_lock_runtime.cpp` → `SingleGlobalLock_runtime.cpp` (wrong case) |
| `expli_instr/cpp/Makefile` | 49 | Added `-I../backends` to SGL `EXTRA_INC` (was empty) |
| `backends/tm_impl/tiny_stm/tinystm_wbctl.hpp` | 317–325 | Wrapped unconditional `fprintf(stderr,...)` in `#ifndef NDEBUG` |
...
- `backends/tm_impl/tiny_stm/tinystm_wbctl.hpp`: WBCTL runtime with write-back and `is_stack_addr`
- `backends/runtimes/TinySTM_runtime.cpp`: `tm_nested_call_counter`, path detection, runtime init
- `plugin/tm_pipeline.mk`: Plugin build system with per-backend rules
- `expli_instr/cpp/Makefile`: Expli C++ build with BACKEND selection
- `expli_instr/rust/benchmarks/src/bin/`: Rust benchmark binaries (`stamp_vacation.rs`, etc.)
