...
- **C++ `TxExecutor` hierarchy** (`expli_tm_api/tx_executor.hpp`): `TxExecutor` base (virtual `execute`/`execute_async`), `InlineExecutor` (inline retry loop), `QueueExecutor` (thread pool + round-robin + work stealing + spin-wait sync), `tx_transaction(exec, body)` free function, `executor()`/`set_executor()` globals. Verified: `-Wall -Wextra -Wpedantic` clean, links with TinySTM runtime.
- **tx_transaction body capture fix**: Changed from `[&body]` (by ref — dangling with QueueExecutor) to `[body = std::move(body)]` (by value). Works for both inline and queue modes.
- **Rust `tm-executor` crate**: `TxExecutor` trait, `InlineExecutor`, `QueueExecutor` (workers + CV + spin-wait sync), `tx_execute(exec, f)` convenience function. Tests: 4/4 PASS. Builds cleanly alongside `containers`.
- **Bank benchmark `--queue` mode**: `bank.cpp` — `run_tx()` dispatch wrapper checks global executor pointer. `--queue` flag creates a `QueueExecutor`. Money conserved in both inline (51K tx/s) and queue (39K tx/s) modes.
- **Makefile fix**: `tm_region_allocator.cpp` added to all `BUILD_RULE` invocations (compulsory link dependency). Bank include paths fixed from `../../../expli_tm_api/` to bare `expli_tm_api/` (via `-I..`).
- **Commits**: `8be93db` (per-thread slab + atomic-counter pending + TxExecutor hierarchy + perf counters), `daeb734` (Rust tm-executor crate + C++ bank queue integration).
