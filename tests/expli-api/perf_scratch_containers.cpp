...
template <typename F> static double measure(F &&f)
{
	double best = 1e30;
...
static void bench_set(int N)
{
	...
	// === CONTAINS (hit) ===
	{
		...
		double v = measure([&]() {
			volatile size_t sink = 0;
			for (int k : hits)
				if (std::binary_search(sv.begin(), sv.end(), k))
					sink++;
			(void)sink;
		});
		printf("  contains(hit):  ScratchSet %8.4f s  std::set %8.4f s  sorted_vec %8.4f "
		       "s\n",
		       t,
		       u,
		       v);
	...
	// === CONTAINS (miss) ===
	{
		...
		printf("  contains(miss): ScratchSet %8.4f s  std::set %8.4f s  sorted_vec %8.4f "
		       "s\n",
		       t,
		       u,
		       v);
	...
	// === ERASE ===
	{
		...
		printf("  erase:          ScratchSet %8.4f s  std::set %8.4f s  (vec skip, "
		       "O(n))\n",
		       t,
		       u);
	}
	...
	// === ITERATE ===
	{
		...
		printf("  iterate:        ScratchSet %8.4f s  std::set %8.4f s  sorted_vec %8.4f "
		       "s\n",
		       t,
		       u,
		       v);
