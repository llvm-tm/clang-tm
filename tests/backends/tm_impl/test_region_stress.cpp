                                ...
                                int64_t n_nodes)
{
	...
	free_fn(head);
	return {name, n_nodes, sum, sec, static_cast<int64_t>(n_nodes / sec)};
}
                            ...
                            int n_threads)
{
	...
	return {name, total, total_sum, sec, static_cast<int64_t>(total / sec)};
}
...
int main(int argc, char **argv)
{
	...
	auto r_tm = bench_single("TM region", alloc_tm, free_tm, n_nodes);
	auto r_mal = bench_single("std::malloc", alloc_malloc, free_malloc, n_nodes);

	printf("  %-14s %10lld %14lld\n",
	       r_tm.name,
	       (long long)(r_tm.sec * 1000),
	       (long long)r_tm.allocs_per_sec);
	printf("  %-14s %10lld %14lld\n",
	       r_mal.name,
	       (long long)(r_mal.sec * 1000),
	       (long long)r_mal.allocs_per_sec);

	double ratio_st = (double)r_tm.allocs_per_sec / (double)r_mal.allocs_per_sec;
	...
	auto r_tm_mt = bench_mt("TM region", alloc_tm, n_nodes, n_threads);
	auto r_mal_mt = bench_mt("std::malloc", alloc_malloc, n_nodes, n_threads);

	printf("  %-14s %10lld %14lld\n",
	       r_tm_mt.name,
	       (long long)(r_tm_mt.sec * 1000),
	       (long long)r_tm_mt.allocs_per_sec);
	printf("  %-14s %10lld %14lld\n",
	       r_mal_mt.name,
	       (long long)(r_mal_mt.sec * 1000),
	       (long long)r_mal_mt.allocs_per_sec);

	double ratio_mt = (double)r_tm_mt.allocs_per_sec / (double)r_mal_mt.allocs_per_sec;
	printf("\n  Speedup: %.1f× (TM region vs std::malloc)\n", ratio_mt);
	...
	printf("\nPhase 6: Free/Reuse correctness\n");
	{
		...
		printf("  free+reuse: %s\n", ok ? "PASS" : "FAIL");
		if (!ok) {
			fprintf(stderr, "FAIL: free/reuse corruption\n");
			return 1;
		}
	}
	...
	printf("\nPhase 7: Alloc/free exceeding 64 GB total\n");
	{
		...
		bool pass = (stm::g_next_slab_idx.load() * stm::g_slab_size <
		             (uint64_t)TOTAL_GB * 1024 * 1024 *
		                 1024); // used < TOTAL_GB → blocks recycled
		printf("  %lld iters in %.1fs  (%lld allocs/sec)  "
		       "slabs used: %zu/%zu (%.0f%%)  %s\n",
		       (long long)iterations,
		       sec,
		       (long long)rate,
		       (size_t)stm::g_next_slab_idx.load(),
		       stm::g_num_slabs,
