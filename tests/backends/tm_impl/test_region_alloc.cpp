...
static int test_init()
{
	...
	T(stm::g_tm_region_end > stm::g_tm_region_start, "end > start");
...
static int test_malloc_small()
{
	...
	for (size_t sz = 1; sz <= 256; sz++) {
		...
		T(check_pattern(p, sz, static_cast<uint8_t>(sz)), "small malloc write/verify");
	}
...
static int test_alignment()
{
	...
	for (size_t sz = 1; sz <= 128; sz++) {
		void *p = stm::tm_region_malloc(sz);
		T((reinterpret_cast<uintptr_t>(p) & 15) == 0, "16-byte alignment");
	}
	for (size_t sz = 1024; sz <= 64 * 1024; sz *= 2) {
		void *p = stm::tm_region_malloc(sz);
		T((reinterpret_cast<uintptr_t>(p) & 15) == 0, "16-byte alignment (large)");
	}
	// Edge cases: odd sizes
	for (size_t sz : {17, 33, 65, 1, 3, 7, 9, 31, 63}) {
		void *p = stm::tm_region_malloc(sz);
		T((reinterpret_cast<uintptr_t>(p) & 15) == 0, "16-byte alignment (odd size)");
	}
...
static int test_isTMAddress()
{
	...
	T(!stm::isTMAddress(reinterpret_cast<void *>(below)), "below start not in region");

	// Above end
	uintptr_t above = reinterpret_cast<uintptr_t>(end);
	T(!stm::isTMAddress(reinterpret_cast<void *>(above)), "address at end not in region");
...
static int test_multi_thread()
{
	...
	for (int t = 0; t < nthreads; t++) {
		threads.emplace_back([t, &ptrs, nthreads]() {
			...
			for (int i = 0; i < ALLOCS_PER; i++) {
				...
				void *p = stm::tm_region_malloc(sz);
				if (!p) {
					fprintf(stderr, "  FAIL thread %d: null at iter %d\n", t, i);
					return;
				}
				fill_pattern(p, sz, static_cast<uint8_t>(t * 64 + i));
	...
	for (int t = 0; t < nthreads; t++) {
		for (size_t i = 0; i < ptrs[t].size(); i++) {
			...
			T(check_pattern(p, sz, static_cast<uint8_t>(t * 64 + i)),
...
static int test_oversized()
{
	...
	T(q_start < p_start || q_start >= p_end, "oversized allocs do not overlap");
...
static int test_bump_order()
{
	...
	for (size_t i = 1; i < ranges.size(); i++)
		T(ranges[i].first >= ranges[i - 1].second, "sequential allocs do not overlap");

	// Verify data
	for (int i = 0; i < N; i++) {
		size_t sz = (i % 32 + 1) * 7;
		T(check_pattern(ptrs[i], sz, static_cast<uint8_t>(i)), "bump-order data intact");
	}
