...
TX void vector_tx(int thread_id, int base)
{
	...
	for (size_t i = 0; i < local.size(); i++) {
		if (local[i] != base + (int)i) {
			fprintf(stderr,
			        "FAIL: vector_tx local verification failed at thread=%d base=%d "
			        "i=%zu val=%d\n",
			        thread_id,
			        base,
			        i,
	...
	for (size_t i = 0; i < local.size(); i++) {
		if (local[i] != (base + (int)i) * 3 + 1) {
			fprintf(stderr,
			        "FAIL: vector_tx local post-modify verification at thread=%d "
			        "base=%d\n",
			        thread_id,
			        base);
			fflush(stderr);
...
MAIN int main()
{
	...
	if (tx_count < expected_tx_count) {
		fprintf(stderr,
		        "FAIL: g_tx_count = %lld, expected at least %lld\n",
		        (long long)tx_count,
		        (long long)expected_tx_count);
		return 1;
	}
