...
MAIN int main()
{
	...
	fprintf(stderr, "  ok: %s\n", g_queue_ok == 1 ? "YES" : "NO");
	fprintf(stderr,
	        "  first cell: x=%d y=%d z=%d v=%ld (expected x=0 y=1 z=2 v=0)\n",
	        g_queue_popped_x[0],
	        g_queue_popped_y[0],
	        g_queue_popped_z[0],
	        g_queue_popped_v[0]);
	fprintf(stderr,
	        "  last cell:  x=%d y=%d z=%d v=%ld (expected x=490 y=491 z=492 v=49000)\n",
	        g_queue_popped_x[49],
	        g_queue_popped_y[49],
	        g_queue_popped_z[49],
	        g_queue_popped_v[49]);
	...
	for (int i = 0; i < g_queue_count && i < 50; i++) {
		...
		if (g_queue_popped_x[i] != ex || g_queue_popped_y[i] != ey ||
			...
			fprintf(stderr,
			        "  WRONG at cell %d: got (%d,%d,%d,%ld) expected (%d,%d,%d,%ld)\n",
			        i,
			        g_queue_popped_x[i],
			        g_queue_popped_y[i],
			        g_queue_popped_z[i],
