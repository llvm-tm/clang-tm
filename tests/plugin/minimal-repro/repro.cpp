...
__attribute__((annotate("shared"), noinline)) void tx_work(std::vector<OuterStruct> &vec,
                                                           int n,
                                                           int thread_id)
{
	for (int i = 0; i < n; i++) {
	...
	for (int i = 0; i < n; i++) {
		...
		if (elem.id != thread_id * 1000 + i) {
			fprintf(stderr,
			        "FAIL: wrong id at i=%d, got %d expected %d\n",
			        i,
			        elem.id,
			        thread_id * 1000 + i);
			exit(1);
	...
	for (int i = 0; i < n && i < 10; i++) {
		if (vec[i].inner.ids.size() != 1 || vec[i].inner.ids[0] != i) {
			fprintf(stderr,
			        "FAIL: inner vector wrong at i=%d, size=%zu val=%d\n",
			        i,
			        vec[i].inner.ids.size(),
			        vec[i].inner.ids[0]);
			exit(1);
		}
	}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < n; i++) {
		if (vec[i].id == i)
			ok++;
		if (i < 10 && vec[i].inner.ids.size() == 1)
			ok++;
	}
