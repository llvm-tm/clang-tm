...
template <typename Set> double bench_insert(Set &s, const std::vector<int> &keys)
{
	Timer t;
	t.begin();
	for (int k : keys)
...
template <typename Set> double bench_contains_hit(Set &s, const std::vector<int> &keys)
{
	size_t c = 0;
	Timer t;
	t.begin();
...
double bench_contains_miss(Set &s, const std::vector<int> &keys, int miss_base)
{
	...
	t.begin();
	for (int k : keys)
		if (s.contains(k + miss_base))
			c++;
	sink(c);
	return t.elapsed();
}

template <typename Set> double bench_erase(Set &s, const std::vector<int> &keys)
{
	Timer t;
	t.begin();
	for (int k : keys)
...
template <typename Set> double bench_iterate(Set &s)
{
	size_t c = 0;
	Timer t;
	t.begin();
...
double bench_mixed(Set &s, const std::vector<int> &keys, int miss_base)
{
	...
	for (size_t i = 0; i < half; i++) {
		...
		if (s.contains(keys[i]))
			c++;
		if (s.contains(keys[i] + miss_base))
			c++;
	}
