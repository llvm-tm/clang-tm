...
int main()
{
	...
	// Many inserts (triggers grow)
	{
		...
		for (int i = 0; i < 1000; i++) {
			if (!s.insert(i)) {
				char buf[64];
				snprintf(buf, sizeof(buf), "insert %d", i);
				CHECK(0, buf);
			}
		}
		...
		for (int i = 0; i < 1000; i++) {
			if (!s.contains(i)) {
				char buf[64];
				snprintf(buf, sizeof(buf), "contains %d", i);
				CHECK(0, buf);
			}
		}
		for (int i = 0; i < 1000; i++) {
			if (!s.erase(i)) {
				char buf[64];
				snprintf(buf, sizeof(buf), "erase %d", i);
				CHECK(0, buf);
			}
		}
	...
	// Many keys
	{
		...
		for (int i = 0; i < 100; i++) {
			if (!s.contains(i * 7 + 3)) {
				char buf[64];
				snprintf(buf, sizeof(buf), "contains key %d", i * 7 + 3);
				CHECK(0, buf);
			}
		}
	}
	...
	// Erase from middle (backward-shift correctness)
	{
		...
		for (int i = 0; i < 32; i++) {
			if (i % 2 == 0) {
				if (s.contains(i)) {
					char buf[64];
					snprintf(buf, sizeof(buf), "erased even %d", i);
					CHECK(0, buf);
				}
			} else {
				if (!s.contains(i)) {
					char buf[64];
					snprintf(buf, sizeof(buf), "kept odd %d", i);
					CHECK(0, buf);
				}
			}
		}
