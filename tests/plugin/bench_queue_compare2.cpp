...
// --- Sync TX (light) ---
__attribute__((noinline, annotate("shared"))) void inc_sync() { counter++; }

// --- Async TX (light) ---
__attribute__((noinline, annotate("async_shared"))) void inc_async() { counter++; }

// --- Heavy TX (100 increments batched) ---
__attribute__((noinline, annotate("shared"))) void inc_heavy()
{
	for (int j = 0; j < 100; j++)
		counter++;
}
...
int main()
{
	...
	int ok = (counter ==
	          100000); // only last phase matters (counter reset between phases)
	printf("%s\n", ok ? "PASS" : "FAIL");
	return ok ? 0 : 1;
}
