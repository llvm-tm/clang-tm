...
static bool tx_end() { return tinystm::commit(); }
...
static void manual_map_insert(int64_t key, int64_t val)
{
	...
	if (sz >= cap) {
		size_t new_cap = cap == 0 ? 4 : cap * 2;
		auto *new_buf = (std::pair<int64_t, int64_t> *)tm_malloc(
		    new_cap * sizeof(std::pair<int64_t, int64_t>));
		for (size_t i = 0; i < sz; i++) {
...
#define MANUAL_TX(body)                                                                  \
	do {                                                                                 \
		tx_begin();                                                                      \
		body if (!tx_end()) { tx_abort(); }                                              \
	} while (0)
	...
	for (size_t i = 1; i < g_map_data.size(); i++) {
		if (g_map_data[i].first <= g_map_data[i - 1].first) {
			printf("  FAIL: map not sorted at %zu: %lld <= %lld\n",
			       ...
			       (long long)g_map_data[i - 1].first);
			ok = false;
		}
