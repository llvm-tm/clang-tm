...
TX void tx_add(int64_t delta) { g_counter += delta; }

TX void tx_write_array(int idx, int64_t val) { g_array[idx] = val; }

TX int64_t tx_read_array(int idx) { return g_array[idx]; }

TX int64_t tx_read_counter() { return g_counter; }
...
int main(int argc, char **argv)
{
	...
	if (strcmp(mode, "init") == 0) {
		printf("MODE INIT: counter=%ld array=[%ld,%ld,%ld,%ld]\n",
		       ...
		       (long)tx_read_array(0),
		       (long)tx_read_array(1),
		       (long)tx_read_array(2),
		       (long)tx_read_array(3));

	} else if (strcmp(mode, "run") == 0) {
		printf("MODE RUN: initial_counter=%ld delta=%ld iters=%d threads=%d\n",
		       (long)initial_counter,
		       (long)delta,
		       iterations,
		       n_threads);
