...
template <typename T> static explicit_slist::Node<T> *alloc_sl_node(const T &val)
{
...
template <typename T> static explicit_slist::Node<T> *alloc_sl_node_tm(const T &val)
{
	auto *n = (explicit_slist::Node<T> *)expli::TM<int>::malloc(
	    sizeof(explicit_slist::Node<T>));
	n->data = val;
	n->next = nullptr;
	return n;
}
...
static void test_rbtree_standalone()
{
	...
	for (int k : keys) {
		...
		CHECK(explicit_rbtree::insert<false>(&tree, n) == nullptr, "rbtree insert new");
		nodes.push_back(n);
	}

	for (int k : keys) {
		CHECK(explicit_rbtree::contains<false>(&tree, k), "rbtree contains after insert");
	...
	CHECK(!explicit_rbtree::contains<false>(&tree, -1), "rbtree absent key");
...
static void test_rbtree_tx()
{
	...
	expli::TM<int>::transaction([&]() {
		...
		CHECK(explicit_rbtree::insert<true>(tree, n1) == nullptr, "rbtree tx insert n1");
		CHECK(explicit_rbtree::insert<true>(tree, n2) == nullptr, "rbtree tx insert n2");
		CHECK(explicit_rbtree::insert<true>(tree, n3) == nullptr, "rbtree tx insert n3");
...
static void test_slist_standalone()
{
	...
	for (int k : keys) {
		...
		CHECK(explicit_slist::insert<false>(&list, n), "slist insert");
	}

	for (int k : keys) {
		CHECK(explicit_slist::contains<false>(&list, k), "slist contains after insert");
	}
...
static void test_hashmap_standalone()
{
	...
	size_t cap = 16384;
	map.slots = (explicit_hashmap::Slot<int, int> *)
	    std::calloc(cap, sizeof(explicit_hashmap::Slot<int, int>));
	map.capacity = cap;
	map.size = 0;
	...
	for (int k : keys) {
		CHECK(explicit_hashmap::insert<false>(&map, k, k * 2), "hashmap insert");
	}
	...
	CHECK(explicit_hashmap::erase<false>(&map, keys[0]), "hashmap erase");
...
static void test_hashmap_tx()
{
	...
	size_t cap = 4096;
	map->slots = (explicit_hashmap::Slot<int, int> *)
	    expli::TM<int>::calloc(cap, sizeof(explicit_hashmap::Slot<int, int>));
	map->capacity = cap;
	map->size = 0;

	expli::TM<int>::transaction([&]() {
		for (int i = 0; i < 2000; i++) {
			CHECK(explicit_hashmap::insert<true>(map, i, i * 3), "hashmap tx insert");
		}
                                 ...
                                 const std::vector<int> &my_keys)
{
	...
	for (int k : my_keys) {
		...
		expli::TM<int>::transaction([&]() { explicit_rbtree::insert<true>(tree, n); });
	}

	for (int k : my_keys) {
		expli::TM<int>::transaction([&]() {
			CHECK(explicit_rbtree::contains<true>(tree, k), "stress rbtree contains own");
		});
	}
                                  ...
                                  const std::vector<int> &my_keys)
{
	...
	for (int k : my_keys) {
		expli::TM<int>::transaction([&]() {
			...
			CHECK(v != nullptr && *v == k * 2, "stress hashmap find own");
		});
	}
...
static void test_hashmap_multithread()
{
	...
	for (int t = 0; t < NTH; t++)
		for (int k : thread_keys[t]) {
			int *v = explicit_hashmap::find<false>(map, k);
			CHECK(v != nullptr && *v == k * 2, "hashmap multi-threaded verify");
		}
