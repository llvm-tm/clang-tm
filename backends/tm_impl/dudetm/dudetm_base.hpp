...
{
	if (g_ctrl)
		g_ctrl->shutdown.store(true, std::memory_order_relaxed);
}
...
inline void publish_batch(const DUDERedoEntry *entries, size_t count)
{
	if (count == 0)
		return;
...
static inline size_t addr_to_file_off(uintptr_t addr)
{
...
inline void replay_op(const DUDERedoEntry &e, uint8_t *local_persist)
{
	...
	case OP_WRITE: {
		...
		stm::write_value_to_addr(reinterpret_cast<void *>(e.addr), e.val, e.type);
		if (off != (size_t)-1) {
			size_t sz = 8;
			switch (e.type) {
			case stm::ValueType::UINT8:
				sz = 1;
				break;
			case stm::ValueType::UINT16:
				sz = 2;
				break;
			case stm::ValueType::UINT32:
			case stm::ValueType::FLOAT:
				sz = 4;
				break;
			default:
				break;
			}
			memcpy(local_persist + off, reinterpret_cast<void *>(e.addr), sz);
		}
		break;
	}
	case OP_MALLOC: {
...
inline void replayer_loop()
{
	int fd = open(PERSIST_FILE, O_RDWR, 0644);
	if (fd < 0) {
		perror("dudetm replayer: open persist");
		_exit(1);
	}

	uint8_t *local_persist = (uint8_t *)mmap((void *)PERSIST_MMAP_ADDR,
	                                         g_persist_size,
	...
	fprintf(stderr,
	        "[DUDETM] replayer exiting, pid=%d, ops=%llu\n",
	        getpid(),
...
// ── Init ───────────────────────────────────────────────────────
inline void init(uint32_t sym_count, void **sym_addresses, uint64_t *sym_sizes)
{
	...
	if (pfd < 0) {
		perror("dudetm: open persist");
		exit(1);
	}

	struct stat pst;
	bool file_has_data = (fstat(pfd, &pst) == 0 && (size_t)pst.st_size >= g_persist_size);

	if (ftruncate(pfd, (off_t)g_persist_size) < 0) {
		perror("dudetm: ftruncate persist");
		close(pfd);
		exit(1);
	}

	g_persist_base = (uint8_t *)mmap((void *)PERSIST_MMAP_ADDR,
	                                 g_persist_size,
	                                 PROT_READ | PROT_WRITE,
	                                 MAP_SHARED | MAP_FIXED,
	                                 pfd,
	...
	if (g_persist_base == MAP_FAILED) {
		g_persist_base = (uint8_t *)
		    mmap(nullptr, g_persist_size, PROT_READ | PROT_WRITE, MAP_SHARED, pfd, 0);
		if (g_persist_base == MAP_FAILED) {
			perror("dudetm: mmap persist");
			close(pfd);
			exit(1);
		}
		fprintf(stderr,
		        "[DUDETM] WARNING: persist at %p (not fixed addr)\n",
		        (void *)g_persist_base);
	}
	close(pfd);
