...
void tm_exit_thread()
#endif
{ tm_hook_exit_thread(); norecbf::exit_thread(); }
...
{
	return norecbf::tm_read_i2(addr);
}

static uint32_t real_tm_read_i4(uint32_t *addr)
{
	return norecbf::tm_read_i4(addr);
}

static uint64_t real_tm_read_i8(uint64_t *addr)
{
	return norecbf::tm_read_i8(addr);
}
...
static void *real_tm_read_ptr(void **addr) {
    return norecbf::tm_read_ptr(addr);
}
