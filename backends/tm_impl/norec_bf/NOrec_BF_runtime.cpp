...
extern const TMRealHooks g_norecbf_hooks;
...
static inline bool check_retry_or_init()
{
	if (tm_longjmp_ret != 0) {
		...
		// norecbf::begin() has already been called by the retry path
		return true;
	}
	// First time or no plugin: set up setjmp if not done
	if (!tm_is_init_ready) {
		norecbf::jmpbuf = &tm_jmpbuf;
...
TM_PLUGIN_LIFECYCLE_FN(do_tm_init, void tm_init())
{
	...
	norecbf::init();
	tm_register_real_hooks(&g_norecbf_hooks);
}
...
static uint8_t real_tm_read_i1(uint8_t *addr) { return norecbf::tm_read_i1(addr); }

static uint16_t real_tm_read_i2(uint16_t *addr) { return norecbf::tm_read_i2(addr); }

static uint32_t real_tm_read_i4(uint32_t *addr) { return norecbf::tm_read_i4(addr); }

static uint64_t real_tm_read_i8(uint64_t *addr) { return norecbf::tm_read_i8(addr); }

static float real_tm_read_f4(float *addr) { return norecbf::tm_read_f4(addr); }

static double real_tm_read_f8(double *addr) { return norecbf::tm_read_f8(addr); }

static void *real_tm_read_ptr(void **addr) { return norecbf::tm_read_ptr(addr); }

static void real_tm_write_i1(uint8_t *addr, uint8_t val)
{
	norecbf::tm_write_i1(addr, val);
}

static void real_tm_write_i2(uint16_t *addr, uint16_t val)
{
	norecbf::tm_write_i2(addr, val);
}

static void real_tm_write_i4(uint32_t *addr, uint32_t val)
{
	norecbf::tm_write_i4(addr, val);
}

static void real_tm_write_i8(uint64_t *addr, int64_t val)
{
	norecbf::tm_write_i8(addr, static_cast<uint64_t>(val));
}

static void real_tm_write_f4(float *addr, float val) { norecbf::tm_write_f4(addr, val); }

static void real_tm_write_f8(double *addr, double val)
{
	norecbf::tm_write_f8(addr, val);
}

static void real_tm_write_ptr(void **addr, void *val)
{
	norecbf::tm_write_ptr(addr, val);
}
...
static void real_tm_free(void *ptr)
{
	...
	TM_EVENT(FREE, ptr, 0);
	if (g_in_tx) {
		norecbf::tm_write_i1(reinterpret_cast<uint8_t *>(ptr), 0);
...
void tm_read_i16(void *addr, void *out)
{
	...
	out_words[0] = norecbf::tm_read_i8(static_cast<uint64_t *>(addr) + 0);
	out_words[1] = norecbf::tm_read_i8(static_cast<uint64_t *>(addr) + 1);
}
...
void tm_read_i32(void *addr, void *out)
{
	auto *out_words = static_cast<uint64_t *>(out);
	for (int i = 0; i < 4; i++)
		out_words[i] = norecbf::tm_read_i8(static_cast<uint64_t *>(addr) + i);
}
...
void tm_write_i16(void *addr, void *val)
{
	...
	for (int i = 0; i < 2; i++)
		norecbf::tm_write_i8(static_cast<uint64_t *>(addr) + i, val_words[i]);
}
...
void *tm_read_z(uint8_t *addr, uint64_t len)
{
	...
	for (uint64_t i = 0; i < len / 8; i++) {
		tm_buffer[i] = norecbf::tm_read_i8(((uint64_t *)addr) + i);
	}
	uint64_t rem = len % 8;
	for (uint64_t i = 0; i < rem; i++) { // rem > 0
		tm_buffer[i] = norecbf::tm_read_i1(addr + (len - rem - 1) + i);
	}
	return tm_buffer;
}
...
{
	for (uint64_t i = 0; i < len / 8; i++) {
		norecbf::tm_write_i8(((uint64_t *)dst) + i, *(((uint64_t *)src) + i));
	}
	uint64_t rem = len % 8;
	for (uint64_t i = 0; i < rem; i++) { // rem > 0
		norecbf::tm_write_i1(dst + (len - rem - 1) + i, *(src + (len - rem - 1) + i));
	}
}
...
void tm_memset(uint8_t *addr, uint8_t val, uint64_t len)
{
	for (uint64_t i = 0; i < len; i++) {
		norecbf::tm_write_i1(&addr[i], val);
	}
}
...
const TMRealHooks g_norecbf_hooks = {
    .begin = real_tm_begin,
    .end = real_tm_end,
