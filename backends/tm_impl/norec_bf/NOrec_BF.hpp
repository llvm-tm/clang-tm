...
// NOrec-BF: NOrec + Bloom-filter fast validation.
//
// Standard NOrec validates a transaction by RE-READING the entire read set
// from memory and comparing values (value-based validation).  NOrec-BF
// replaces this memory traversal with an O(1) (w.r.t. read-set size) Bloom
// filter disjointness test:
//
//   - A global committed-writes Bloom filter g_gc accumulates the addresses
//     written by committed transactions (inserted under the global lock).
//   - Each transaction maintains a read-set Bloom filter read_bf (addresses
//     it read) and records the filter generation at begin().
//   - At validate(): if the generation is unchanged AND read_bf ∩ g_gc is
//     empty, then NO committed transaction wrote any address in the
//     read-set since the last filter rebuild.  Because Bloom filters have
//     no false negatives, this is a SOUND proof that validation passes —
//     without touching memory.  Cost: O(kBloomWords) word ANDs.
//   - If the intersection is non-empty (false positive possible) or the
//     filter was rebuilt while the transaction was in flight, NOrec-BF
//     falls back to exact value-based validation (the original algorithm).
//
// The generation counter bounds saturation: when g_gc is ~50% full a writer
// (under the lock) bumps the generation and clears the filter.  In-flight
// transactions see the generation change and take the exact path, so the
// fast path can never observe a rebuilt filter as "definitely disjoint".
//
...
// NOrec-BF filter width (in 64-bit words).  The global committed-writes
// filter and every transaction's read-set filter share this width so the
// disjointness test is a simple word-AND.
static constexpr size_t kBloomWords = 64;
...
public:
	...
	// NOrec-BF: read-set Bloom filter (addresses this transaction read) and
	// the committed-writes filter generation observed at begin().  If
	// g_gc_gen still equals gen at validate() time, g_gc was not rebuilt
	// while this transaction was in flight and the disjointness test is a
	// sound proof that no committed writer touched the read-set.
	stm::BloomFilter<kBloomWords> read_bf;
	uint64_t gen = 0;
...
extern std::atomic<norecbf::word_t> thr_counter;

// NOrec-BF committed-writes filter + generation (defined in
// NOrec_BF_globals.hpp).
extern std::atomic<uint64_t> g_gc_gen;
extern stm::BloomFilter<kBloomWords> g_gc;
...
begin()     //
{
	...
	tx->write_set.clear();
	tx->read_bf.clear();
	tx->gen = g_gc_gen.load(std::memory_order_acquire);
...
validate()    //
{
	...
	while (true) {
		...
		// NOrec-BF fast path.  If the committed-writes filter was NOT rebuilt
		// since begin() (generation unchanged) and the read-set filter is
		// disjoint from g_gc, then no committed writer touched any read
		// address since the last rebuild — and, because the generation
		// matched, none of those writes can be from this transaction's
		// interval.  Bloom filters have no false negatives, so this is a
		// sound proof that validation passes without re-reading memory.
		// The clock double-check (same pattern as the exact path below)
		// brackets the filter read, so a concurrent commit/rebuild under the
		// lock is always detected and retried.
		if (tx->gen == g_gc_gen.load(std::memory_order_acquire) &&
		    tx->read_bf.disjoint_with(g_gc)) {
			if (time == get_clock())
				return time;
			continue;
		}
...
commit()    //
{
	...
	// NOrec-BF: record every committed write in the global filter while
	// still holding the lock, so no reader can observe a half-updated
	// filter (a reader's clock double-check retries if the lock was taken
	// while it read g_gc).  When the filter saturates (~50% bits set),
	// bump the generation and rebuild it: in-flight transactions fall back
	// to exact validation via the generation mismatch, and fresh ones get a
	// clean filter.
	for (auto &w : tx->write_set) {
		if (w.addr != nullptr)
			g_gc.insert(reinterpret_cast<uint64_t>(w.addr));
	}
	if (g_gc.popcount() > kBloomWords * 64 / 2) {
		g_gc_gen.fetch_add(1, std::memory_order_acq_rel);
		g_gc.clear();
	}
...
)
{
	...
	tx->read_bf.insert(reinterpret_cast<uint64_t>(addr));
