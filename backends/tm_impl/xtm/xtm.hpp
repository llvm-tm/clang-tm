...
inline void free_private_page(void *p) { std::free(p); }
...
inline bool commit()
{
	...
	if (!tx->read_only) {
		...
		for (auto &kv : tx->read_set) {
			...
			uint64_t current_ver = g_xadt[idx].version.load(std::memory_order_acquire);
...
inline any_type_t read_word(Transaction *tx, void *addr, ValueType sz)
{
	...
	if (wit != tx->write_set.end()) {
		...
		return read_value_from_addr((void *)((uintptr_t)wit->second + offset), sz);
	}
...
inline void write_word(Transaction *tx, void *addr, any_type_t val, ValueType sz)
{
	...
	uint64_t expected = 0;
	if (!g_xadt[idx].owner_tx_id.compare_exchange_strong(expected,
	                                                     tx->id,
