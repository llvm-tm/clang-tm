...
{
	return xtm::tm_read<uint8_t, xtm::ValueType::UINT8>(addr);
}
static uint16_t real_tm_read_i2(uint16_t *addr)
{
	return xtm::tm_read<uint16_t, xtm::ValueType::UINT16>(addr);
}
static uint32_t real_tm_read_i4(uint32_t *addr)
{
	return xtm::tm_read<uint32_t, xtm::ValueType::UINT32>(addr);
}
static uint64_t real_tm_read_i8(uint64_t *addr)
{
	return xtm::tm_read<uint64_t, xtm::ValueType::UINT64>(addr);
}

static void real_tm_write_i1(uint8_t *addr, uint8_t val)
{
	xtm::tm_write<uint8_t, xtm::ValueType::UINT8>(addr, val);
}
static void real_tm_write_i2(uint16_t *addr, uint16_t val)
{
	xtm::tm_write<uint16_t, xtm::ValueType::UINT16>(addr, val);
}
static void real_tm_write_i4(uint32_t *addr, uint32_t val)
{
	xtm::tm_write<uint32_t, xtm::ValueType::UINT32>(addr, val);
}
static void real_tm_write_i8(uint64_t *addr, int64_t val)
{
	xtm::tm_write<uint64_t, xtm::ValueType::UINT64>(addr, val);
}

static float real_tm_read_f4(float *addr)
{
	return xtm::tm_read<float, xtm::ValueType::FLOAT>(addr);
}
static double real_tm_read_f8(double *addr)
{
	return xtm::tm_read<double, xtm::ValueType::DOUBLE>(addr);
}

static void real_tm_write_f4(float *addr, float val)
{
	xtm::tm_write<float, xtm::ValueType::FLOAT>(addr, val);
}
static void real_tm_write_f8(double *addr, double val)
{
	xtm::tm_write<double, xtm::ValueType::DOUBLE>(addr, val);
}

static void *real_tm_read_ptr(void **addr)
{
	return xtm::tm_read<void *, xtm::ValueType::POINTER>(addr);
}
static void real_tm_write_ptr(void **addr, void *val)
{
	xtm::tm_write<void *, xtm::ValueType::POINTER>(addr, val);
}
