...
inline uint64_t make_version_entry(uint64_t ver) { return ver << 1; }
inline uint64_t read_version(uint64_t entry) { return entry >> 1; }
inline bool is_locked(uint64_t entry) { return (entry & VERSION_LOCK_BIT) != 0; }
...
// ── Clock helpers ────────────────────────────────────────────────
inline uint64_t get_clock() { return g_global_clock.load(std::memory_order_acquire); }
...
inline bool commit()
{
	...
	if (!tx->read_only && !tx->write_set.empty()) {
		...
		std::sort(addrs.begin(), addrs.end(), [](void *a, void *b) {
