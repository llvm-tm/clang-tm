...
static void real_tm_write_ptr(void **addr, void *val)
{
	...
	*addr = val;
	std::atomic_thread_fence(std::memory_order_release);
	if (off != (size_t)-1)
		persist_write(off, &val, sizeof(void *));
}
...
{
	if (!g_mmap_base || size == 0)
		return malloc(size);
	if (g_in_tx) {
		// Bump-allocate from the persistent heap
		uint64_t old_bump = __atomic_fetch_add(reinterpret_cast<uint64_t *>(g_mmap_base +
		                                                                    16),
		                                       size,
		                                       __ATOMIC_RELAXED);
		if (old_bump + size > PERSIST_HEAP_SIZE) {
			fprintf(stderr,
			        "PersistentSGL: OOM (%zu > %zu)\n",
			        old_bump + size,
			        PERSIST_HEAP_SIZE);
			return malloc(size);
		}
...
static void *real_tm_get_thread_state() { return (void *)&g_tm_persist_state; }
...
#endif
{
	...
	g_mmap_base = (uint8_t *)mmap((void *)PERSIST_MMAP_FIXED_ADDR,
	                              g_mmap_size,
	...
	if (g_mmap_base == MAP_FAILED) {
		...
		g_mmap_base = (uint8_t *)
		    mmap(nullptr, g_mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
		if (g_mmap_base == MAP_FAILED) {
		...
		fprintf(stderr,
		        "PersistentSGL: WARNING: mmap at 0x%llx failed, using ASLR address %p\n"
		        "  std::map and similar pointer-based containers will NOT persist "
		        "correctly.\n",
		        (unsigned long long)PERSIST_MMAP_FIXED_ADDR,
		        (void *)g_mmap_base);
	} else {
	...
	if (file_has_data) {
		...
		if (fmagic == magic && fver == version && fcount == n) {
			...
			fprintf(stderr,
			        "PersistentSGL: restored %u symbols from %s\n",
			        n,
			        PERSIST_FILE);
		} else {
...
void tm_write_i16(void *addr, void *val)
{
	...
	for (int i = 0; i < 2; i++) {
		...
		std::atomic_thread_fence(std::memory_order_release);
		if (off != (size_t)-1)
			persist_write(off, &val_words[i], 8);
	}
}
...
void tm_write_z(volatile uint8_t *dst, volatile uint8_t *src, uint64_t len)
{
	...
	std::atomic_thread_fence(std::memory_order_release);
	if (off != (size_t)-1)
		persist_write(off, (const void *)src, len);
}
