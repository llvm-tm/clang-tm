...
TM_PLUGIN_LIFECYCLE_FN(do_tm_exit, void tm_exit()) { stm::tm_region_destroy(); }
...
static uint8_t real_tm_read_i1(uint8_t *addr) { return swisstm::tm_read_i1(addr); }

static uint16_t real_tm_read_i2(uint16_t *addr) { return swisstm::tm_read_i2(addr); }

static uint32_t real_tm_read_i4(uint32_t *addr) { return swisstm::tm_read_i4(addr); }

static uint64_t real_tm_read_i8(uint64_t *addr) { return swisstm::tm_read_i8(addr); }
...
static void real_tm_write_f4(float *addr, float val) { swisstm::tm_write_f4(addr, val); }
