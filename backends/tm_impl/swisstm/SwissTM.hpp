...
public:
	...
	{
		return reinterpret_cast<word_t *>((uintptr_t)addr &
		                                  ~((uintptr_t)sizeof(word_t) - 1));
	}
	...
	static bool cm_should_abort(TxDescriptor *tx, OwnershipRecord *orec)
	{
		...
		if (owner != UNLOCKED) {
			...
			word_t owner_cm_ts = __atomic_load_n(&owner_entry->owner->cm_ts,
			                                     __ATOMIC_ACQUIRE);
			...
			if (idx_it != tx->write_log_index.end()) {
				...
				if (e->type == VT) {
					if constexpr (std::is_same_v<T, uint8_t>)
						return e->new_value.u1;
					else if constexpr (std::is_same_v<T, uint16_t>)
						return e->new_value.u2;
					else if constexpr (std::is_same_v<T, uint32_t>)
						return e->new_value.u4;
					else if constexpr (std::is_same_v<T, uint64_t>)
						return e->new_value.u8;
					else if constexpr (std::is_same_v<T, float>)
						return e->new_value.f4;
					else if constexpr (std::is_same_v<T, double>)
						return e->new_value.f8;
					else
						return static_cast<T>(e->new_value.ptr);
				}
				...
				while (true) {
					if (is_locked(w_lock_val)) {
						if (stm::tm_token_soft_spin(tx->succ_abort_count, tx->id, 3)) {
							while (
							    is_locked(orec->w_lock.load(std::memory_order_relaxed))) {
								stm::tm_cpu_relax();
							}
	...
	static void commit(TxDescriptor *tx)
	{
		...
		for (auto &re : tx->read_set) {
			...
			if (it != locked_orecs.end()) {
				...
				word_t old = re.orec->r_lock.exchange(READ_LOCKED,
