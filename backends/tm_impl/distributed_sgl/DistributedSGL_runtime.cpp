...
#endif
{
	...
	void *base = mmap(nullptr, g_mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if (base == MAP_FAILED) {
		perror("mmap");
		exit(1);
	}
	...
	int expected = 0;
	if (g_state->process_count.compare_exchange_strong(expected,
	                                                   nproc,
...
void tm_memset(volatile uint8_t *a, uint8_t v, uint64_t l) { memset((void *)a, v, l); }
