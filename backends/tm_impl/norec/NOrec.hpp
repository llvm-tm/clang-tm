...
inline void exit()
{
	...
	// g_tm_abort_count) have static storage duration and are reclaimed by
	// the OS at process exit. The only dynamic allocation, the per-thread
	// Transaction, is freed by exit_thread(). The 64 GB TM region mapping
	// is likewise OS-reclaimed (see stm::tm_region_destroy). The old
