...
__device__ uint64_t csmv_device_read(void *data_addr, uint64_t start_clock) {
    ...
    CSMVVersionNode *node = atomic_load(&entry->head);
    ...
    while (node) {
        ...
        int any_match = __any_sync(0xFFFFFFFF, match);
        if (any_match) {
...
__device__ void csmv_device_write(CSMVObjectEntry *entry,
    ...
    if (threadIdx.x == 0) {
        ...
        new_node->next = atomic_load(&entry->head);
        atomic_store(&entry->head, new_node);
    }
    __syncwarp();
}
...
__device__ uint64_t csmv_device_begin() {
    return atomic_add((unsigned long long*)d_csmv_clock, 0);
}

__device__ uint64_t csmv_device_commit() {
    return atomic_add((unsigned long long*)d_csmv_clock, 1) + 1;
}
