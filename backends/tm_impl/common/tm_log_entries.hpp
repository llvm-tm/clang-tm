...
template <typename A = void *> struct BitmapRedoEntry {
...
template <typename A = void *> struct BitmapReadEntry {
	A addr{};        // 8-byte aligned address
...
template <typename A = void *> struct BitmapFullEntry {
	A addr{}; // 8-byte aligned address
	word_t new_value{};
	word_t old_value{};
	uint8_t new_valid{};
...
template <typename A = void *> struct RedoEntry {
	A addr{};
	any_type_t new_val{};
	ValueType type{};
};

template <typename A = void *> struct UndoEntry {
	A addr{};
	any_type_t old_val{};
	ValueType type{};
};

template <typename A = void *> struct FullEntry {
	A addr{};
	any_type_t new_val{};
	any_type_t old_val{};
	ValueType type{};
...
template <typename A = void *> struct ReadEntry {
	A addr{};
