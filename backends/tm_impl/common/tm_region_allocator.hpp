...
{
#ifdef __APPLE__
	g_tm_stack_high = static_cast<const char *>(pthread_get_stackaddr_np(pthread_self()));
...
{
	return const_cast<char *>(chunk_base) + g_sc_data_off[sc] +
	       block_idx * g_sc_block_size[sc];
}
...
inline void tm_region_free(void *ptr) noexcept
{
	...
	if (hdr->magic == CHUNK_MAGIC && hdr->size_class < MAX_CLASSES &&
	    g_sc_block_size[hdr->size_class] > 0) {
