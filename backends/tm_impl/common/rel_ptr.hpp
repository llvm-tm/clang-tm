...
template <typename T> class RelPtr
{
...
public:
	...
	// Copy
	RelPtr(const RelPtr &other) noexcept
	    : off_(other.off_)
	{
	}
	RelPtr &operator=(const RelPtr &other) noexcept
	{
		off_ = other.off_;
		return *this;
	}
	RelPtr &operator=(T *ptr) noexcept
	{
		*this = RelPtr(ptr);
		return *this;
	}
	RelPtr &operator=(std::nullptr_t) noexcept
	{
		off_ = 0;
		return *this;
	}
	...
	template <typename U>
	RelPtr(const RelPtr<U> &other) noexcept
	    : off_(other.off_)
	{
	}
	template <typename U> RelPtr &operator=(const RelPtr<U> &other) noexcept
	{
		off_ = other.off_;
		return *this;
	}
...
private:
	...
	static char *get_base()
	{
		...
		if (!b) {
			fprintf(stderr,
			        "RelPtr: base address not set! Call RelPtr<T>::set_base() first.\n");
			std::abort();
		}
		return b;
	}
...
public:
	...
	{
		return off_ == 0 ? nullptr : reinterpret_cast<void *>(get_base() + off_);
	}
	explicit operator bool() const noexcept { return off_ != 0; }
	int64_t offset() const noexcept { return off_; }
	bool operator==(std::nullptr_t) const noexcept { return off_ == 0; }

	template <typename T> RelPtr<T> to() const noexcept
	{
		RelPtr<T> r;
		r = *this;
		return r;
...
private:
	...
	static char *get_base()
	{
		char *b = base_ref();
		if (!b) {
			std::fprintf(stderr, "RelPtr: base not set\n");
			std::abort();
		}
		return b;
	}
...
namespace std
{
template <typename T> struct hash<RelPtr<T>> {
