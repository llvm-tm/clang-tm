...
#define TM_PERF_INC(c, f)                                                                \
	do {                                                                                 \
		++(c).f;                                                                         \
	} while (0)
#define TM_PERF_SET(c, f, v)                                                             \
	do {                                                                                 \
		(c).f = (v);                                                                     \
	} while (0)
#define TM_PERF_ADD(c, f, v)                                                             \
	do {                                                                                 \
		(c).f += (v);                                                                    \
	} while (0)

#define TM_PERF_BEGIN(c)                                                                 \
	do {                                                                                 \
		if (!(c).t_start)                                                                \
			(c).t_start = tm_perf_now();                                                 \
	} while (0)
#define TM_PERF_END(c)                                                                   \
	do {                                                                                 \
		(c).t_end = tm_perf_now();                                                       \
	} while (0)
...
#define TM_PERF_SCOPE(c, field)                                                          \
	for (int64_t _p_t0 = tm_perf_now(), _p_flag = 1; _p_flag;                            \
	     _p_flag = 0, (c).field += tm_perf_now() - _p_t0)

#define TM_PERF_REGISTER(c)                                                              \
	do {                                                                                 \
		tm_perf_registry().all.push_back((c));                                           \
	} while (0)
...
inline void tm_perf_dump()
{
	...
	for (auto *c : reg.all) {
		...
		if (d > wall_s)
			wall_s = d;
		fprintf(stderr,
		        "%-18s %8lld %8lld %8lld %8lld %8lld %8lld %8lld %8lld\n",
		        c->name,
	...
	fprintf(stderr, "\n");
	fprintf(stderr,
	        "Wall: %.3f s  Rate: %.0f tx/s\n",
	        wall_s,
	        wall_s > 0 ? q / wall_s : 0);
	if (q > 0) {
		fprintf(stderr,
		        "Avg: alloc=%.0f push=%.0f exec=%.0f wait=%.0f  overhead=%.0f ns\n",
		        (double)an / q,
		        (double)pn / q,
		        (double)en / q,
		        (double)wn / q,
	...
	if (ws > 0)
		fprintf(stderr, "Worker spins: %lld\n", (long long)ws);
	if (wt > 0)
		fprintf(stderr, "Worker idle:  %.3f s\n", (double)wt / 1e9);
#endif
}
