...
template <typename T, typename Hash = std::hash<T>> class TMSafeHashSet
{
	struct Slot {
		bool occupied = false;
		T value;
	...
	size_t hash_idx(const T &v) const { return Hash{}(v) & (m_cap - 1); }
...
public:
	...
	{
		Slot *p = m_data;
		while (p < m_data + m_cap && !p->occupied)
			++p;
	...
	bool contains(const T &v) const { return find_occupied(v) < m_cap; }
