...
{
	switch (t) {
	case EventType::TX_BEGIN:
		return "TX_BEGIN";
	case EventType::TX_END:
		return "TX_END";
	case EventType::TX_ABORT:
		return "TX_ABORT";
	case EventType::TX_RETRY:
		return "TX_RETRY";
	case EventType::READ_LOCK_ACQUIRE:
		return "READ_LOCK_ACQUIRE";
	case EventType::READ_VERSION_CHECK:
		return "READ_VERSION_CHECK";
	case EventType::WRITE_LOCK_ACQUIRE:
		return "WRITE_LOCK_ACQUIRE";
	case EventType::WRITE_SET_INSERT:
		return "WRITE_SET_INSERT";
	case EventType::COMMIT_LOCK_ACQUIRE:
		return "COMMIT_LOCK_ACQUIRE";
	case EventType::COMMIT_WRITEBACK:
		return "COMMIT_WRITEBACK";
	case EventType::COMMIT_SUCCESS:
		return "COMMIT_SUCCESS";
	case EventType::GAP_CHECK:
		return "GAP_CHECK";
	case EventType::LOCK_RELEASE:
		return "LOCK_RELEASE";
	case EventType::RETRY_END:
		return "RETRY_END";
	case EventType::MALLOC:
		return "MALLOC";
	case EventType::FREE:
		return "FREE";
	case EventType::FLUSH_DEFERRED:
		return "FLUSH_DEFERRED";
	case EventType::CLEAR_SPEC_ALLOC:
		return "CLEAR_SPEC_ALLOC";
	case EventType::DEFERRED_FREE_ACTUAL:
		return "DEFERRED_FREE_ACTUAL";
	case EventType::DOUBLE_FREE:
		return "DOUBLE_FREE";
	case EventType::MOVE_DEFERRED_TO_RETIRED:
		return "MOVE_DEFERRED_TO_RETIRED";
	case EventType::FLUSH_RETIRED:
...
struct EventRing {
	...
	void dump(size_t count = 0, FILE *fp = stderr)
	{
		...
		fprintf(fp,
		        "--- Event log (%llu entries, dumping from #%llu) ---\n",
		        (unsigned long long)h,
		        (unsigned long long)start);
		for (uint64_t i = start; i < h; i++) {
			EventEntry &e = entries[i & EVENT_RING_MASK];
			fprintf(fp,
			        "[%12llu] thr=0x%llx %-20s addr1=0x%llx addr2=0x%llx data=%llu\n",
...
#define TM_EVENT(type, addr1, data)                                                      \
	do {                                                                                 \
		stm::get_event_ring().log(stm::EventType::type,                                  \
		                          (uint64_t)(addr1),                                     \
		                          0,                                                     \
		                          (uint64_t)(data));                                     \
	} while (0)
