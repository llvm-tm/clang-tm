...
static void *stub_tm_get_env() { return (void *)&tm_jmpbuf; }

static void stub_tm_set_jmpbuf(void *buf) { (void)buf; }

static void *stub_tm_get_thread_state() { return (void *)&tm_nested_call_counter; }
...
__attribute__((weak)) void (*tm_trace)(uint32_t,
                                       void *,
                                       uint64_t,
                                       uint64_t) = tm_trace_stub;
...
void tm_hook_exit_thread() { s_thread_count.fetch_sub(1); }

const TMRealHooks *tm_get_real_hooks() { return &s_real_hooks; }
...
void tm_swap_runtime(const TMRealHooks *hooks)
{
	std::lock_guard<std::mutex> lock(s_hook_mutex);
	if (hooks->begin)
		tm_begin = hooks->begin;
	if (hooks->end)
		tm_end = hooks->end;
	if (hooks->malloc)
		tm_malloc = hooks->malloc;
	if (hooks->calloc)
		tm_calloc = hooks->calloc;
	if (hooks->realloc)
		tm_realloc = hooks->realloc;
	if (hooks->free)
		tm_free = hooks->free;
	if (hooks->read_i1)
		tm_read_i1 = hooks->read_i1;
	if (hooks->read_i2)
		tm_read_i2 = hooks->read_i2;
	if (hooks->read_i4)
		tm_read_i4 = hooks->read_i4;
	if (hooks->read_i8)
		tm_read_i8 = hooks->read_i8;
	if (hooks->read_f4)
		tm_read_f4 = hooks->read_f4;
	if (hooks->read_f8)
		tm_read_f8 = hooks->read_f8;
	if (hooks->read_ptr)
		tm_read_ptr = hooks->read_ptr;
	if (hooks->write_i1)
		tm_write_i1 = hooks->write_i1;
	if (hooks->write_i2)
		tm_write_i2 = hooks->write_i2;
	if (hooks->write_i4)
		tm_write_i4 = hooks->write_i4;
	if (hooks->write_i8)
		tm_write_i8 = hooks->write_i8;
	if (hooks->write_f4)
		tm_write_f4 = hooks->write_f4;
	if (hooks->write_f8)
		tm_write_f8 = hooks->write_f8;
	if (hooks->write_ptr)
		tm_write_ptr = hooks->write_ptr;
	if (hooks->get_env)
		tm_get_env = hooks->get_env;
	if (hooks->set_jmpbuf)
		tm_set_jmpbuf = hooks->set_jmpbuf;
	if (hooks->get_thread_state)
		tm_get_thread_state = hooks->get_thread_state;
	// Also update s_real_hooks so tm_hook_init_thread() doesn't revert
	if (hooks->begin)
		s_real_hooks.begin = hooks->begin;
	if (hooks->end)
		s_real_hooks.end = hooks->end;
	if (hooks->malloc)
		s_real_hooks.malloc = hooks->malloc;
	if (hooks->calloc)
		s_real_hooks.calloc = hooks->calloc;
	if (hooks->realloc)
		s_real_hooks.realloc = hooks->realloc;
	if (hooks->free)
		s_real_hooks.free = hooks->free;
	if (hooks->read_i1)
		s_real_hooks.read_i1 = hooks->read_i1;
	if (hooks->read_i2)
		s_real_hooks.read_i2 = hooks->read_i2;
	if (hooks->read_i4)
		s_real_hooks.read_i4 = hooks->read_i4;
	if (hooks->read_i8)
		s_real_hooks.read_i8 = hooks->read_i8;
	if (hooks->read_f4)
		s_real_hooks.read_f4 = hooks->read_f4;
	if (hooks->read_f8)
		s_real_hooks.read_f8 = hooks->read_f8;
	if (hooks->read_ptr)
		s_real_hooks.read_ptr = hooks->read_ptr;
	if (hooks->write_i1)
		s_real_hooks.write_i1 = hooks->write_i1;
	if (hooks->get_env)
		s_real_hooks.get_env = hooks->get_env;
	if (hooks->set_jmpbuf)
		s_real_hooks.set_jmpbuf = hooks->set_jmpbuf;
	if (hooks->write_i2)
		s_real_hooks.write_i2 = hooks->write_i2;
	if (hooks->write_i4)
		s_real_hooks.write_i4 = hooks->write_i4;
	if (hooks->write_i8)
		s_real_hooks.write_i8 = hooks->write_i8;
	if (hooks->write_f4)
		s_real_hooks.write_f4 = hooks->write_f4;
	if (hooks->write_f8)
		s_real_hooks.write_f8 = hooks->write_f8;
	if (hooks->write_ptr)
		s_real_hooks.write_ptr = hooks->write_ptr;
	if (hooks->get_thread_state)
		s_real_hooks.get_thread_state = hooks->get_thread_state;
}
...
static void trace_write_line_ext(FILE *f,
                                 uint64_t ts,
                                 uint64_t tid,
                                 int type,
                                 uint64_t txid,
...
{
	fprintf(f,
	        "%llu %llu %d %llu 0x%llx %d 0x%llx %u%s%s\n",
...
static void trace_write_line(FILE *f,
                             uint64_t ts,
                             uint64_t tid,
                             int type,
                             uint64_t addr,
...
static void trace_begin()
{
	if (g_trace_file) {
		...
		t_tx_reads = 0;
		t_tx_writes = 0;
		trace_write_line_ext(g_trace_file,
		                     ts,
		                     tid,
		                     2,
		                     t_current_tx_id,
	...
	if (s_trace_real_hooks.begin)
		s_trace_real_hooks.begin();
}
...
static void trace_end()
{
	if (g_trace_file) {
		...
		trace_write_line_ext(g_trace_file,
		                     ...
		                     0,
		                     0,
		                     (t_tx_reads << 32) | t_tx_writes,
		                     0,
		                     nullptr);
	}
	if (s_trace_real_hooks.end)
		s_trace_real_hooks.end();
}
...
{
	void *p = s_trace_real_hooks.calloc ? s_trace_real_hooks.calloc(n, s)
	                                    : std::calloc(n, s);
	if (g_trace_file && p) {
		uint64_t ts = g_trace_ts.fetch_add(1);
		uint64_t tid = (uint64_t)(uintptr_t)pthread_self() & 0xffff;
...
{
	void *p = s_trace_real_hooks.realloc ? s_trace_real_hooks.realloc(old, s)
	                                     : std::realloc(old, s);
	if (g_trace_file) {
		uint64_t ts = g_trace_ts.fetch_add(1);
		uint64_t tid = (uint64_t)(uintptr_t)pthread_self() & 0xffff;
...
static void trace_free(void *p)
{
	...
	if (s_trace_real_hooks.free)
		s_trace_real_hooks.free(p);
	else if (p)
		std::free(p);
}
...
static void trace_write_i1(uint8_t *a, uint8_t v)
{
	...
	if (s_trace_real_hooks.write_i1)
		s_trace_real_hooks.write_i1(a, v);
}
static void trace_write_i2(uint16_t *a, uint16_t v)
{
	...
	if (s_trace_real_hooks.write_i2)
		s_trace_real_hooks.write_i2(a, v);
}
static void trace_write_i4(uint32_t *a, uint32_t v)
{
	...
	if (s_trace_real_hooks.write_i4)
		s_trace_real_hooks.write_i4(a, v);
}
static void trace_write_i8(uint64_t *a, int64_t v)
{
	...
	if (s_trace_real_hooks.write_i8)
		s_trace_real_hooks.write_i8(a, v);
}
static void trace_write_f4(float *a, float v)
{
	...
	if (s_trace_real_hooks.write_f4)
		s_trace_real_hooks.write_f4(a, v);
}
static void trace_write_f8(double *a, double v)
{
	...
	if (s_trace_real_hooks.write_f8)
		s_trace_real_hooks.write_f8(a, v);
}
static void trace_write_ptr(void **a, void *v)
{
	...
	if (s_trace_real_hooks.write_ptr)
		s_trace_real_hooks.write_ptr(a, v);
}
...
static void end_with_invariant()
{
	...
	if (g_invariant_cb) {
		...
		if (result != 0) {
			fprintf(stderr,
			        "TM-INVARIANT: FAIL (callback returned %d "
...
// Sampling mode
enum class TMSamplingMode : uint8_t { None = 0, RateLimited, Phase, Adaptive };
...
extern "C" void tm_set_sample_rate(uint64_t rate) { g_sample_rate = rate > 0 ? rate : 1; }

extern "C" uint64_t tm_get_sample_rate() { return g_sample_rate; }
...
{
	if (should_sample() && s_real_hooks.write_i1)
		s_real_hooks.write_i1(a, v);
	else
		*a = v;
}
static void sample_write_i2(uint16_t *a, uint16_t v)
{
	if (should_sample() && s_real_hooks.write_i2)
		s_real_hooks.write_i2(a, v);
	else
		*a = v;
}
static void sample_write_i4(uint32_t *a, uint32_t v)
{
	if (should_sample() && s_real_hooks.write_i4)
		s_real_hooks.write_i4(a, v);
	else
		*a = v;
}
static void sample_write_i8(uint64_t *a, int64_t v)
{
	if (should_sample() && s_real_hooks.write_i8)
		s_real_hooks.write_i8(a, v);
	else
		*a = (uint64_t)v;
}
static void sample_write_f4(float *a, float v)
{
	if (should_sample() && s_real_hooks.write_f4)
		s_real_hooks.write_f4(a, v);
	else
		*a = v;
}
static void sample_write_f8(double *a, double v)
{
	if (should_sample() && s_real_hooks.write_f8)
		s_real_hooks.write_f8(a, v);
	else
		*a = v;
}
static void sample_write_ptr(void **a, void *v)
{
	if (should_sample() && s_real_hooks.write_ptr)
		s_real_hooks.write_ptr(a, v);
	else
		*a = v;
}

// Abort notification for adaptive sampling
static void sample_notify_abort() { t_abort_count++; }
