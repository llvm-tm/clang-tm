...
static void pr_stm_lane_thread(LaneState *ls,
                               int num_addrs,
                               ...
                               int writes_per_thread)
{
	...
	for (int tx_iter = 0; tx_iter < 10; tx_iter++) {
		...
		for (int r = 0; r < reads_per_thread; r++) {
			...
			uint32_t lock_word = __atomic_load_n(&cpu_lock_table[addr_idx],
			                                     __ATOMIC_ACQUIRE);
		...
		sync->phase = 3;
		{
			...
			for (int i = 0; i < ls->num_reads; i++) {
				...
				uint32_t lock_word = __atomic_load_n(&cpu_lock_table[ls->read_addrs[i]],
				                                     __ATOMIC_ACQUIRE);
		...
		for (int w = 0; w < ls->num_writes; w++) {
			...
			uint32_t old = __atomic_load_n(&cpu_lock_table[ai], __ATOMIC_ACQUIRE);
		...
		for (int w = 0; w < ls->num_writes && !sync->warp_abort.load(); w++) {
			int ai = (int)ls->write_addrs[w];
			uint32_t expected = __atomic_load_n(&cpu_lock_table[ai], __ATOMIC_ACQUIRE);
			if (!pr_stm_is_locked(expected)) {
				uint32_t desired = pr_stm_make_entry(ls->priority,
		...
		if (sync->warp_abort.load(std::memory_order_acquire)) {
			...
			for (int w = 0; w < ls->num_writes; w++) {
				int ai = (int)ls->write_addrs[w];
				uint32_t lw = __atomic_load_n(&cpu_lock_table[ai], __ATOMIC_RELAXED);
                        ...
                        int writes_per_thread)
{
	...
	for (int w = 0; w < num_warps; w++) {
		for (int l = 0; l < warp_size; l++) {
			int idx = w * warp_size + l;
			threads.emplace_back(pr_stm_lane_thread,
			                     &states[idx],
			                     num_addrs,
			                     reads_per_thread,
			                     writes_per_thread);
