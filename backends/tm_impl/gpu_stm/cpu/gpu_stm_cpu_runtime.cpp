...
struct [[gnu::aligned(64)]] GpuStmThreadTx {
	...
	struct {
		uint32_t lock_idx;
		uint32_t ver;
	} reads[PR_STM_MAX_READS];
	struct {
		uint32_t lock_idx;
		void *data_addr;
		uint64_t val;
		uint8_t bytes;
	} writes[PR_STM_MAX_WRITES];
};
...
static void real_tm_end()
{
	...
	for (int i = 0; i < g_tx.num_reads; i++) {
		uint32_t lw = __atomic_load_n(&gpu_lock_table[g_tx.reads[i].lock_idx],
		                              __ATOMIC_ACQUIRE);
	...
	for (int i = 0; i < g_tx.num_writes; i++) {
		...
		while (true) {
			...
			uint32_t desired = pr_stm_make_entry(priority,
			                                     ...
			                                     1);
			if (__atomic_compare_exchange_n(&gpu_lock_table[lock_idx],
			                                &expected,
			                                desired,
			                                0,
			                                __ATOMIC_ACQUIRE,
	...
	for (int i = 0; i < g_tx.num_writes; i++) {
		...
		case 1:
			__atomic_store_n((uint8_t *)data_addr, (uint8_t)val, __ATOMIC_RELEASE);
			break;
		case 2:
			__atomic_store_n((uint16_t *)data_addr, (uint16_t)val, __ATOMIC_RELEASE);
			break;
		case 4:
			__atomic_store_n((uint32_t *)data_addr, (uint32_t)val, __ATOMIC_RELEASE);
			break;
		case 8:
			__atomic_store_n((uint64_t *)data_addr, val, __ATOMIC_RELEASE);
			break;
		}
	}
...
{
	return (uint8_t)read_common(get_lock_entry(a), (uint32_t *)a, 1);
}
static uint16_t real_tm_read_i2(uint16_t *a)
{
	return (uint16_t)read_common(get_lock_entry(a), (uint32_t *)a, 2);
}
static uint32_t real_tm_read_i4(uint32_t *a)
{
	return (uint32_t)read_common(get_lock_entry(a), a, 4);
}
static uint64_t real_tm_read_i8(uint64_t *a)
{
	return read_common(get_lock_entry(a), a, 8);
}
static float real_tm_read_f4(float *a)
{
	...
	int wi = find_write(a);
	if (wi >= 0) {
		uint32_t r = (uint32_t)g_tx.writes[wi].val;
		__builtin_memcpy(&v, &r, 4);
		return v;
	}
	uint32_t r = (uint32_t)read_common(get_lock_entry(a), (uint32_t *)a, 4);
	__builtin_memcpy(&v, &r, 4);
	return v;
}
static double real_tm_read_f8(double *a)
{
	...
	if (wi >= 0) {
		double v;
		uint64_t r = g_tx.writes[wi].val;
		__builtin_memcpy(&v, &r, 8);
		return v;
	}
	double v;
	uint64_t r = read_common(get_lock_entry(a), a, 8);
	__builtin_memcpy(&v, &r, 8);
	return v;
}
...
static void real_tm_write_i1(uint8_t *a, uint8_t v)
{
	write_common(get_lock_entry(a), (uint32_t *)a, v, 1);
}
static void real_tm_write_i2(uint16_t *a, uint16_t v)
{
	write_common(get_lock_entry(a), (uint32_t *)a, v, 2);
}
static void real_tm_write_i4(uint32_t *a, uint32_t v)
{
	write_common(get_lock_entry(a), a, v, 4);
}
static void real_tm_write_i8(uint64_t *a, int64_t v)
{
	write_common(get_lock_entry(a), (uint32_t *)a, (uint64_t)v, 8);
}
static void real_tm_write_f4(float *a, float v)
{
	uint32_t r;
	__builtin_memcpy(&r, &v, 4);
	real_tm_write_i4((uint32_t *)a, r);
}
static void real_tm_write_f8(double *a, double v)
{
	uint64_t r;
	__builtin_memcpy(&r, &v, 8);
	real_tm_write_i8((uint64_t *)a, (int64_t)r);
}
