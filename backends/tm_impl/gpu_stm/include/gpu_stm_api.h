...
inline uint32_t pr_stm_make_entry(uint8_t priority, uint32_t version, int locked)
{
	return ((uint32_t)priority << PR_STM_PRIORITY_SHIFT) |
	       (version << PR_STM_VERSION_SHIFT) | (locked ? PR_STM_LOCKED_BIT : 0u);
}

TM_GPU_HOST_DEVICE
inline uint8_t pr_stm_get_priority(uint32_t entry)
{
	return (uint8_t)(entry >> PR_STM_PRIORITY_SHIFT);
}

TM_GPU_HOST_DEVICE
inline uint32_t pr_stm_get_version(uint32_t entry)
{
	return (entry >> PR_STM_VERSION_SHIFT) &
	       (PR_STM_VERSION_MASK >> PR_STM_VERSION_SHIFT);
}
...
typedef void (*pr_stm_tx_body_t)(int lane_id,
                                 int warp_id,
                                 void *data,
                                 void *shared_scratch);

int gpu_pr_stm_launch(int num_warps, pr_stm_tx_body_t tx_body, void *tx_data);
