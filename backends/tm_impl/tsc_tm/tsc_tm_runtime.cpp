...
static void *real_tm_get_thread_state() { return (void *)&g_tm_thread_state; }
...
extern const TMRealHooks g_tsctm_hooks;
...
TM_PLUGIN_LIFECYCLE_FN(do_tm_init, extern "C" void tm_init())
{
	...
	tsctm::init();
	tm_register_real_hooks(&g_tsctm_hooks);
}
...
extern "C" int tm_setjmp() { return 0; }
...
{
	tsctm::set_jmp_env_external(&tm_jmpbuf);
...
static uint8_t real_tm_read_i1(uint8_t *addr) { return tsctm::tm_read_i1(addr); }

static uint16_t real_tm_read_i2(uint16_t *addr) { return tsctm::tm_read_i2(addr); }

static uint32_t real_tm_read_i4(uint32_t *addr)
{
	uint32_t r = tsctm::tm_read_i4(addr);
	return r;
}

static uint64_t real_tm_read_i8(uint64_t *addr) { return tsctm::tm_read_i8(addr); }

extern "C" void tm_read_i16(void *addr, void *out)
{
	auto *out_words = static_cast<uint64_t *>(out);
	out_words[0] = tsctm::tm_read_i8(static_cast<uint64_t *>(addr) + 0);
	out_words[1] = tsctm::tm_read_i8(static_cast<uint64_t *>(addr) + 1);
}
extern "C" void tm_read_i32(void *addr, void *out)
{
	auto *out_words = static_cast<uint64_t *>(out);
	for (int i = 0; i < 4; i++)
		out_words[i] = tsctm::tm_read_i8(static_cast<uint64_t *>(addr) + i);
}
...
static float real_tm_read_f4(float *addr) { return tsctm::tm_read_f4(addr); }

static double real_tm_read_f8(double *addr) { return tsctm::tm_read_f8(addr); }

static void *real_tm_read_ptr(void **addr)
{
	return tsctm::tm_read_ptr((volatile void **)addr);
}
...
extern "C" void *tm_read_z(uint8_t *addr, uint64_t len)
{
	...
	for (uint64_t i = 0; i < len; i++) {
		tm_buffer[i] = tsctm::tm_read_i1(&addr[i]);
	}
	return tm_buffer;
}
...
extern "C" void tm_write_i16(void *addr, void *val)
{
	...
	for (int i = 0; i < 2; i++)
		tsctm::tm_write_i8(static_cast<uint64_t *>(addr) + i, val_words[i]);
}
...
static void real_tm_write_f4(float *addr, float val) { tsctm::tm_write_f4(addr, val); }

static void real_tm_write_f8(double *addr, double val) { tsctm::tm_write_f8(addr, val); }

static void real_tm_write_ptr(void **addr, void *val)
{
	tsctm::tm_write_ptr((volatile void **)addr, val);
}

extern "C" void tm_write_z(uint8_t *dst, uint8_t *src, uint64_t len)
{
	for (uint64_t i = 0; i < len; i++) {
		tsctm::tm_write_i1(&dst[i], src[i]);
	}
}
...
{
	return tm_track_alloc_result(stm::tm_region_malloc(size), size);
}
...
static void real_tm_free(void *ptr)
{
	...
	TM_EVENT(FREE, ptr, 0);
	if (g_in_tx) {
		tsctm::tm_write_i1(reinterpret_cast<volatile uint8_t *>(ptr), 0);
...
const TMRealHooks g_tsctm_hooks = {
    .begin = real_tm_begin,
    .end = real_tm_end,
