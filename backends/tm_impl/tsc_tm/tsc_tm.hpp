/**
 * TSC-TM - TL2 variant using a TSC-based clock instead of a global atomic clock
 *
 * Standard TL2 commits by doing an atomic RMW on a single global version clock
 * (g_clock.fetch_add), which is a cross-core contention hotspot.  TSC-TM
 * replaces that clock with the CPU cycle counter (stm::tm_timestamp()):
 *
 *   - begin()      reads the cycle counter as start_version (rdtsc / cntvct).
 *   - commit()     reads the cycle counter as commit_ts.
 *   - Guard (orec) versions are commit timestamps, exactly as TL2 stores global
 *     clock versions.  The only CAS in the whole backend is the per-guard lock
 *     acquisition in try_acquire_guard() — there is NO global atomic counter.
 *
 * Soundness of TSC versions:
 *   1. TSC synchronisation invariant: on x86 (constant/invariant TSC) and ARM64
 *      (cntvct_el0 is a system-wide virtual counter) the cycle counter is
 *      ~synchronised across cores, so a timestamp taken later in wall-clock
 *      time is >= one taken earlier.
 *   2. Per-guard monotonic bump: a guard is released with
 *      version = max(commit_ts, version_seen_at_acquisition + 1).  Every
 *      commit therefore stores a strictly larger version in every guard it
 *      touches than the version that guard had before — even if two commits
 *      fall in the same TSC tick.  A reader that validated against the old
 *      version observes the new one and aborts.
 *   3. Optional Spanner-style commit-wait: if TSC_TM_WAIT_CYCLES > 0, a writer
 *      spins until tm_timestamp() >= commit_ts + wait before releasing guards,
 *      which makes commit timestamps globally strictly increasing (TrueTime
 *      style).  It costs ~wait cycles per writing transaction; 0 relies on the
 *      per-guard bump alone and is what the benchmarks use.
 *
 * Write-set size bound: 64-bit TSC values are stored as (version << 1) in the
 * guard word, so the top bit of the TSC is dropped on extraction — irrelevant
 * for any practical uptime.
 */
...
// Commit-wait in TSC cycles (0 = rely on per-guard monotonic bump only).
// ~2000 cycles is roughly a microsecond on a 2-3 GHz core.
static constexpr uint64_t kTscWaitCycles = 0;

// Global abort counter — defined in tsc_tm_runtime.cpp
} // namespace tsctm
...
template <typename T> inline bool isTMAddress(volatile T *addr) noexcept
{
...
struct WriteSetEntry {
	...
	void set_old(word_t v)
	{
		switch (dtype) {
		case DataType::UINT8:
			values.u8[0] = (uint8_t)v;
			break;
		case DataType::UINT16:
			values.u16[0] = (uint16_t)v;
			break;
		case DataType::UINT32:
			values.u32[0] = (uint32_t)v;
			break;
		case DataType::FLOAT:
			values.u32[0] = (uint32_t)v;
			break;
		case DataType::UINT64:
			values.u64[0] = (uint64_t)v;
			break;
		case DataType::DOUBLE:
			values.u64[0] = (uint64_t)v;
			break;
	...
	void set_new(word_t v)
	{
		switch (dtype) {
		case DataType::UINT8:
			values.u8[1] = (uint8_t)v;
			break;
		case DataType::UINT16:
			values.u16[1] = (uint16_t)v;
			break;
		case DataType::UINT32:
			values.u32[1] = (uint32_t)v;
			break;
		case DataType::FLOAT:
			values.u32[1] = (uint32_t)v;
			break;
		case DataType::UINT64:
			values.u64[1] = (uint64_t)v;
			break;
		case DataType::DOUBLE:
			values.u64[1] = (uint64_t)v;
			break;
...
{
private:
	// TSC-TM has NO global version clock.  Versions come from the CPU cycle
	// counter via stm::tm_timestamp(); the only shared atomics are the
	// per-guard (orec) locks below.
...
public:
	...
	static word_t get_clock() { return stm::tm_timestamp(); }
	...
	static bool try_acquire_guard(Transaction *tx, word_t *addr, word_t *acq_version)
	{
		(void)tx;
		word_t idx = get_guard_idx(addr);
		word_t guard = g_guards[idx].load(std::memory_order_acquire);
		while (!(guard & LOCK_MASK)) {
			...
			if (g_guards[idx].compare_exchange_weak(guard,
			                                        desired,
			                                        std::memory_order_acquire,
			                                        std::memory_order_acquire)) {
				if (acq_version)
					*acq_version = (guard & VERSION_MASK) >> 1;
				return true;
			}
		}
		return false;
	...
	// Release with a monotonically increasing version: store
	// max(commit_ts, acq_version + 1).  This guarantees the guard's version
	// strictly increases on every commit that touches it, even if two commits
	// fall in the same TSC tick (the TSC-resolution problem that a global
	// fetch_add clock avoids by construction).
	static void release_guard_with_version(word_t *addr,
	                                       word_t commit_ts,
	                                       word_t acq_version)
	{
		word_t idx = get_guard_idx(addr);
		word_t v = (commit_ts > acq_version + 1) ? commit_ts : (acq_version + 1);
		g_guards[idx].store((v << 1) & VERSION_MASK, std::memory_order_release);
	}

	static word_t read_guard(word_t *guard_addr)
	{
		return reinterpret_cast<std::atomic<word_t> *>(guard_addr)
		    ->load(std::memory_order_acquire);
	}

	// ---- Type conversion helpers ----
	template <typename T> static word_t to_word(T val)
	{
	...
	template <typename T> static T from_word(word_t val)
	{
		if constexpr (std::is_integral_v<T>) {
		...
		if (bloom_might_contain(tx, (word_t *)addr)) {
			...
			// 8-byte UINT64 writes but individual fields are read at sub-offsets.
			{
				...
				for (auto &e : tx->write_set) {
					...
					unsigned entry_sz = swapSize(e.dtype);
					if (entry_sz == 0 || entry_sz <= req_sz2)
						continue;
	...
	static bool commit(Transaction *tx)
	{
		...
		// Step 4: Read the TSC as the commit timestamp (TSC-TM has no global
		// atomic version clock).  Optional Spanner-style commit-wait makes
		// commit timestamps globally strictly increasing; when disabled (the
		// default) the per-guard monotonic bump in release_guard_with_version
		// keeps every guard version strictly increasing on its own.
		word_t commit_ts = stm::tm_timestamp();
		if (kTscWaitCycles > 0 && commit_ts != 0) {
			while ((stm::tm_timestamp() - commit_ts) < kTscWaitCycles) {
				stm::tm_cpu_relax();
			}
		}
		tx->commit_version = commit_ts;
		...
		for (auto &re : tx->read_set) {
			...
			word_t guard_idx = (word_t)(((uintptr_t)re.guard_addr - (uintptr_t)g_guards) /
			                            sizeof(g_guards[0]));
		...
		// Release each unique guard exactly once, storing a version strictly
		// greater than the one the guard had before this commit.
		{
			bool released_guard[GUARD_TABLE_SIZE] = {false};
			for (auto &e : tx->write_set) {
				word_t idx = get_guard_idx(e.addr);
				if (released_guard[idx])
					continue;
				released_guard[idx] = true;
				release_guard_with_version(e.addr, commit_ts, acq_version[idx]);
				TM_EVENT2(LOCK_RELEASE, (word_t)e.addr, commit_ts, 0);
			}
		}
...
{
	if (current_tx) {
		if (current_tx->active)
			STM::abort_tx(current_tx);
		delete current_tx;
		current_tx = nullptr;
	}
}
