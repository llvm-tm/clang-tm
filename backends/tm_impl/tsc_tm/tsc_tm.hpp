...
public:
    ...
    static bool try_acquire_guard(Transaction* tx, word_t* addr, word_t* acq_version) {
        ...
        while (!(guard & LOCK_MASK)) {
            ...
            if (g_guards[idx].compare_exchange_weak(guard, desired,
                ...
                if (acq_version) *acq_version = (guard & VERSION_MASK) >> 1;
                return true;
            }
        }
        return false;
    ...
    static void release_guard_with_version(word_t* addr, word_t commit_ts, word_t acq_version) {
        word_t idx = get_guard_idx(addr);
