...
int tm_region_init() noexcept
{
	...
	int expected = 0;
	if (!g_region_inited.compare_exchange_strong(expected, 1, std::memory_order_acq_rel))
		return 0;
	...
	g_slab_size = static_cast<size_t>(TM_SLAB_PAGES) * static_cast<size_t>(page_size);
	...
	if (addr == MAP_FAILED) {
		fprintf(stderr,
		        "FATAL: tm_region_init mmap(%zu) failed: %s\n",
		        region_size,
		        strerror(errno));
...
{
	g_tm_globals.push_back(
	    {static_cast<const char *>(addr), static_cast<const char *>(addr) + size});
}
