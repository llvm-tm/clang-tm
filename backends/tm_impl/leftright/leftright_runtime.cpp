...
static void real_tm_begin()
{
	...
	if (c <= 1) {
		// Benchmarks with hand-rolled sigsetjmp retry loops never call
		// tm_set_jmpbuf; default to the global tm_jmpbuf they setjmp into,
		// otherwise abort's siglongjmp(*jmpbuf_ptr) dereferences null.
		if (leftright::jmpbuf_ptr == nullptr)
			leftright::jmpbuf_ptr = &tm_jmpbuf;
		g_in_tx = true;
...
{
	return leftright::tm_read<uint8_t, leftright::ValueType::UINT8>(addr);
}
static uint16_t real_tm_read_i2(uint16_t *addr)
{
	return leftright::tm_read<uint16_t, leftright::ValueType::UINT16>(addr);
}
static uint32_t real_tm_read_i4(uint32_t *addr)
{
	return leftright::tm_read<uint32_t, leftright::ValueType::UINT32>(addr);
}
static uint64_t real_tm_read_i8(uint64_t *addr)
{
	return leftright::tm_read<uint64_t, leftright::ValueType::UINT64>(addr);
}
static void real_tm_write_i1(uint8_t *addr, uint8_t val)
{
	leftright::tm_write<uint8_t, leftright::ValueType::UINT8>(addr, val);
}
static void real_tm_write_i2(uint16_t *addr, uint16_t val)
{
	leftright::tm_write<uint16_t, leftright::ValueType::UINT16>(addr, val);
}
static void real_tm_write_i4(uint32_t *addr, uint32_t val)
{
	leftright::tm_write<uint32_t, leftright::ValueType::UINT32>(addr, val);
}
static void real_tm_write_i8(uint64_t *addr, int64_t val)
{
	leftright::tm_write<uint64_t, leftright::ValueType::UINT64>(addr,
...
{
	return leftright::tm_read<float, leftright::ValueType::FLOAT>(addr);
}
static double real_tm_read_f8(double *addr)
{
	return leftright::tm_read<double, leftright::ValueType::DOUBLE>(addr);
}
static void real_tm_write_f4(float *addr, float val)
{
	leftright::tm_write<float, leftright::ValueType::FLOAT>(addr, val);
}
static void real_tm_write_f8(double *addr, double val)
{
	leftright::tm_write<double, leftright::ValueType::DOUBLE>(addr, val);
}

static void *real_tm_read_ptr(void **addr)
{
	return leftright::tm_read<void *, leftright::ValueType::POINTER>(addr);
}
static void real_tm_write_ptr(void **addr, void *val)
{
	leftright::tm_write<void *, leftright::ValueType::POINTER>(addr, val);
}
