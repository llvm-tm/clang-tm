...
// ── Clock helpers ────────────────────────────────────────────────
inline uint64_t get_clock() { return g_clock.load(std::memory_order_acquire); }
...
{
	uint32_t expected = 0;
	return g_commit_lock.compare_exchange_strong(expected,
	                                             1,
	                                             std::memory_order_acquire,
	                                             std::memory_order_relaxed);
}

inline void release_commit_lock() { g_commit_lock.store(0, std::memory_order_release); }
		...
		if (isQueueActive()) {
			...
			if (valid) {
				for (auto &it : tx->read_set) {
					...
					if (std::memcmp(&cur, &r.captured_value, type_size(r.type)) != 0) {
						valid = false;
						break;
					}
				}
...
// ── Write word ──────────────────────────────────────────────────
inline void write_word(Transaction *tx,
                       void *addr,
                       any_type_t val,
                       ValueType sz,
                       bool skip_write_set = false)
{
