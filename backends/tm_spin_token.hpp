...
inline bool tm_token_try_acquire(int64_t tx_id) {
    int64_t expected = -1;
    return g_tx_token.compare_exchange_strong(expected, tx_id,
...
inline void tm_token_release() {
    g_tx_token.store(-1, std::memory_order_release);
}
...
inline void tm_token_release_if_held(int64_t tx_id) {
    ...
    g_tx_token.compare_exchange_strong(expected, -1,
