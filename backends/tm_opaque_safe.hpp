...
#define TM_OPAQUE_SAFE_TABLE(X)                                                    \
    X("tm_", 1)                                                                    \
    X("sigsetjmp", 0) X("siglongjmp", 0) X("longjmp", 0)                          \
    X("malloc", 0) X("free", 0) X("calloc", 0) X("realloc", 0)                    \
    X("aligned_alloc", 0)                                                          \
    X("_Znw", 1) X("_Zna", 1) X("_Zdl", 1) X("_Zda", 1)                          \
    X("llvm.", 1)                                                                  \
    X("__cxa_", 1) X("_ZSt", 1) X("_ZNSt", 1) X("_ZNKSt", 1)                     \
    X("_Unwind", 1)                                                                \
    X("pthread_", 1)                                                               \
    X("printf", 0) X("fprintf", 0) X("fflush", 0) X("puts", 0)                    \
    X("putchar", 0) X("putc", 0) X("snprintf", 0) X("sprintf", 0)                 \
    X("exit", 0) X("abort", 0)                                                     \
    X("strlen", 0) X("strcmp", 0) X("strncmp", 0)                                 \
    X("strtol", 0) X("strtoul", 0) X("strtoll", 0) X("strtoull", 0)               \
    X("strtof", 0) X("strtod", 0) X("strtold", 0)                                 \
    X("atoi", 0) X("atol", 0) X("atoll", 0) X("atof", 0)                          \
    X("abs", 0) X("labs", 0) X("llabs", 0)                                        \
    X("bzero", 0)                                                                  \
    X("memcmp", 0) X("memcpy", 0) X("memset", 0) X("memmove", 0)                  \
    X("wmemchr", 0)                                                                \
    X("posix_memalign", 0)                                                         \
    X("drand48", 0) X("srand48", 0)
#endif
...
static constexpr size_t TM_KNOWN_SAFE_TABLE_SIZE =
    sizeof(TM_KNOWN_SAFE_TABLE) / sizeof(TM_KNOWN_SAFE_TABLE[0]);
