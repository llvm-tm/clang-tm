...
public:
    struct Lock {
        ...
        word_t get_version() const {
            return (state.load(std::memory_order_acquire) >> (LOCK_EXTRA_BITS + INCARNATION_BITS));
        }
        
        word_t get_incarnation() const {
            return (state.load(std::memory_order_acquire) >> LOCK_EXTRA_BITS) & INCARNATION_MASK;
        }
        ...
        bool is_locked_by(word_t tx_id) const {
            ...
            return (s & LOCK_MASK) && ((s >> (LOCK_EXTRA_BITS + INCARNATION_BITS)) == tx_id);
        }
        ...
        void inc_abort(word_t current_incarnation) {
            ...
            word_t new_incarnation = ((current >> LOCK_EXTRA_BITS) & INCARNATION_MASK) + 1;
...
static void write_word_wt(Transaction* tx, volatile word_t* addr, word_t value, ValueType type) {
    if (!tx || !tx->active) {
        *addr = value;
        return;
    }
...
inline uint8_t tm_read_i1(volatile uint8_t* addr) {
    ...
    word_t word = read_word_wt(tx, (volatile word_t*)((word_t)addr & ~7), ValueType::UINT8);
...
inline uint16_t tm_read_i2(volatile uint16_t* addr) {
    ...
    word_t word = read_word_wt(tx, (volatile word_t*)((word_t)addr & ~7), ValueType::UINT16);
    uint8_t off = (word_t)addr & 7;
