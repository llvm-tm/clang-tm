...
public:
    ...
    TMRBTreeMap() : m_root(nullptr), m_size(0) {}
    ...
    V& operator[](const K& k) {
        ...
        z->parent = p;
        if (!p) m_root = z;
        else if (z->data.first < p->data.first) p->left = z;
        else p->right = z;
        insert_fixup(z);
        m_size++;
    ...
    size_t erase(const K& k) {
        Node* z = m_root;
        while (z) {
            if (k < z->data.first) z = z->left;
            else if (z->data.first < k) z = z->right;
            else break;
        }
        if (!z) return 0;
...
public:
    ...
    TMRBTreeMultiMap() : m_root(nullptr), m_size(0) {}
    ...
    void insert(const std::pair<K, V>& p) {
        ...
        while (x) {
            parent = x;
            if (!(x->data.first < p.first)) x = x->left;
            else x = x->right;
        }
        auto* z = new Node(p.first, p.second);
        z->parent = parent;
        if (!parent) m_root = z;
        else if (!(parent->data.first < z->data.first)) parent->left = z;
        else parent->right = z;
        insert_fixup(z);
        m_size++;
    }
