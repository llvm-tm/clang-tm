...
volatile int size_addrs;
volatile void *accessed_addrs[0x2FFFF];
volatile short fq_read_addrs[0x2FFFF];
volatile short fq_write_addrs[0x2FFFF];

#define ACCESS_ADDR(addr, isWrite) ({ \
    int i = 0, j = size_addrs; \
    while (accessed_addrs[i] < addr && i < size_addrs) ++i; \
    if (accessed_addrs[i] != addr && size_addrs < 32768) { \
        while (j > i) { \
            accessed_addrs[j] = accessed_addrs[j-1]; \
            fq_read_addrs[j] = fq_read_addrs[j-1]; \
            fq_write_addrs[j] = fq_write_addrs[j-1]; \
            --j; \
        } \
        accessed_addrs[i] = addr; \
        size_addrs++;\
    } \
    if (isWrite) { \
        fq_write_addrs[i]++; \
...
#define MAX_ACCESS(isWrite) ({ \
    int i = 0, res = 0; \
    while (i < size_addrs) { \
        if (isWrite) { \
            if (res < fq_write_addrs[i]) { \
                res = fq_write_addrs[i]; \
            } \
        } else { \
            if (res < fq_read_addrs[i]) { \
                res = fq_read_addrs[i]; \
            } \
        } \
        ++i; \
...
#define MIN_ACCESS(isWrite) ({ \
    int i = 0, res = 0x7FFFFF; \
    while (i < size_addrs) { \
        if (isWrite) { \
            if (res > fq_write_addrs[i]) { \
                res = fq_write_addrs[i]; \
...
#define AVG_ACCESS() ({ \
    ...
    double res = 0; \
    while (i < size_addrs) { \
        res += fq_write_addrs[i] + fq_read_addrs[i]; \
        ++i; \
    } \
    res / size_addrs; \
})
...
#define STDDEV_ACCESS() ({ \
    int i = 0; \
    double avg = AVG_ACCESS(), res = 0; \
    while (i < size_addrs) { \
        res += SQ(avg - fq_write_addrs[i] - fq_read_addrs[i]); \
        ++i; \
    } \
    res / size_addrs; \
})
...
#ifdef TL2_STATS
    clock_t diff = clock() - Self->InitTS;
    long R = Self->TxReads, W = Self->TxWrites, RW = R + W;
    __sync_add_and_fetch(global_stats+1, 1);
    __sync_add_and_fetch(global_stats+2, diff);
    __sync_add_and_fetch(global_stats+3, RW > 0 ? diff / RW : 0);
    __sync_add_and_fetch(global_stats+4, W);
    __sync_add_and_fetch(global_stats+5, R);
    __sync_add_and_fetch(global_stats+6, W > 0 ? diff / W : 0);
    __sync_add_and_fetch(global_stats+7, R > 0 ? diff / R : 0);
    global_stats[8] = size_addrs;
    global_stats[9] = MAX_ACCESS(1);
    global_stats[10] = MIN_ACCESS(1);
    global_stats[11] = MAX_ACCESS(0);
    global_stats[12] = MIN_ACCESS(0);
    global_stats[13] = AVG_ACCESS();
    global_stats[14] = STDDEV_ACCESS();
#endif
