...
#
# Builds an explicit-API benchmark, runs it with trace emission,
...
#   -m, --benchmark Benchmark name from benchmarks/cpp/
...
if [ -z "$SIM_ONLY" ]; then
  ...
  (cd "$SCRIPT_DIR/benchmarks/cpp" && make -j4 BACKEND="$BACKEND" bin/"$BENCHMARK" > /dev/null 2>&1)
  ...
  BACKEND="$BACKEND" TM_TRACE_PATH="$RAW_TRACE" "$BIN" $ARGS > /dev/null 2>&1
