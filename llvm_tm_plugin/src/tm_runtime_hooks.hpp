...
static Value *emitTMRead(IRBuilder<> &B, Value *Ptr, Type *Ty,
    ...
    if (Ty->isIntegerTy()) {
        ...
        FunctionCallee Hook;
        if (BitWidth <= 128)      Hook = H.read_i16;
        else if (BitWidth <= 256) Hook = H.read_i32;
        else                      Hook = H.read_i64;
    ...
    if (auto *FVT = dyn_cast<FixedVectorType>(Ty)) {
        ...
        if (ElemSizeBytes == 0) ElemSizeBytes = 1;
        for (unsigned i = 0; i < NumElems; i++) {
            Value *ElemPtr = B.CreateGEP(Type::getInt8Ty(Ctx), PC,
    ...
    for (unsigned i = 0; i < NumBytes; i++) {
        Value *BytePtr = B.CreateGEP(Type::getInt8Ty(Ctx), PC,
...
static bool emitTMWrite(IRBuilder<> &B, Value *Ptr, Value *Val,
    ...
    if (Ty->isIntegerTy(1))  { B.CreateCall(H.write_i1, {PC, B.CreateZExt(Val, Type::getInt8Ty(Ctx))}); return true; }
    ...
    if (Ty->isIntegerTy()) {
        unsigned BitWidth = Ty->getIntegerBitWidth();
        FunctionCallee Hook;
        if (BitWidth <= 128)      Hook = H.write_i16;
        else if (BitWidth <= 256) Hook = H.write_i32;
        else                      Hook = H.write_i64;
        AllocaInst *Alloca = B.CreateAlloca(Ty);
    ...
    for (unsigned i = 0; i < NumBytes; i++) {
        Value *ByteVal = B.CreateTrunc(
            B.CreateLShr(IntVal, ConstantInt::get(IntTy, i * 8)),
            Type::getInt8Ty(Ctx));
        Value *BytePtr = B.CreateGEP(Type::getInt8Ty(Ctx), PC,
                                      ConstantInt::get(i64Ty, i));
