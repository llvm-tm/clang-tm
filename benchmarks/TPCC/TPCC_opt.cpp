/**
 * TPC-C Benchmark - Full Specification Implementation v5.11
 *
 ...
 *
 * This is the OPTIMIZED variant — local variables inside TX functions
 * are annotated with TM_LOCAL to skip unnecessary TM instrumentation
 * (reducing read/write-set overhead).
 */
...
constexpr int MAX_ORDERS_PER_DISTRICT = 10000;  // allocated slots (3000 init + room for new)
...
static inline int idx_w(int w_id) {
    return w_id - 1;
}
...
static inline int idx_ord(int w_id, int d_id, int o_id) {
    return ((w_id - 1) * g_num_districts + (d_id - 1)) * MAX_ORDERS_PER_DISTRICT + (o_id - 1);
}
...
static inline int idx_ol(int w_id, int d_id, int o_id, int ol_num) {
    return (((w_id - 1) * g_num_districts + (d_id - 1)) * MAX_ORDERS_PER_DISTRICT + (o_id - 1))
            * MAX_OL_PER_ORDER + (ol_num - 1);
}

static inline int idx_i(int i_id) {
    return i_id - 1;
}
...
static void gen_string(char* dest, int len, const char* src) {
    int slen = 0;
    while (slen < len - 1 && src[slen]) slen++;
...
TM void init_data() {
    TM_LOCAL int nw = g_num_warehouses;
    TM_LOCAL int nd = g_num_districts;
    TM_LOCAL int nc = g_num_customers;
    TM_LOCAL int ni = g_num_items;

    // ---- Warehouse (§4.3.3.1) ----
    for (TM_LOCAL int w = 1; w <= nw; w++) {
        TM_LOCAL int wi = idx_w(w);
    ...
    // ---- District (§4.3.3.1) ----
    for (TM_LOCAL int w = 1; w <= nw; w++) {
        for (TM_LOCAL int d = 1; d <= nd; d++) {
            TM_LOCAL int di = idx_d(w, d);
    ...
    for (TM_LOCAL int w = 1; w <= nw; w++) {
        for (TM_LOCAL int d = 1; d <= nd; d++) {
            for (TM_LOCAL int c = 1; c <= nc; c++) {
                TM_LOCAL int ci = idx_c(w, d, c);
    ...
    // ---- Item (§4.3.3.1) ----
    for (TM_LOCAL int i = 1; i <= ni; i++) {
        TM_LOCAL int ii = idx_i(i);
        g_item[ii].i_id = i;
    ...
    for (TM_LOCAL int w = 1; w <= nw; w++) {
        for (TM_LOCAL int i = 1; i <= ni; i++) {
            TM_LOCAL int si = idx_s(w, i);
            g_stock[si].s_i_id = i;
            g_stock[si].s_w_id = w;
            g_stock[si].s_quantity = 100;
            for (TM_LOCAL int d = 1; d <= 10; d++)
    ...
    for (TM_LOCAL int w = 1; w <= nw; w++) {
        for (TM_LOCAL int d = 1; d <= nd; d++) {
            for (TM_LOCAL int c = 1; c <= nc; c++) {
                TM_LOCAL int hi = g_history_count++;
    ...
    for (TM_LOCAL int w = 1; w <= nw; w++) {
        for (TM_LOCAL int d = 1; d <= nd; d++) {
            ...
            for (TM_LOCAL int o = 1; o <= PREPOPULATED_ORDERS; o++) {
                TM_LOCAL int c_id = (int)(ord_rng() % nc) + 1;
                TM_LOCAL int ol_cnt = (int)(ord_rng() % 11) + 5;

                TM_LOCAL int o_idx = idx_ord(w, d, o);
                ...
                for (TM_LOCAL int l = 1; l <= ol_cnt; l++) {
                    TM_LOCAL int ol_idx = idx_ol(w, d, o, l);
                    TM_LOCAL int i_id;
                    do {
                ...
                if (o > 2100) {
                    TM_LOCAL int no_idx = idx_no(w, d, o);
...
TX int txn_new_order(int w_id, int d_id, int c_id,
                     int num_items, int* item_ids, int* supplier_ws, int* quantities) {
    TM_LOCAL int d_idx = idx_d(w_id, d_id);
    TM_LOCAL int o_id = g_district[d_idx].d_next_o_id++;
    g_order_count++;

    TM_LOCAL int o_idx = idx_ord(w_id, d_id, o_id);
    ...
    TM_LOCAL int no_idx = idx_no(w_id, d_id, o_id);
    g_neworder[no_idx].no_o_id = o_id;
    g_neworder[no_idx].no_d_id = d_id;
    g_neworder[no_idx].no_w_id = w_id;
    g_neworder_count++;

    TM_LOCAL float total_amount = 0;
    TM_LOCAL int all_local = 1;

    for (TM_LOCAL int i = 0; i < num_items; i++) {
        TM_LOCAL int ol_number = i + 1;
        TM_LOCAL int ol_idx = idx_ol(w_id, d_id, o_id, ol_number);

        TM_LOCAL int ol_i_id  = item_ids[i];
        TM_LOCAL int ol_sw_id = supplier_ws[i];
        TM_LOCAL int ol_qty   = quantities[i];
        ...
        TM_LOCAL float i_price = g_item[idx_i(ol_i_id)].i_price;
        TM_LOCAL float ol_amount = (float)ol_qty * i_price;
        g_orderline[ol_idx].ol_amount = ol_amount;
        total_amount += ol_amount;

        TM_LOCAL int s_idx = idx_s(ol_sw_id, ol_i_id);
        ...
        s.s_ytd += ol_qty;
        s.s_order_cnt++;
        if (ol_sw_id != w_id) s.s_remote_cnt++;

        gen_string(g_orderline[ol_idx].ol_dist_info, 25,
                   stock_dist_string(&s, d_id));
    }

    g_order[o_idx].o_all_local = (all_local ? 1.0f : 0.0f);
...
TX float txn_order_status(int w_id, int d_id, int c_id) {
    TM_LOCAL int c_idx = idx_c(w_id, d_id, c_id);
    TM_LOCAL float balance = g_customer[c_idx].c_balance;

    TM_LOCAL int d_idx = idx_d(w_id, d_id);
    TM_LOCAL int max_o = g_district[d_idx].d_next_o_id - 1;
    TM_LOCAL int latest_o_id = -1;

    for (TM_LOCAL int o_id = max_o; o_id >= 1; o_id--) {
        TM_LOCAL int o_idx = idx_ord(w_id, d_id, o_id);
    ...
    if (latest_o_id > 0) {
        TM_LOCAL int o_idx = idx_ord(w_id, d_id, latest_o_id);
        TM_LOCAL int ol_cnt = g_order[o_idx].o_ol_cnt;
        for (TM_LOCAL int l = 1; l <= ol_cnt; l++) {
            TM_LOCAL int ol_idx = idx_ol(w_id, d_id, latest_o_id, l);
            TM_LOCAL volatile float amount = g_orderline[ol_idx].ol_amount;
            (void)amount;
        }
    }
...
TX void txn_delivery(int w_id, int carrier_id) {
    for (TM_LOCAL int d_id = 1; d_id <= g_num_districts; d_id++) {
        TM_LOCAL int found_no = -1;
        for (TM_LOCAL int o_id = 1; o_id < MAX_ORDERS_PER_DISTRICT; o_id++) {
            TM_LOCAL int no_idx = idx_no(w_id, d_id, o_id);
        ...
        TM_LOCAL int o_idx = idx_ord(w_id, d_id, found_no);
        g_order[o_idx].o_carrier_id = carrier_id;

        TM_LOCAL int ol_cnt = g_order[o_idx].o_ol_cnt;
        TM_LOCAL float ol_total = 0;
        for (TM_LOCAL int l = 1; l <= ol_cnt; l++) {
            TM_LOCAL int ol_idx = idx_ol(w_id, d_id, found_no, l);
            ol_total += g_orderline[ol_idx].ol_amount;
            g_orderline[ol_idx].ol_delivery_d = (int)(::time(nullptr));
        }

        TM_LOCAL int c_id = g_order[o_idx].o_c_id;
        TM_LOCAL int c_idx = idx_c(w_id, d_id, c_id);
...
TX int txn_stock_level(int w_id, int d_id, int threshold) {
    TM_LOCAL int d_idx = idx_d(w_id, d_id);
    TM_LOCAL int next_o_id = g_district[d_idx].d_next_o_id;

    TM_LOCAL int start_o_id = next_o_id - 20;
    if (start_o_id < 1) start_o_id = 1;

    TM_LOCAL std::unordered_set<int> counted;

    for (TM_LOCAL int o_id = start_o_id; o_id < next_o_id; o_id++) {
        TM_LOCAL int o_idx = idx_ord(w_id, d_id, o_id);
        if (g_order[o_idx].o_w_id == 0) continue;

        TM_LOCAL int ol_cnt = g_order[o_idx].o_ol_cnt;
        for (TM_LOCAL int l = 1; l <= ol_cnt; l++) {
            TM_LOCAL int ol_idx = idx_ol(w_id, d_id, o_id, l);
            TM_LOCAL int i_id = g_orderline[ol_idx].ol_i_id;
            if (counted.count(i_id)) continue;
            counted.insert(i_id);

            TM_LOCAL int s_idx = idx_s(w_id, i_id);
...
THREAD void run(Worker* w) {
    TM_LOCAL std::uniform_int_distribution<int> wdist(1, g_num_warehouses);
    TM_LOCAL std::uniform_int_distribution<int> ddist(1, g_num_districts);
    TM_LOCAL std::uniform_int_distribution<int> cdist(1, g_num_customers);
    TM_LOCAL std::uniform_int_distribution<int> op_dist(0, 99);
    TM_LOCAL std::uniform_int_distribution<int> item_dist(1, g_num_items);
    TM_LOCAL std::uniform_int_distribution<int> qty_dist(1, 10);
    TM_LOCAL std::uniform_int_distribution<int> olcount_dist(5, 15);
    TM_LOCAL std::uniform_int_distribution<int> threshold_dist(10, 20);

    while (!done.load() && w->loops > 0) {
        w->loops--;
        TM_LOCAL int r = op_dist(*w->rng);
        TM_LOCAL int w_id = wdist(*w->rng);
        TM_LOCAL int d_id = ddist(*w->rng);

        if (r < 45) {
            TM_LOCAL int c_id = cdist(*w->rng);
            TM_LOCAL int num_items = olcount_dist(*w->rng);

            TM_LOCAL int item_ids[15];
            TM_LOCAL int supplier_ws[15];
            TM_LOCAL int quantities[15];

            for (TM_LOCAL int i = 0; i < num_items; i++) {
                TM_LOCAL int r2 = (int)((*w->rng)() % 100);
                item_ids[i] = item_dist(*w->rng);

                for (TM_LOCAL int j = 0; j < i; j++) {
                ...
                if (r2 < 1 && g_num_warehouses > 1) {
                    TM_LOCAL int other_w = wdist(*w->rng);
                    while (other_w == w_id)
                        other_w = wdist(*w->rng);
            ...
            TM_LOCAL int c_id = cdist(*w->rng);
            TM_LOCAL float amount = 100.00f + (float)((*w->rng)() % 9900);
            ...
            TM_LOCAL int carrier_id = ((int)((*w->rng)() % 10) + 1);
            txn_delivery(w_id, carrier_id);

        } else {
            TM_LOCAL int threshold = threshold_dist(*w->rng);
...
MAIN int main(int argc, char* argv[]) {
    TM_LOCAL int threads  = 4;
    TM_LOCAL int duration = 10000;
    ...
    g_district   = (District*)malloc(g_num_warehouses * g_num_districts * sizeof(District));
    g_customer   = (Customer*)malloc(g_num_warehouses * g_num_districts * g_num_customers * sizeof(Customer));
    g_history    = (History*)malloc(1000000 * sizeof(History));
    g_order      = (Order*)malloc(g_num_warehouses * g_num_districts * MAX_ORDERS_PER_DISTRICT * sizeof(Order));
    g_neworder   = (NewOrder*)malloc(g_num_warehouses * g_num_districts * MAX_ORDERS_PER_DISTRICT * sizeof(NewOrder));
    g_orderline  = (OrderLine*)malloc(g_num_warehouses * g_num_districts * MAX_ORDERS_PER_DISTRICT * MAX_OL_PER_ORDER * sizeof(OrderLine));
    g_item       = (Item*)malloc(g_num_items * sizeof(Item));
    g_stock      = (Stock*)malloc(g_num_warehouses * g_num_items * sizeof(Stock));

    if (!g_warehouse || !g_district || !g_customer || !g_history ||
        !g_order || !g_neworder || !g_orderline || !g_item || !g_stock) {
        std::cerr << "FATAL: malloc failed\n";
        return 1;
    }

    std::cout << "TPC-C Benchmark (Full Specification v5.11) — OPTIMIZED (TM_LOCAL)\n";
    std::cout << "=================================================================\n";
    ...
    std::cout << "  Orders:     " << PREPOPULATED_ORDERS << " per district (pre-populated)\n";
    ...
    std::cout << "  OrderLines: " << (g_num_warehouses * g_num_districts * PREPOPULATED_ORDERS * 10) << " (avg)\n";
    std::cout << "  New-Orders: " << g_neworder_count << "\n";
    std::cout << "  History:    " << g_history_count << "\n\n";

    TM_LOCAL int loops = duration / 10;
    std::vector<Worker> ws(threads);
    std::vector<std::thread> thr;
    std::vector<std::mt19937> rngs(threads);

    for (TM_LOCAL int i = 0; i < threads; i++) {
    ...
    TM_LOCAL auto start = std::chrono::high_resolution_clock::now();
    for (TM_LOCAL int i = 0; i < threads; i++)
    ...
    TM_LOCAL auto end = std::chrono::high_resolution_clock::now();
    TM_LOCAL auto ms  = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

    TM_LOCAL uint64_t ops = total_ops.load();
