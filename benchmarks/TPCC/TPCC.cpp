    ...
    for (int w = 1; w <= nw; w++) {
        for (int d = 1; d <= nd; d++) {
            for (int c = 1; c <= nc; c++) {
                ...
                g_customer[ci].c_discount = 0.3000f;    // ~mid-range [0.0000, 0.5000] (§4.3.3.1)
...
TX int txn_new_order(int w_id, int d_id, int c_id,
    ...
    for (int i = 0; i < num_items; i++) {
        ...
        if (ol_sw_id != w_id) all_local = 0;                // remote item
...
MAIN int main(int argc, char* argv[]) {
    ...
    std::cout << "TPC-C Benchmark (Full Specification v5.11)\n";
    std::cout << "==========================================\n";
