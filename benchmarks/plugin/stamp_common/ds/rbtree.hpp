...
template <typename K, typename V> struct TM RBNode {
	K key;
	V val;
	RBNode *left;
	RBNode *right;
...
template <typename K, typename V> struct TM RBTree {
	RBNode<K, V> *root;
	...
	while (node != tree->root && node->parent->color == RBNode<K, V>::RED) {
		...
		if (parent == grandparent->left) {
			...
			if (uncle && uncle->color == RBNode<K, V>::RED) {
				...
				if (rl)
					rl->parent = grandparent;
				r->parent = grandparent->parent;
				if (!grandparent->parent)
					tree->root = r;
				else if (grandparent->parent->right == grandparent)
					grandparent->parent->right = r;
				else
					grandparent->parent->left = r;
				r->left = grandparent;
