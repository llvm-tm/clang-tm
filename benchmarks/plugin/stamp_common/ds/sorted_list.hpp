...
template <typename T> struct TM SLNode {
	T val;
	SLNode *next;
};

template <typename T> struct TM SortedList {
	SLNode<T> *head;
...
template <typename T> static inline bool slist_remove(SortedList<T> *list, const T &val)
{
	if (!list->head)
		return false;
...
template <typename T> static inline bool slist_contains(SortedList<T> *list, const T &val)
{
	long (*cmp)(const T &, const T &) = list->cmp;
	SLNode<T> *cur = list->head;
	while (cur) {
