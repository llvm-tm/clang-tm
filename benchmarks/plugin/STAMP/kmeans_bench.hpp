...
static inline int find_nearest_cluster(const double *point,
...
TX static void kmeans_merge_global(KMeansData *data, double *local_sum, int *local_count)
{
	for (int c = 0; c < data->nclusters; c++) {
		...
		if (lc > 0) {
			...
			for (int d = 0; d < data->ndims; d++) {
				data->new_centers_sum[c * data->ndims + d] += local_sum[c * data->ndims +
				                                                        d];
			}
		}
	}
...
static void kmeans_accum_local(double *local_sum,
                               int *local_count,
                               KMeansData *data,
                               int start,
                               int end)
...
TX static double kmeans_update_one_cluster(KMeansData *data, int c)
{
	...
	double delta = 0.0;
	for (int d = 0; d < data->ndims; d++) {
		double new_val = data->new_centers_sum[c * data->ndims + d] /
