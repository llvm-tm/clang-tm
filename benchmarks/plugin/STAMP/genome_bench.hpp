...
static int seg_find_insert_pos(char **strs, int *lens, int count, const char *s, int slen)
{
	int lo = 0, hi = count;
	while (lo < hi) {
		int mid = (lo + hi) / 2;
...
__attribute__((annotate("tm_allow_opaque"))) TX static void genome_match(
    GenomeData *data,
    int start,
    int end,
    TMSafeMap<uint64_t, TMSafeVector<char **>> &hash_table)
		...
		for (int i = 0; i < data->unique_count; i++) {
			...
			if (fit != hash_table.end()) {
				for (char **pcandidate : fit->second) {
					if (pcandidate == &data->unique_segments[i])
						continue;
					char *b = *pcandidate;
					int blen = data->unique_segment_lens[pcandidate -
