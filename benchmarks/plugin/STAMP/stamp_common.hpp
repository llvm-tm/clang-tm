...
template <typename T> static T *tm_new_array(size_t n)
{
	return (T *)tm_calloc(n, sizeof(T));
}
...
static inline double tm_acos(double x)
{
	...
	double z2 = z * z;
	double asin_z = z * (1.0 + z2 * (1.0 / 6.0 +
	                                 z2 * (3.0 / 40.0 +
	                                       z2 * (5.0 / 112.0 + z2 * 35.0 / 1152.0))));
	return 2.0 * asin_z;
}
...
__attribute__((annotate("tm_allow_opaque"))) inline uint64_t wall_clock_now()
{
	return std::chrono::steady_clock::now().time_since_epoch().count();
}

template <typename Engine> class FastPRNG
{
public:
	...
	uint64_t range(uint64_t max) { return engine_() % max; }
	double uniform() { return (double)engine_() / (double)engine_.max(); }
