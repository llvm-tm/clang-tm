...
static void circumcircle_center(double &cx,
                                double &cy,
                                double &cr,
                                const YadaPoint &a,
                                const YadaPoint &b,
                                const YadaPoint &c)
{
	...
	cx = ((a.x * a.x + a.y * a.y) * (b.y - c.y) + (b.x * b.x + b.y * b.y) * (c.y - a.y) +
...
static double triangle_min_angle(const YadaPoint &a,
                                 const YadaPoint &b,
                                 const YadaPoint &c)
{
...
inline void yada_generate_mesh()
{
	...
	data->neighbors = (long *)tm_calloc(YADA_MAX_ELEMENTS * YADA_MAX_NEIGHBORS,
	                                    sizeof(long));
	...
	auto add_element = [&](const YadaPoint &a, const YadaPoint &b, const YadaPoint &c) {
		...
		data->pt_x[id * 3] = a.x;
		data->pt_y[id * 3] = a.y;
		data->pt_x[id * 3 + 1] = b.x;
		data->pt_y[id * 3 + 1] = b.y;
		data->pt_x[id * 3 + 2] = c.x;
		data->pt_y[id * 3 + 2] = c.y;
		circumcircle_center(data->circ_x[id],
		                    data->circ_y[id],
		                    data->circ_r[id],
		                    a,
		                    b,
	...
	if (g_yada_i && g_yada_i[0] != '\0') {
		...
		if (is_mesh) {
			...
			if (!fn.is_open()) {
				std::cerr << "Error: could not open " << prefix << ".node\n";
				std::exit(1);
			}
			...
			if (!fe.is_open()) {
				std::cerr << "Error: could not open " << prefix << ".ele\n";
				std::exit(1);
			}
			int nt = 0, npt = 3, ea = 0;
			fe >> nt >> npt >> ea;
	...
	for (int i = 0; i < ne; i++) {
		...
		if (!bad) {
			for (int e = 0; e < 3 && !bad; e++) {
				YadaPoint pts[3] = {{data->pt_x[i * 3], data->pt_y[i * 3]},
	...
	std::cout << "Yada mesh: " << ne << " elements, " << whc
	          << " bad (angle constraint=" << data->angle_constraint << " deg)\n";
...
TX __attribute__((annotate("tm_allow_opaque"))) static void yada_apply_refinement(
    int el_id,
    const int *cavity,
    int cavity_count,
    const YadaEdge *border,
    int border_count,
    const YadaPoint &centroid)
{
	...
	for (int i = 0; i < border_count; i++) {
		...
		circumcircle_center(g_yada->circ_x[tid],
		                    g_yada->circ_y[tid],
		                    g_yada->circ_r[tid],
		                    centroid,
		                    border[i].a,
...
THREAD void worker_yada(ThreadData *td)
{
	...
	for (int iter = 0; iter < td->loops && !stop_workers; iter++) {
		...
		if (el_id < 0) {
			empty_count++;
			if (empty_count >= 3)
				break; // Heap empty, no more work
			continue;
		}
		empty_count = 0;
		if (data->is_garbage[el_id] != 0)
			continue;
		...
		if (num_new > 0) {
			...
			for (int i = 0; i < (int)num_new; i++) {
				...
				bool enc = is_encroached(ta, tb, tc) || is_encroached(tb, tc, ta) ||
