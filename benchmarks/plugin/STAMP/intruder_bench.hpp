...
inline void intruder_generate_packets()
{
	...
	std::vector<std::string> dict_vec =
	    {"about", "attack", "back", "root",  "system", "access", "all",   "after",
	     "also",  "and",    "any",  "are",   "but",    "can",    "come",  "could",
	     "did",   "do",     "each", "find",  "first",  "for",    "from",  "get",
	     "go",    "has",    "have", "her",   "here",   "him",    "his",   "how",
	     "into",  "its",    "just", "know",  "like",   "look",   "make",  "man",
	     "may",   "more",   "most", "must",  "new",    "no",     "not",   "now",
	     "old",   "one",    "only", "other", "our",    "out",    "over",  "own",
	     "part",  "people", "said", "say",   "see",    "she",    "shell", "should",
	     "site",  "some",   "such", "take",  "than",   "that",   "their", "them",
	     "then",  "there"};
	...
	data->fragment_storage = (char *)tm_calloc(g_intruder_n * INTRUDER_MAX_PACKETS *
	                                               INTRUDER_MAX_DATA,
	                                           1);
...
static inline bool detect_attack(const char *data,
                                 int data_len,
                                 char **dictionary,
                                 int dict_size)
{
...
__attribute__((annotate("tm_allow_opaque"))) TX static void process_decoder(
    IntruderData *data,
    const Packet &pkt)
{
	...
	char *frag = &data->fragment_storage[(flow_idx * INTRUDER_MAX_PACKETS + fc) *
	                                     INTRUDER_MAX_DATA];
	...
	if (df.num_fragments_received == pkt.num_fragments) {
		...
		for (int i = 0; i < pkt.num_fragments; i++) {
			char *src = &data->fragment_storage[(flow_idx * INTRUDER_MAX_PACKETS + i) *
			                                    INTRUDER_MAX_DATA];
			int slen = 0;
			while (slen < INTRUDER_MAX_DATA && src[slen])
				slen++;
...
THREAD void worker_intruder(ThreadData *td)
{
	...
	for (;;) {
		...
		if (df.flow_id >= 0) {
			bool attack = detect_attack(df.data,
			                            df.data_len,
			                            data->dictionary,
