...
static inline Task task_heap_pop(Task *heap, int &count)
{
	...
	while (true) {
		...
		int l = 2 * i + 1;
		int r = 2 * i + 2;
		if (l < count && heap[l].score > heap[largest].score)
			largest = l;
		if (r < count && heap[r].score > heap[largest].score)
			largest = r;
		if (largest == i)
			break;
		Task tmp = heap[i];
...
TX static double compute_density_ll(BayesData *data,
                                    int var,
                                    const int *parents_vec,
                                    int np)
{
	int ncfg = 1 << np;
	int c0[128], c1[128];
	...
	for (int r = 0; r < data->num_record; r++) {
		...
		if (data->records[r * data->num_var + var] == 0)
			c0[cfg]++;
		else
			c1[cfg]++;
	}
...
inline void bayes_generate_network()
{
	...
	data->base_penalty = g_bayes_i > 0 ? -0.5 * std::log((double)nrec) * g_bayes_i : 0.0;
	g_bayes = data;
...
{
	if (parents_has(data, to, from))
		return false;
	if (has_path(data, to, from))
		return false;
	if (data->global_max_edges > 0 && data->parents_cnt[to] >= data->global_max_edges)
		return false;
...
TX static Task find_best_insert(BayesData *data, int to)
{
	...
	for (int from = 0; from < data->num_var; from++) {
		...
		if (has_path(data, to, from))
			continue;
		if (data->global_max_edges > 0 && pc >= data->global_max_edges)
			continue;
