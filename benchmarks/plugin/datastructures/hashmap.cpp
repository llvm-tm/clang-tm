...
TX bool txn_erase(int key) { return erase(key); }

TX bool txn_contains(int key) { return contains(key); }

TX int txn_get(int key, int default_val = -1) { return get(key, default_val); }

TX int rangeCount(int minKey, int maxKey) { return countRange(minKey, maxKey); }
