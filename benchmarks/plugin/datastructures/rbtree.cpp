...
int size(int n) { return n == sentinel ? 0 : node_size[n]; }

bool isRed(int n) { return n != sentinel && node_red[n]; }
...
int newNode(int key, int value)
{
	...
	if (free_head != -1) {
		...
		if (n >= MAX_NODES - 1) {
			fprintf(stderr,
			        "FATAL: rbtree node pool exhausted (MAX_NODES=%d)\n",
			        MAX_NODES);
			std::abort();
		}
	}
...
int treeSize() { return (root == sentinel) ? 0 : node_size[root]; }

TX void txn_insert(int key, int value) { insert(key, value); }

TX void txn_erase(int key) { erase(key); }

TX bool txn_contains(int key) { return contains(key); }

TX int txn_get(int key, int default_val) { return get(key, default_val); }
...
MAIN int main(int argc, char *argv[])
{
	...
	if (argc > 2)
		initial_size = std::atoi(argv[2]);
	if (argc > 3)
		duration_ms = std::atoi(argv[3]);
	if (argc > 4)
		read_pct = std::atoi(argv[4]);
	if (argc > 5)
		write_pct = std::atoi(argv[5]);
	if (argc > 6)
		range_max = std::atoi(argv[6]);

	// Validate arguments
	if (nb_threads < 1) {
		std::cerr << "ERROR: nb_threads must be >= 1\n";
		return 1;
	}
	if (range_max < 10) {
		std::cerr << "ERROR: range_max must be >= 10\n";
		return 1;
	}
	if (read_pct + write_pct > 95) {
		std::cerr << "ERROR: read_pct + write_pct must be <= 95\n";
		return 1;
	}
	if (initial_size >= MAX_NODES) {
		std::cerr << "ERROR: initial_size too large for MAX_NODES=" << MAX_NODES << "\n";
		return 1;
	}
