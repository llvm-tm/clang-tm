...
int size(int n) { return (n == -1) ? 0 : node_size[n]; }
...
int treeSize() { return (root == -1) ? 0 : node_size[root]; }

TX void txn_insert(int key, int value) { root = insert(root, key, value); }

TX void txn_erase(int key) { root = erase(root, key); }

TX bool txn_contains(int key) { return contains(root, key); }

TX int txn_get(int key, int default_val) { return get(root, key, default_val); }
