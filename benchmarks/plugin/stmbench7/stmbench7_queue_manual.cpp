	...
	if (idx >= 0 && idx < (int)g_nodes.size()) {
		...
		for (int eid : g_nodes[idx].edgeIds) {
			if (eid < (int)g_edges.size()) {
				int nb = (g_edges[eid].fromNodeId == idx) ? g_edges[eid].toNodeId
...
ASYNC_TX void op_sum_all_edges()
{
	volatile uint64_t sum = 0;
	for (auto &e : g_edges)
		sum += (uint64_t)(e.id + e.fromNodeId + e.toNodeId + e.type);
	(void)sum;
}
...
static void gen_args(int op_idx, std::mt19937 &rng, int &a0, int &a1)
{
	...
	case 2:
		a0 = (int)(rng() % sz);
		break; // traverse_neighbourhood
	case 3:
		a0 = (int)(rng() % sz);
		break; // update_neighbourhood
	case 4:
		break; // sum_all_nodes (no args)
	case 5:
		break; // sum_all_edges (no args)
	case 6:
		break; // add_node (no args)
	case 7:
		a0 = (int)(rng() % sz);
		a1 = (int)(rng() % sz);
		if (a1 == a0)
...
MAIN int main()
{
	...
	while (elapsed_ns < DURATION_NS) {
		...
		if (batch_count % 10 == 0) {
			...
			std::cout << "  [" << batch_count << " batches] " << total_ops << " ops, "
	...
	std::cout << "\n\nResults\n"
	          ...
	          << "Total ops:  " << total_ops << "\n"
	          << "Ops/sec:    "
	          << (elapsed_sec > 0 ? (uint64_t)(total_ops / elapsed_sec) : 0) << "\n";
