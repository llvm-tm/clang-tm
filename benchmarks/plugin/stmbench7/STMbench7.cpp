...
TX OpResult op_lt1()
{
	...
	for (auto &ca : g_complexAssemblies)
		sum += ca.id + ca.level + ca.buildDate;
	for (auto &ba : g_baseAssemblies)
...
{
	for (auto &ca : g_complexAssemblies)
		ca.buildDate = (ca.buildDate + 1) % 365 + 1000;
	for (auto &ba : g_baseAssemblies)
		ba.buildDate = (ba.buildDate + 1) % 365 + 1000;
	return {0, true};
}
...
TX OpResult op_lt5()
{
	...
	for (auto &c : g_connections)
		sum += c.id + c.fromAtomicPartId + c.toAtomicPartId + c.type;
	return {(uint64_t)sum, false};
}
...
TX OpResult op_st3_traverse(int ap_idx)
{
	...
	for (int conn_idx : ap.connectionIds) {
		if (conn_idx < (int)g_connections.size()) {
			...
			int nb = (c.fromAtomicPartId == ap_idx) ? c.toAtomicPartId
			                                        : c.fromAtomicPartId;
...
TX OpResult op_st4_update_ca(int ca_idx)
{
	if (ca_idx < 0 || ca_idx >= (int)g_complexAssemblies.size())
		return {0, true};
	g_complexAssemblies[ca_idx].buildDate = (g_complexAssemblies[ca_idx].buildDate + 1) %
	                                            365 +
	                                        1000;
	return {1, true};
}
...
{
	if (ba_idx < 0 || ba_idx >= (int)g_baseAssemblies.size())
		return {0, true};
	g_baseAssemblies[ba_idx].buildDate = (g_baseAssemblies[ba_idx].buildDate + 1) % 365 +
	                                     1000;
	return {1, true};
}
...
TX OpResult op_op1_lookup_ap(int id)
{
	...
	if (it == g_apById.end())
		return {0, false};
	if (it->second < 0 || it->second >= (int)g_atomicParts.size())
		return {0, false};
...
TX OpResult op_op2_lookup_cp(int id)
{
	...
	if (it == g_cpById.end())
		return {0, false};
	if (it->second < 0 || it->second >= (int)g_compositeParts.size())
		return {0, false};
...
TX OpResult op_op3_lookup_doc(int id)
{
	...
	if (it == g_docById.end())
		return {0, false};
	if (it->second < 0 || it->second >= (int)g_documents.size())
		return {0, false};
	return {(uint64_t)(g_documents[it->second].buildDate + g_documents[it->second].type),
	        false};
}
...
TX OpResult op_op4_lookup_ba(int id)
{
	...
	if (it == g_baById.end())
		return {0, false};
	if (it->second < 0 || it->second >= (int)g_baseAssemblies.size())
		return {0, false};
...
TX OpResult op_op5_lookup_ca(int id)
{
	...
	if (it == g_caById.end())
		return {0, false};
	if (it->second < 0 || it->second >= (int)g_complexAssemblies.size())
		return {0, false};
...
{
	if (conn_idx < 0 || conn_idx >= (int)g_connections.size())
		return {0, true};
...
{
	if (parent_ca_idx < 0 || parent_ca_idx >= (int)g_complexAssemblies.size())
		return {0, true};
...
static OpResult wrap_op3(std::mt19937 &rng)
{
	return op_op3_lookup_doc(rng() % MAX_DOCUMENTS);
}
...
static OpResult wrap_op11(std::mt19937 &rng)
{
	return op_op11_update_ap(pick_ap(rng), rng() % 100, rng() % 100);
}
static OpResult wrap_op12(std::mt19937 &rng)
{
	return op_op12_update_weight(pick_ap(rng), (rng() % 50) + 1);
}
static OpResult wrap_op13(std::mt19937 &rng)
{
	return op_op13_update_doc(pick_doc(rng), 1000 + (rng() % 365));
}
static OpResult wrap_op14(std::mt19937 &rng)
{
	return op_op14_update_cp(pick_cp(rng), 1000 + (rng() % 365));
}
static OpResult wrap_op15(std::mt19937 &rng)
{
	return op_op15_update_ba(pick_ba(rng), 1000 + (rng() % 365));
}

static OpResult wrap_sm1(std::mt19937 &rng)
{
	return op_sm1_create_cp(MAX_CP + (rng() % 1000));
}
...
static OpResult wrap_sm5(std::mt19937 &rng)
{
	return op_sm5_create_conn(pick_ap(rng), pick_ap(rng), rng() % 3);
}
...
static OpDesc pick_operation(std::mt19937 &rng, int writePercent)
{
	...
	if (r < 5)
		cat = OpClass::LONG_TRAV;
	else if (r < 45)
		cat = OpClass::SHORT_TRAV;
	else if (r < 90)
		cat = OpClass::SHORT_OP;
	else
		cat = OpClass::STRUCT_MOD;
	...
	if (wantRead) {
		switch (cat) {
		case OpClass::LONG_TRAV:
			pool = g_readOnlyLT;
			poolSize = NUM_RO_LT;
			break;
		case OpClass::SHORT_TRAV:
			pool = g_readOnlyST;
			poolSize = NUM_RO_ST;
			break;
		case OpClass::SHORT_OP:
			pool = g_readOnlyOP;
			poolSize = NUM_RO_OP;
			break;
		default:
...
MAIN int main(int argc, char *argv[])
{
	...
	case 1:
		writePercent = 10;
		break; // read-dominated  (§3: 90% RO, 10% UP)
	case 2:
		writePercent = 40;
		break; // read-write      (§3: 60% RO, 40% UP)
	case 3:
		writePercent = 90;
		break; // write-dominated (§3: 10% RO, 90% UP)
	default:
		writePercent = 10;
	}
	...
	std::cout << "STMbench7 (EuroSys 2007 Spec)\n"
	          << "==============================\n"
	          << "Workload:   " << workload << " (" << (100 - writePercent) << "% read, "
	          << writePercent << "% write)\n"
	          << "Threads:    " << nb_threads << "\n"
	...
	std::cout << "Data structure (§2: medium OO7 size):\n"
	          ...
	          << ")\n"
	          << "  Connections:       " << g_connections.size() << " (spec: ≥"
	          << MAX_CONN << ")\n"
	...
	std::cout << "\nResults\n"
	          ...
	          << "  Long traversals:      " << lt << " ("
	          << (ops > 0 ? lt * 100.0 / ops : 0.0) << "%, spec ~5%)\n"
	          << "  Short traversals:     " << st << " ("
	          << (ops > 0 ? st * 100.0 / ops : 0.0) << "%, spec ~40%)\n"
	          << "  Short operations:     " << sop << " ("
	          << (ops > 0 ? sop * 100.0 / ops : 0.0) << "%, spec ~45%)\n"
	          << "  Structure mods:       " << sm << " ("
	          << (ops > 0 ? sm * 100.0 / ops : 0.0) << "%, spec ~10%)\n"
	          << "\nRead/Update split:\n"
