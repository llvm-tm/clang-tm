...
TX void txn_read(int key, char *out_data, int max_len) { read(key, out_data, max_len); }
...
TX void txn_update(int key) { update(key); }

TX void txn_insert(int key) { insert(key); }
...
THREAD void worker(ThreadData *data)
{
	...
	while (!stop_workers.load(std::memory_order_relaxed)) {
		...
		if (g_distribution == Distribution::ZIPFIAN) {
			...
			key = (g_record_counter - (uniform_dist(*data->rng) % latest_bound) +
			       data->key_range) %
			      data->key_range;
		} else {
...
MAIN int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-t") == 0 && i + 1 < argc) {
			...
			case 'a':
			case 'A':
				workload = WorkloadType::A;
				break;
			case 'b':
			case 'B':
				workload = WorkloadType::B;
				break;
			case 'c':
			case 'C':
				workload = WorkloadType::C;
				break;
			case 'd':
			case 'D':
				workload = WorkloadType::D;
				break;
			case 'e':
			case 'E':
				workload = WorkloadType::E;
				break;
			case 'f':
			case 'F':
				workload = WorkloadType::F;
				break;
			}
			...
			char d = argv[++i][0];
			if (d == 'u' || d == 'U')
				g_distribution = Distribution::UNIFORM;
			else if (d == 'z' || d == 'Z')
				g_distribution = Distribution::ZIPFIAN;
			else if (d == 'l' || d == 'L')
				g_distribution = Distribution::LATEST;
		}
	}
	...
	uint64_t total_ops = 0, total_reads = 0, total_updates = 0, total_inserts = 0,
	         total_scans = 0, total_scan_results = 0;
	...
	std::cout << "\nResults\n"
	          ...
	          << "  Reads:       " << total_reads << " ("
	          << (total_ops > 0 ? total_reads * 100.0 / total_ops : 0) << "%)\n"
	          << "  Updates:     " << total_updates << " ("
	          << (total_ops > 0 ? total_updates * 100.0 / total_ops : 0) << "%)\n"
	          << "  Inserts:     " << total_inserts << " ("
	          << (total_ops > 0 ? total_inserts * 100.0 / total_ops : 0) << "%)\n"
	          << "  Scans:       " << total_scans << " ("
	          << (total_ops > 0 ? total_scans * 100.0 / total_ops : 0) << "%)\n"
