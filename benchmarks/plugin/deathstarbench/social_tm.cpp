...
TX void tx_compose_post(int64_t author_id)
{
	...
	for (size_t i = 0; i < g_followers[author_id].size(); ++i) {
		...
		g_nodes[follower_id].timeline_count = g_nodes[follower_id].timeline_count + 1;
	}
}
...
TX int64_t tx_read_user_timeline(int64_t user_id) { return g_nodes[user_id].post_count; }
...
{
	g_nodes[followee_id].follower_count = g_nodes[followee_id].follower_count + 1;
	g_nodes[follower_id].following_count = g_nodes[follower_id].following_count + 1;
}
...
void do_transaction_work(std::mt19937 &rng, int nb_users, std::atomic<uint64_t> *ops)
{
...
void do_follow_unfollow_work(std::mt19937 &rng, int nb_users, std::atomic<uint64_t> *ops)
{
...
TX int64_t tx_expected_posts() { return g_next_post_id; }
...
MAIN int main(int argc, char *argv[])
{
	...
	g_followers.resize(nb_users);
	{
		...
		for (int i = 0; i < nb_users; ++i) {
			int num_followees = std::uniform_int_distribution<int>(MIN_FOLLOWEES,
			                                                       MAX_FOLLOWEES)(
			    graph_rng);
			...
			g_followees[i].assign(candidates.begin(), candidates.begin() + num_followees);
			// Reverse: build follower lists
			for (int f : g_followees[i]) {
				g_followers[f].push_back(i);
			}
			g_nodes[i].follower_count = static_cast<int64_t>(g_followers[i].size());
			g_nodes[i].following_count = static_cast<int64_t>(g_followees[i].size());
	...
	if (!follow_unfollow_mode) {
		...
		for (auto &fl : g_followees)
			total_edges += fl.size();
		std::cout << "Graph:       " << total_edges << " follow-edges ("
	...
	if (follow_unfollow_mode) {
		...
		if (invariant_ok) {
			std::cout << "PASS: sum(post_count) == g_next_post_id  [" << final_tx_posts
			          << " == " << final_tx_expected << "]\n"
			          << "      Invariant verified in single atomic snapshot.\n";
			return 0;
		} else {
			std::cerr << "FAIL: sum(post_count) != g_next_post_id  [" << final_tx_posts
			          << " != " << final_tx_expected << "]\n"
			          << "      Atomicity violation detected.\n";
			return 1;
		}
