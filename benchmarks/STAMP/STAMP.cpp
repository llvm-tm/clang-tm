/**
 * STAMP Benchmark Suite Implementation
 *
...
static void print_usage()
{
	std::cout
	    ...
	    << "  ssca2:    -s <scale> -i <iterations> -u <uni_prob> -l <max_path> -p "
	       "<max_edges>\n"
	    << "  vacation: -n <queries> -q <pct_query> -r <relations> -u <pct_user> -t "
	       "<tasks>\n"
...
void run_benchmark(BenchmarkType bench, int threads)
{
	...
	for (int i = 0; i < threads; i++) {
		switch (bench) {
		case BenchmarkType::BAYES:
			workers.emplace_back(worker_bayes, &td[i]);
			break;
		case BenchmarkType::GENOME:
			workers.emplace_back(worker_genome, &td[i]);
			break;
		case BenchmarkType::KMEANS:
			workers.emplace_back(worker_kmeans, &td[i]);
			break;
		case BenchmarkType::INTRUDER:
			workers.emplace_back(worker_intruder, &td[i]);
			break;
		case BenchmarkType::LABYRINTH:
			workers.emplace_back(worker_labyrinth, &td[i]);
			break;
		case BenchmarkType::SSCA2:
			workers.emplace_back(worker_ssca2, &td[i]);
			break;
		case BenchmarkType::VACATION:
			workers.emplace_back(worker_vacation, &td[i]);
			break;
		case BenchmarkType::YADA:
			workers.emplace_back(worker_yada, &td[i]);
			break;
		}
	}
...
MAIN int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-t") == 0) {
			...
			if (strcmp(name, "bayes") == 0)
				g_benchmark = BenchmarkType::BAYES;
			else if (strcmp(name, "genome") == 0)
				g_benchmark = BenchmarkType::GENOME;
			else if (strcmp(name, "intruder") == 0)
				g_benchmark = BenchmarkType::INTRUDER;
			else if (strcmp(name, "kmeans") == 0)
				g_benchmark = BenchmarkType::KMEANS;
			else if (strcmp(name, "labyrinth") == 0)
				g_benchmark = BenchmarkType::LABYRINTH;
			else if (strcmp(name, "ssca2") == 0)
				g_benchmark = BenchmarkType::SSCA2;
			else if (strcmp(name, "vacation") == 0)
				g_benchmark = BenchmarkType::VACATION;
			else if (strcmp(name, "yada") == 0)
				g_benchmark = BenchmarkType::YADA;
			else {
			...
			case BenchmarkType::BAYES:
				if (strcmp(argv[i], "-v") == 0)
					parse_int(argc, argv, i, g_bayes_v);
					...
					parse_int(argc, argv, i, g_bayes_i);
				else if (strcmp(argv[i], "-e") == 0)
					parse_int(argc, argv, i, g_bayes_e);
				else {
			...
			case BenchmarkType::GENOME:
				if (strcmp(argv[i], "-g") == 0)
					parse_int(argc, argv, i, g_genome_g);
				else if (strcmp(argv[i], "-s") == 0)
			...
			case BenchmarkType::INTRUDER:
				if (strcmp(argv[i], "-a") == 0)
					parse_int(argc, argv, i, g_intruder_a);
				else if (strcmp(argv[i], "-l") == 0)
					parse_int(argc, argv, i, g_intruder_l);
				else if (strcmp(argv[i], "-n") == 0)
					parse_int(argc, argv, i, g_intruder_n);
				else if (strcmp(argv[i], "-s") == 0)
					parse_int(argc, argv, i, g_intruder_s);
				else {
					std::cerr << "Unknown flag: " << argv[i] << "\n";
					return 1;
				}
			...
			case BenchmarkType::KMEANS:
				if (strcmp(argv[i], "-m") == 0)
					parse_int(argc, argv, i, g_kmeans_m);
				else if (strcmp(argv[i], "-n") == 0)
					parse_int(argc, argv, i, g_kmeans_n);
				else if (strcmp(argv[i], "-t") == 0)
					parse_double(argc, argv, i, g_kmeans_t);
				else if (strcmp(argv[i], "-i") == 0) {
					if (i + 1 < argc)
						g_kmeans_i = argv[++i];
				} else {
			...
			case BenchmarkType::LABYRINTH:
				if (strcmp(argv[i], "-x") == 0)
					parse_int(argc, argv, i, g_labyrinth_x);
				else if (strcmp(argv[i], "-y") == 0)
					parse_int(argc, argv, i, g_labyrinth_y);
				else if (strcmp(argv[i], "-z") == 0)
					parse_int(argc, argv, i, g_labyrinth_z);
				else if (strcmp(argv[i], "-n") == 0)
					parse_int(argc, argv, i, g_labyrinth_n);
				else {
					std::cerr << "Unknown flag: " << argv[i] << "\n";
					return 1;
				}
			...
			case BenchmarkType::SSCA2:
				if (strcmp(argv[i], "-s") == 0)
					parse_int(argc, argv, i, g_ssca2_s);
				else if (strcmp(argv[i], "-i") == 0)
					parse_int(argc, argv, i, g_ssca2_i);
				else if (strcmp(argv[i], "-u") == 0)
					parse_double(argc, argv, i, g_ssca2_u);
				else if (strcmp(argv[i], "-l") == 0)
					parse_int(argc, argv, i, g_ssca2_l);
				else if (strcmp(argv[i], "-p") == 0)
					parse_int(argc, argv, i, g_ssca2_p);
				else {
					std::cerr << "Unknown flag: " << argv[i] << "\n";
					return 1;
				}
			...
			case BenchmarkType::VACATION:
				if (strcmp(argv[i], "-n") == 0)
					parse_int(argc, argv, i, g_vacation_n);
				else if (strcmp(argv[i], "-q") == 0)
					parse_int(argc, argv, i, g_vacation_q);
				else if (strcmp(argv[i], "-r") == 0)
					parse_int(argc, argv, i, g_vacation_r);
				else if (strcmp(argv[i], "-u") == 0)
					parse_int(argc, argv, i, g_vacation_u);
				else if (strcmp(argv[i], "-t") == 0)
					parse_int(argc, argv, i, g_vacation_t);
				else {
					std::cerr << "Unknown flag: " << argv[i] << "\n";
					return 1;
				}
			...
			case BenchmarkType::YADA:
				if (strcmp(argv[i], "-a") == 0)
					parse_int(argc, argv, i, g_yada_angle);
				else if (strcmp(argv[i], "-j") == 0)
					parse_double(argc, argv, i, g_yada_jitter);
				else if (strcmp(argv[i], "-i") == 0) {
					if (i + 1 < argc)
						g_yada_i = argv[++i];
				} else {
	...
	std::cout << "========= STAMP Benchmark Suite =========\n";
	std::cout << "=========================================\n";
