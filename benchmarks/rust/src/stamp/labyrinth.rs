...
pub fn test() -> i32 {
    ...
    if path.is_empty() {
        eprintln!("FAIL: empty grid BFS found no path");
        fails += 1;
    }
    if path.len() < 5 {
        eprintln!("FAIL: path too short: {}", path.len());
        fails += 1;
    }
    ...
    if !blocked {
        eprintln!("FAIL: obstacle should block path");
        fails += 1;
    }

    if fails > 0 {
        eprintln!("labyrinth: {} test(s) failed", fails);
    }
    fails
}
        ...
        for tid in 0..config.threads {
            ...
            s.spawn(move || {
                ...
                while !sc.load(Ordering::Relaxed) {
                    ...
                    if local[start_i] != 0 || local[dst_i] != 0 {
                        continue;
                    }
