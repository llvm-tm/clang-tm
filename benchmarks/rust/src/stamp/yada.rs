...
fn make_edge(ax: f64, ay: f64, bx: f64, by: f64) -> Edge {
    ...
    if abit < bbit || (abit == bbit && ay.to_bits() < by.to_bits()) {
        Edge {
            a: (abit as i64, ay.to_bits() as i64),
            b: (bbit as i64, by.to_bits() as i64),
        }
    } else {
        Edge {
            a: (bbit as i64, by.to_bits() as i64),
            b: (abit as i64, ay.to_bits() as i64),
        }
    }
}
...
struct YadaState {
    ...
    pt_x: Vec<TmCell<f64>>,
    pt_y: Vec<TmCell<f64>>,
    circ_x: Vec<TmCell<f64>>,
    circ_y: Vec<TmCell<f64>>,
    circ_r: Vec<TmCell<f64>>,
    min_angle: Vec<TmCell<f64>>,
    encroached: Vec<TmCell<i64>>,
    is_garbage: Vec<TmCell<i64>>,
    nbr_cnt: Vec<TmCell<i64>>,
    nbr_data: Vec<TmCell<i64>>,
    work_heap: Vec<TmCell<i64>>,
    work_heap_cnt: TmCell<i64>,
    elem_cnt: TmCell<i64>,
    angle: f64,
}
...
pub fn test() -> i32 {
    ...
    total += 1;
    if (cx - 0.5).abs() > 1e-10 {
        eprintln!("  FAIL: circumcenter x {}", cx);
        fails += 1;
    }
    total += 1;
    if (cy - 0.5).abs() > 1e-10 {
        eprintln!("  FAIL: circumcenter y {}", cy);
        fails += 1;
    }
    total += 1;
    {
        let expected_r = (2.0f64).sqrt() / 2.0;
        if (cr - expected_r).abs() > 1e-10 {
            eprintln!("  FAIL: circumradius {}", cr);
            fails += 1;
        }
    }
    total += 1;
    {
        ...
        if (ang - 60.0).abs() > 1e-10 {
            eprintln!("  FAIL: equilateral angle {}", ang);
            fails += 1;
        }
    }
    total += 1;
    {
        ...
        if (ang - 45.0).abs() > 1e-10 {
            eprintln!("  FAIL: right angle {}", ang);
            fails += 1;
        }
    }
    total += 1;
    {
        if !is_encroached(0.0, 0.0, 2.0, 0.0, 1.0, 0.5) {
            eprintln!("  FAIL: point inside edge circle");
            fails += 1;
        }
    }
    total += 1;
    {
        if is_encroached(0.0, 0.0, 2.0, 0.0, 1.0, 1.5) {
            eprintln!("  FAIL: point outside edge circle");
            fails += 1;
        }
    }
...
pub fn run(config: &Config, stop: &AtomicBool, _ops: &AtomicU64) {
    ...
    println!(
        "  Grid: {}×{}  Angle: {}°  Jitter: {:.2}",
        grid_size, grid_size, angle as i32, jitter
    );
    ...
    for i in 0..grid_size - 1 {
        for j in 0..grid_size - 1 {
            ...
            for triple in [(i0, i1, i3), (i0, i3, i2)] {
                ...
                pt_x.extend([ax, bx, cx_]);
                pt_y.extend([ay, by, cy_]);
                circ_x.push(ccx);
                circ_y.push(ccy);
                circ_r.push(cr);
                min_angle.push(ang);
            ...
            for vi in 0..3 {
                for vj in 0..3 {
                    if pt_x[i * 3 + vi].to_bits() == pt_x[j * 3 + vj].to_bits()
                        && pt_y[i * 3 + vi].to_bits() == pt_y[j * 3 + vj].to_bits()
                    {
                        shared += 1;
                    }
                }
    ...
    for i in 0..init_cnt {
        unsafe {
            *nbr_cnt[i].ptr() = neighbors[i].len() as i64;
        }
        for &n in &neighbors[i] {
            nbr_data.push(TmCell::new(n as i64));
        }
        for _ in neighbors[i].len()..MAX_NEIGHBORS {
            nbr_data.push(TmCell::new(-1));
        }
    }
    ...
    for _ in init_cnt..max_elems {
        ...
        circ_y.push(0.0);
        circ_r.push(0.0);
        min_angle.push(180.0);
        encroached.push(TmCell::new(0));
        is_garbage.push(TmCell::new(1));
        nbr_cnt.push(TmCell::new(0));
    ...
    let state = Arc::new(YadaState {
        max_elems,
        pt_x: pt_x.into_iter().map(TmCell::new).collect(),
            ...
            s.spawn(move || {
                loop {
                    ...
                    let num_new = transaction(|tx| {
                        ...
                        if tx.read(&st.is_garbage[eid]) != 0 {
                            return 0i64;
                        }
                        let p0 = (tx.read(&st.pt_x[eid * 3]), tx.read(&st.pt_y[eid * 3]));
                        let p1 = (
                            tx.read(&st.pt_x[eid * 3 + 1]),
                            tx.read(&st.pt_y[eid * 3 + 1]),
                        );
                        let p2 = (
                            tx.read(&st.pt_x[eid * 3 + 2]),
                            tx.read(&st.pt_y[eid * 3 + 2]),
                        );
                        ...
                        for edge in &dedup {
                            ...
                            let ea = (
                                f64::from_bits(edge.a.0 as u64),
                                f64::from_bits(edge.a.1 as u64),
                            );
                            let eb = (
                                f64::from_bits(edge.b.0 as u64),
                                f64::from_bits(edge.b.1 as u64),
                            );
    ...
    let mut garb = 0i64;
    for i in 0..final_el as usize {
        if unsafe { *state.is_garbage[i].ptr() } != 0 {
            garb += 1;
        }
    }
    println!(
        "  Operations: {}  Total elements: {} (garbage: {})  Elapsed: {} ms",
