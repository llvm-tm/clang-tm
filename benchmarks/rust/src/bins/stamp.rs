use benchmarks::stamp::{Bench, Config};
...
fn parse_args() -> Config {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-k" if i + 1 < args.len() => {
                c.clusters = args[i + 1].parse().unwrap_or(5);
                i += 2;
            }
            "-n" if i + 1 < args.len() => {
                c.points = args[i + 1].parse().unwrap_or(10000);
                i += 2;
            }
            "-g" if i + 1 < args.len() => {
                c.gene_length = args[i + 1].parse().unwrap_or(16384);
                i += 2;
            }
            "-s" if i + 1 < args.len() => {
                c.segment_length = args[i + 1].parse().unwrap_or(64);
                i += 2;
            }
            "-x" if i + 1 < args.len() => {
                c.grid_x = args[i + 1].parse().unwrap_or(5);
                i += 2;
            }
            "-y" if i + 1 < args.len() => {
                c.grid_y = args[i + 1].parse().unwrap_or(5);
                i += 2;
            }
            "-z" if i + 1 < args.len() => {
                c.grid_z = args[i + 1].parse().unwrap_or(5);
                i += 2;
            }
            "-r" if i + 1 < args.len() => {
                c.num_rooms = args[i + 1].parse().unwrap_or(10);
                i += 2;
            }
            "-c" if i + 1 < args.len() => {
                c.num_customers = args[i + 1].parse().unwrap_or(100);
                i += 2;
            }
            "-a" if i + 1 < args.len() => {
                c.angle = args[i + 1].parse().unwrap_or(20.0);
                i += 2;
            }
            "-j" if i + 1 < args.len() => {
                c.jitter = args[i + 1].parse().unwrap_or(0.5);
                i += 2;
            }
            "-u" if i + 1 < args.len() => {
                c.prob_unidirectional = args[i + 1].parse().unwrap_or(0.5);
                i += 2;
            }
            "-l" if i + 1 < args.len() => {
                c.subgr_edge_length = args[i + 1].parse().unwrap_or(3);
                i += 2;
            }
            "-i" if i + 1 < args.len() => {
                c.iterations = args[i + 1].parse().unwrap_or(10);
                i += 2;
            }
            "-m" if i + 1 < args.len() => {
                c.max_length = args[i + 1].parse().unwrap_or(128);
                i += 2;
            }
...
fn run_benchmarks(config: &Config) {
    let bench_name = match config.bench {
        Bench::All => "all",
        Bench::KMeans => "kmeans",
        Bench::Labyrinth => "labyrinth",
        Bench::Vacation => "vacation",
        Bench::Genome => "genome",
    ...
    println!(
        "Bench: {bench_name}  Threads: {t}  Duration: {d}ms",
        t = config.threads,
        d = config.duration
    );
    println!();
