...
┗━━┛━━┗━━━┛┗━━━━┛┗━━━━┛\n\
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-a" if i + 1 < args.len() => {
                accounts = args[i + 1].parse().unwrap_or(64);
                i += 2;
            }
            "-s" if i + 1 < args.len() => {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "Usage: fuzz_bank [-t threads] [-n iters] [-a accounts] [-s seed] [--queue]"
                );
                return;
            }
            _ => i += 1,
    ...
    let bank: Arc<Vec<TmCell<i32>>> =
        Arc::new((0..accounts).map(|_| TmCell::new(init_balance)).collect());
