...
impl Database {
    fn new() -> Self {
        ...
        while base_assemblies.len() < MAX_BA * 2 {
            base_assemblies.push(BaseAssembly {
        ...
        while composite_parts.len() < MAX_CP * 2 {
            composite_parts.push(CompositePart {
        ...
        while atomic_parts.len() < MAX_AP * 2 {
            atomic_parts.push(AtomicPart {
        ...
        while connections.len() < MAX_CONN * 2 {
            connections.push(Connection {
                id: connections.len() as i32,
...
fn op_st3(db: &Database, tx: &Transaction, ap_idx: usize) -> i64 {
    ...
    for &ci in &ap.connection_ids {
        if ci < db.connections.len() && tx.read(&db.connections[ci].valid) != 0 {
            let nb = if db.connections[ci].from_atomic_part_id as usize == ap_idx {
                db.connections[ci].to_atomic_part_id as usize
            } else {
                db.connections[ci].from_atomic_part_id as usize
            };
...
fn op_st5(db: &Database, tx: &Transaction, low: i32, high: i32) -> i64 {
    ...
    for &(d, i) in &db.cp_by_date {
        ...
        if i < db.composite_parts.len() && tx.read(&db.composite_parts[i].valid) != 0 {
            count += 1;
        }
    }
    count
...
fn op_st6(db: &Database, tx: &Transaction, ap_idx: usize) {
    if ap_idx < db.atomic_parts.len() {
        tx.write(
            &db.atomic_parts[ap_idx].x,
            (tx.read(&db.atomic_parts[ap_idx].x) + 1) % 100,
        );
        tx.write(
            &db.atomic_parts[ap_idx].y,
            (tx.read(&db.atomic_parts[ap_idx].y) + 1) % 100,
        );
    }
}
fn op_st7(db: &Database, tx: &Transaction, cp_idx: usize) -> i64 {
    ...
    for &ai in &db.composite_parts[cp_idx].atomic_part_ids {
        if ai < db.atomic_parts.len() && tx.read(&db.atomic_parts[ai].valid) != 0 {
            max_w = max_w.max(tx.read(&db.atomic_parts[ai].weight) as i64);
        }
    }
    max_w
}
...
fn op_op1(db: &Database, tx: &Transaction, id: usize) -> i64 {
    if id < db.atomic_parts.len() && tx.read(&db.atomic_parts[id].valid) != 0 {
        let ap = &db.atomic_parts[id];
        return (tx.read(&ap.x) + tx.read(&ap.y) + tx.read(&ap.z)) as i64;
    }
    0
}
fn op_op2(db: &Database, tx: &Transaction, id: usize) -> i64 {
    if id < db.composite_parts.len() && tx.read(&db.composite_parts[id].valid) != 0 {
        return tx.read(&db.composite_parts[id].build_date) as i64;
    }
    0
}
fn op_op3(db: &Database, tx: &Transaction, doc_idx: usize) -> i64 {
    if doc_idx < db.documents.len() {
        return (tx.read(&db.documents[doc_idx].build_date)
            + tx.read(&db.documents[doc_idx].doc_type)) as i64;
    }
    0
}
fn op_op4(db: &Database, tx: &Transaction, ba_idx: usize) -> i64 {
    if ba_idx < db.base_assemblies.len() && tx.read(&db.base_assemblies[ba_idx].valid) != 0 {
        ...
        return ba
            ...
            .map(|&ci| {
                if ci < db.composite_parts.len() && tx.read(&db.composite_parts[ci].valid) != 0 {
                    tx.read(&db.composite_parts[ci].build_date) as i64
                } else {
                    0
                }
            })
...
fn op_op5(db: &Database, tx: &Transaction, ca_idx: usize) -> i64 {
    if ca_idx < db.complex_assemblies.len() {
        ...
        return ca
            ...
            .map(|&ci| {
                if ci < db.complex_assemblies.len() {
                    tx.read(&db.complex_assemblies[ci].build_date) as i64
                } else {
                    0
                }
            })
            .sum::<i64>()
            + ca.child_base_assembly_ids
                .iter()
                .map(|&bi| {
                    if bi < db.base_assemblies.len() && tx.read(&db.base_assemblies[bi].valid) != 0
                    {
                        tx.read(&db.base_assemblies[bi].build_date) as i64
                    } else {
                        0
                    }
                })
...
fn op_op6(db: &Database, tx: &Transaction, ap_idx: usize) -> i64 {
    if ap_idx < db.atomic_parts.len() && tx.read(&db.atomic_parts[ap_idx].valid) != 0 {
        return db.atomic_parts[ap_idx]
            .connection_ids
            .iter()
            .map(|&ci| {
                if ci < db.connections.len() && tx.read(&db.connections[ci].valid) != 0 {
                    tx.read(&db.connections[ci].conn_type) as i64
                } else {
                    0
                }
            })
...
fn op_op7(db: &Database, tx: &Transaction, cp_idx: usize) -> i64 {
    if cp_idx < db.composite_parts.len() && tx.read(&db.composite_parts[cp_idx].valid) != 0 {
        return tx.read(&db.composite_parts[cp_idx].ap_count) as i64;
    }
    0
}
fn op_op8(db: &Database, tx: &Transaction, cp_idx: usize) -> i64 {
    if cp_idx < db.composite_parts.len() && tx.read(&db.composite_parts[cp_idx].valid) != 0 {
        return db.composite_parts[cp_idx].base_assembly_ids.len() as i64;
    }
    0
}
fn op_op9(db: &Database, tx: &Transaction, cp_idx: usize) -> i64 {
    if cp_idx < db.composite_parts.len() && tx.read(&db.composite_parts[cp_idx].valid) != 0 {
        ...
        for &bi in &db.composite_parts[cp_idx].base_assembly_ids {
            if bi < db.base_assemblies.len() && tx.read(&db.base_assemblies[bi].valid) != 0 {
                oldest = oldest.min(tx.read(&db.base_assemblies[bi].build_date) as i64);
            }
        }
        return oldest;
    }
        ...
        return db.atomic_parts[ap_idx]
            ...
            .map(|&ci| {
                if ci < db.connections.len() && tx.read(&db.connections[ci].valid) != 0 {
                    ...
                    if nb < db.atomic_parts.len() && tx.read(&db.atomic_parts[nb].valid) != 0 {
                        return tx.read(&db.atomic_parts[nb].weight) as i64;
                    }
                }
                0i64
            })
...
fn op_op11(db: &Database, tx: &Transaction, ap_idx: usize, new_x: i32, new_y: i32) {
    if ap_idx < db.atomic_parts.len() && tx.read(&db.atomic_parts[ap_idx].valid) != 0 {
        tx.write(&db.atomic_parts[ap_idx].x, new_x);
        tx.write(&db.atomic_parts[ap_idx].y, new_y);
    }
}
fn op_op12(db: &Database, tx: &Transaction, ap_idx: usize, new_w: i32) {
    if ap_idx < db.atomic_parts.len() && tx.read(&db.atomic_parts[ap_idx].valid) != 0 {
        tx.write(&db.atomic_parts[ap_idx].weight, new_w);
    }
}
fn op_op13(db: &Database, tx: &Transaction, doc_idx: usize, new_date: i32) {
    if doc_idx < db.documents.len() {
        tx.write(&db.documents[doc_idx].build_date, new_date);
    }
}
fn op_op14(db: &Database, tx: &Transaction, cp_idx: usize, new_date: i32) {
    if cp_idx < db.composite_parts.len() && tx.read(&db.composite_parts[cp_idx].valid) != 0 {
        tx.write(&db.composite_parts[cp_idx].build_date, new_date);
    }
}
fn op_op15(db: &Database, tx: &Transaction, ba_idx: usize, new_date: i32) {
    if ba_idx < db.base_assemblies.len() && tx.read(&db.base_assemblies[ba_idx].valid) != 0 {
        tx.write(&db.base_assemblies[ba_idx].build_date, new_date);
    }
}
...
fn make_task(db: &Arc<Database>, desc: &OpDesc, rng: &mut Xsrng) -> Task {
    ...
    match (desc.cat, desc.is_read, rng.range(0, 20)) {
        (0, true, _) => Box::new(move |tx| {
            op_lt1(&db, tx);
        }),
        (0, false, _) => Box::new(move |tx| {
            op_lt2(&db, tx);
        }),
        (1, true, s) if s < 4 => {
            let di = rng.range(0, MAX_DOCS);
            Box::new(move |tx| {
                op_st2(&db, tx, di);
            })
        }
        (1, true, s) if s < 8 => {
            let ai = rng.range(0, MAX_AP);
            Box::new(move |tx| {
                op_st3(&db, tx, ai);
            })
        }
        ...
        (1, false, _) => Box::new(move |tx| {
            op_st1(&db, tx);
        }),
        (2, true, s) if s < 10 => {
            let id = rng.range(0, MAX_AP);
            Box::new(move |tx| {
                op_op1(&db, tx, id);
            })
        }
        ...
        (2, false, s) if s < 4 => {
            let ai = rng.range(0, MAX_AP);
            let nx = rng.range_i32(0, 100);
            let ny = rng.range_i32(0, 100);
            Box::new(move |tx| {
                op_op11(&db, tx, ai, nx, ny);
            })
        }
        (2, false, _) => {
            let ai = rng.range(0, MAX_AP);
            let nw = rng.range_i32(1, 51);
            Box::new(move |tx| {
                op_op12(&db, tx, ai, nw);
            })
        }
        ...
        (3, _, s) if s < 17 => {
            let a = rng.range(0, MAX_AP);
            let b = rng.range(0, MAX_AP);
            let t = rng.range_i32(0, 3);
            Box::new(move |tx| {
                op_sm5(&db, tx, a, b, t);
            })
        }
        (3, _, _) => {
            let ci = rng.range(0, MAX_CONN);
            Box::new(move |tx| {
                op_sm6(&db, tx, ci);
            })
        }
...
fn main() {
    ...
    println!(
        "Duration: {} ms  Workers: {}  Batch size: {}",
        duration_ms, nb_threads, batch_size
    );
    ...
    loop {
        ...
        if (elapsed * 1000.0) >= duration_ms as f64 {
            break;
        }
