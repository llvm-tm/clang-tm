...
(`llvm_tm_plugin/tm_pipeline.mk`), which automates the 4-step pipeline:
...
```sh
make TM_PLUGIN=../../llvm_tm_plugin/bin/libTMInstrument_no_setjmp.so bank_singlelock
```
...
make -C benchmarks/datastructures hashmap_SingleGlobalLock
make -C benchmarks/STAMP stamp_tl2
make -C benchmarks/STMbench7 stmbench_tinystm
make -C benchmarks/TPCC tpcc_persistentsgl
make -C benchmarks/YCSB ycsb_singlelock
make -C benchmarks/EigenBench eigen_norec
```
...
cd benchmarks
for d in test/bank datastructures STAMP STMbench7 TPCC YCSB EigenBench; do
    make -C "$d" all
done
```
