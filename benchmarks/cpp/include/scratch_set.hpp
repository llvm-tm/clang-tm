...
template <typename T> class ScratchVector
{
	T *m_data = nullptr;
	size_t m_size = 0;
	size_t m_cap = 0;
...
public:
	...
	explicit ScratchVector(size_t n, const T &val = T{})
	    : m_size(0),
	      m_cap(0)
	{
		resize(n, val);
...
template <typename T> class ScratchSet
{
	T *m_data = nullptr;
	size_t m_size = 0;
	size_t m_cap = 0;
...
public:
	...
	{
		size_t i = lower_bound(v);
		if (i < m_size && !(m_data[i] < v) && !(v < m_data[i]))
			return;
		if (m_size >= m_cap)
			grow(m_cap ? m_cap * 2 : 64);
