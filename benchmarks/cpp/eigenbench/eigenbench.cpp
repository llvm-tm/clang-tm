...
Config parse_args(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (!strcmp(argv[i], "-t"))
			...
			c.a3 = nxt();
		else if (!strcmp(argv[i], "--contention"))
			c.contention = nxt_d();
		else if (!strcmp(argv[i], "--locality"))
			c.locality = nxt_d();
		else if (!strcmp(argv[i], "--density"))
			c.density = nxt_d();
		else if (!strcmp(argv[i], "--mode"))
			c.mode = nxt();
		else if (!strcmp(argv[i], "--enable-r2"))
			c.r2_enabled = true;
		else if (!strcmp(argv[i], "--enable-r3"))
			c.r3_enabled = true;
		else if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
			printf(
			    "Usage: eigenbench [-t n] [-d ms] [--r1 n] [--w1 n] [--r2 n] [--w2 n]\n"
...
int main(int argc, char *argv[])
{
	...
	for (int t = 0; t < cfg.threads; ++t)
		threads.emplace_back([t, &arrays, &cfg]() { run_worker(t, arrays, cfg); });
