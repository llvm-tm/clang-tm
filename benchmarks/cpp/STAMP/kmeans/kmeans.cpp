...
static void parse_args(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-p") && i + 1 < argc)
			...
			g_npoints = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-t") && i + 1 < argc)
			g_threshold = atof(argv[++i]);
		else if (!strcmp(argv[i], "-h")) {
			fprintf(stderr,
			        "Usage: %s -p <threads> -k <clusters> -d <dims> -n <points> -t "
			        "<threshold>\n",
			        argv[0]);
			return;
		}
...
static void kmeans_accumulate(KMeansData *data,
                              int start,
                              int end,
                              double *local_sum,
                              int *local_count)
{
	tx_retry([&]() {
		for (int i = start; i < end; i++) {
			...
			local_count[best]++;
			for (int d = 0; d < data->ndims; d++) {
				local_sum[best * data->ndims + d] += tm_read_double(
				    &data->points[i * data->ndims + d]);
			}
		}
	});
}
	...
	while (!converged && max_iters-- > 0) {
		...
		for (int c = 0; c < nclusters; c++) {
			if (data.new_centers_count[c] > 0) {
				for (int d = 0; d < ndims; d++) {
					double new_val = data.new_centers_sum[c * ndims + d] /
					                 data.new_centers_count[c];
...
static int test_cli_flags()
{
	...
	const char *av2[] =
	    {"kmeans", "-p", "8", "-k", "10", "-d", "3", "-n", "100", "-t", "0.001"};
...
int main(int argc, char *argv[])
{
	...
	g_data.new_centers_sum = (double *)tm_calloc((size_t)g_nclusters * g_ndims,
	                                             sizeof(double));
