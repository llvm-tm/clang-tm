...
template <typename F> inline void tx_retry(F &&body)
{
...
static void parse_args(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-p") && i + 1 < argc)
			...
			g_num_requests = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-h")) {
			fprintf(stderr,
			        "Usage: %s -p <threads> -x <width> -y <height> -z <depth> -n "
			        "<requests>\n",
			        argv[0]);
			exit(0);
		}
...
static bool do_traceback(ScratchVector<Point3D> &path,
                         const long *dist,
                         int w,
                         int h,
                         int d,
...
static void worker(int thread_id, int num_threads)
{
	...
	for (int i = thread_id; i < data->num_requests; i += num_threads) {
		...
		while (true) {
			...
			if (!ok || !traced || path.empty())
				break;

			if (labyrinth_mark(data, path))
				break;
...
static void test_cli_flags()
{
	...
	int save_p = g_num_threads, save_x = g_width, save_y = g_height, save_z = g_depth,
	    save_n = g_num_requests;
