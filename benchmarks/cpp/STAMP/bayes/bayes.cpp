...
static void parse_args(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-v") == 0 && i + 1 < argc)
			g_num_var = atol(argv[++i]);
		else if (strcmp(argv[i], "-r") == 0 && i + 1 < argc)
			g_num_record = atol(argv[++i]);
		else if (strcmp(argv[i], "-n") == 0 && i + 1 < argc)
			g_max_parents = atol(argv[++i]);
		else if (strcmp(argv[i], "-s") == 0 && i + 1 < argc)
			g_seed = (unsigned)atol(argv[++i]);
		else if (strcmp(argv[i], "-i") == 0 && i + 1 < argc)
			g_insert_penalty = atol(argv[++i]);
		else if (strcmp(argv[i], "-e") == 0 && i + 1 < argc)
			g_max_edges_per_var = atol(argv[++i]);
...
static long rng_range(long range) { return (long)(rng_next() % (uint32_t)range); }
...
static double compute_density_ll(int var, const ScratchVector<int> &parents_vec)
{
	...
	for (int c = 0; c < ncfg; c++) {
		...
		if (p0 > 0)
			ll += frac * p0 * std::log(p0);
		if (p1 > 0)
			ll += frac * p1 * std::log(p1);
	}
	return ll;
}
				...
				if (with_ll > base_ll) {
					...
					tx_run([&]() {
						t.score = g_base_penalty +
						          g_num_record *
						              (TM_READ_DOUBLE(g_base_log_likelihood) + delta);
					});
...
static void test_cli_flags()
{
	...
	unsigned ss = g_seed;
	long si = g_insert_penalty, se = g_max_edges_per_var, st = g_num_threads;
	...
	for (int v = 1; v < num_var_i; v++) {
		...
		for (int p = 0; p < np && p < v; p++) {
			...
			for (int j = 0; j < (int)g_parent_count[v]; j++) {
				if (g_parent_data[v * MAX_PARENTS + j] == parent) {
					found = true;
					break;
				}
			}
