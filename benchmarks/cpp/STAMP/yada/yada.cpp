...
static void parse_args(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-a") == 0 && i + 1 < argc)
			g_angle_constraint = atol(argv[++i]);
		else if (strcmp(argv[i], "-j") == 0 && i + 1 < argc)
			g_jitter = atof(argv[++i]);
...
static inline long d2l(double v)
{
	long r;
	memcpy(&r, &v, sizeof(r));
	return r;
}
static inline double l2d(long v)
{
	double r;
	memcpy(&r, &v, sizeof(r));
	return r;
...
static void circumcircle(double &cx,
                         double &cy,
                         double &cr,
                         const Point &a,
                         const Point &b,
...
TX_FUNC static long read_encroached(int id) { return TM_READ_I8(&g_elem_encroached[id]); }

TX_FUNC static long read_is_garbage(int id) { return TM_READ_I8(&g_elem_is_garbage[id]); }
                                   ...
                                   ScratchVector<int> &bfs_queue)
{
	...
	for (auto &edge : border_edges) {
		...
		if (is_encroached(ta, tb, tc) || is_encroached(tb, tc, ta) ||
	...
	if (border_count > 0 && saved_new_id < MAX_ELEMENTS) {
		...
		circumcircle(g_circ_x[saved_new_id],
		             g_circ_y[saved_new_id],
		             g_circ_r[saved_new_id],
		             centroid,
		             p0,
		             p1);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < grid_size; i++) {
		for (int j = 0; j < grid_size; j++) {
			points.push_back(
			    {i * spacing + jitter_rng(g_jitter), j * spacing + jitter_rng(g_jitter)});
		}
	}
