			...
			if (qtail < g_decoded_q_capacity) {
				...
				for (int i = 0; i < total; i++)
					tm_write_i1((uint8_t *)&dst->data[i],
					            (uint8_t)tm_read_i1(
					                (uint8_t *)&g_decoder_flows[idx].data[i]));
...
static void test_cli_flags()
{
	...
	g_num_threads = save_p;
	g_percent_attack = save_a;
	g_max_data_length = save_l;
	g_num_flows = save_n;
	g_seed = save_s;
	if (test_result() != 0)
		exit(1);
...
static void parse_args(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		if (!strcmp(argv[i], "-p") && i + 1 < argc)
			...
			g_percent_attack = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-l") && i + 1 < argc)
			g_max_data_length = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-n") && i + 1 < argc)
			g_num_flows = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-s") && i + 1 < argc)
			g_seed = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-h")) {
			fprintf(stderr,
			        "Usage: %s -p <threads> -a <pct_attack> -l <max_len> -n <flows> -s "
			        "<seed>\n",
			        argv[0]);
			exit(0);
		}
...
int main(int argc, char *argv[])
{
	...
	const char *wordlist[] = {"about", "attack", "back",  "root",   "system", "access",
	                          "all",   "after",  "also",  "and",    "any",    "are",
	                          "but",   "can",    "come",  "could",  "did",    "do",
	                          "each",  "find",   "first", "for",    "from",   "get",
	                          "go",    "has",    "have",  "her",    "here",   "him",
	                          "his",   "how",    "into",  "its",    "just",   "know",
	                          "like",  "look",   "make",  "man",    "may",    "more",
	                          "most",  "must",   "new",   "no",     "not",    "now",
	                          "old",   "one",    "only",  "other",  "our",    "out",
	                          "over",  "own",    "part",  "people", "said",   "say",
	                          "see",   "she",    "shell", "should", "site",   "some",
	                          "such",  "take",   "than",  "that",   "their",  "them",
	                          "then",  "there"};
	...
	g_fragment_storage = (char *)tm_calloc(max_flows * INTRUDER_MAX_PACKETS *
	                                           INTRUDER_MAX_DATA,
	                                       1);
