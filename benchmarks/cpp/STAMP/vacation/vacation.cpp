...
{
	fprintf(stderr,
	        "Usage: %s [-n <queries>] [-r <relations>] [-u <pct_user>] [-t <tasks>] "
	        "[-p <num_threads>]\n",
	        prog);
}
...
static int test_cli_flags()
{
	...
	}
	{
		int nr = 16384, nq = 2, pct = 98, tasks = 4096, thr = 4;
		const char *fake[] =
		    {"vacation", "-n", "5", "-r", "100", "-u", "50", "-t", "200", "-p", "8"};
...
static int test_manager()
{
	...
	int free = -1, price = -1;
	bool ok = false;
	tx_retry([&]() { ok = add_reservation(&data.cars, 1, 100, 200); });
	TEST_EQ(ok, true, "add car reservation");

	tx_retry([&]() { free = query_num_free(&data.cars, 1); });
	TEST_EQ(free, 100, "car num_free after add");

	tx_retry([&]() { price = query_price(&data.cars, 1); });
	TEST_EQ(price, 200, "car price after add");

	tx_retry([&]() { ok = delete_reservation(&data.cars, 1, 60); });
	TEST_EQ(ok, true, "delete partial reservation");

	tx_retry([&]() { free = query_num_free(&data.cars, 1); });
	TEST_EQ(free, 40, "car num_free after partial delete");

	tx_retry([&]() { ok = delete_reservation(&data.cars, 1, 40); });
