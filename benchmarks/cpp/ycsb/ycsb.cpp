...
Config parse_args(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (!strcmp(argv[i], "-t"))
			...
			c.initial_records = nxt();
		else if (!strcmp(argv[i], "-w") && i + 1 < argc) {
			const char *wl = argv[++i];
			if (!strcmp(wl, "a") || !strcmp(wl, "A"))
				c.workload = WL_A;
			else if (!strcmp(wl, "b") || !strcmp(wl, "B"))
				c.workload = WL_B;
			else if (!strcmp(wl, "c") || !strcmp(wl, "C"))
				c.workload = WL_C;
			else if (!strcmp(wl, "d") || !strcmp(wl, "D"))
				c.workload = WL_D;
			else if (!strcmp(wl, "e") || !strcmp(wl, "E"))
				c.workload = WL_E;
			else if (!strcmp(wl, "f") || !strcmp(wl, "F"))
				c.workload = WL_F;
		} else if (!strcmp(argv[i], "-dist") && i + 1 < argc) {
			const char *d = argv[++i];
			if (!strcmp(d, "u") || !strcmp(d, "uniform"))
				c.dist = DIST_UNIFORM;
			else if (!strcmp(d, "z") || !strcmp(d, "zipfian"))
				c.dist = DIST_ZIPFIAN;
			else if (!strcmp(d, "l") || !strcmp(d, "latest"))
				c.dist = DIST_LATEST;
		} else if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
			printf("Usage: ycsb [-t n] [-d ms] [-w a|b|c|d|e|f] [-k n] [-i n] [-dist "
			       "u|z|l]\n");
			exit(0);
		}
	}
...
struct Database {
	...
	{
		if (count.load() >= (uint64_t)max_records)
			return;
		int h = hash(key);
