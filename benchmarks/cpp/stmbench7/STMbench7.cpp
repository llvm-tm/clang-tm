...
static void parse_args(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-t") == 0 && i + 1 < argc)
			...
			g_duration_ms = atoi(argv[++i]);
		else if (strcmp(argv[i], "-w") == 0 && i + 1 < argc) {
			int w = atoi(argv[++i]);
			if (w == 1)
				g_writePercent = 10; // read-dominated
			else if (w == 2)
				g_writePercent = 40; // read-write
			else if (w == 3)
				g_writePercent = 90; // write-dominated
		}
	}
}
...
static void init_data()
{
	...
	for (int i = 0; i < MAX_CP; i++) {
		...
		doc.id = i;
		doc.type.poke(i % 3);
		doc.buildDate.poke(1000 + (i % 365));
		doc.compositePartId = i;
		g_documents[i] = doc;
	...
	// APs + connections
	{
		...
		for (int ci = 0; ci < MAX_CP; ci++) {
			...
			for (int j = 0; j < AP_PER_CP; j++) {
				...
				ap.weight.poke((j % 50) + 1);
				ap.compositePartId = ci;
	...
	printf("Init done: %zu CA, %zu BA, %zu CP, %zu AP, %zu conn\n",
	       g_complexAssemblies.size(),
	       g_baseAssemblies.size(),
	       g_compositeParts.size(),
	       g_atomicParts.size(),
	       g_connections.size());
}
...
static OpResult op_lt1()
{
	...
	for (size_t i = 0; i < g_complexAssemblies.size(); i++)
		s += g_complexAssemblies[i].id + g_complexAssemblies[i].level +
		     g_complexAssemblies[i].buildDate.read();
...
static OpResult op_lt2()
{
	expli::TM<int>::begin();
	for (size_t i = 0; i < g_complexAssemblies.size(); i++)
		g_complexAssemblies[i].buildDate.write(
		    (g_complexAssemblies[i].buildDate.read() + 1) % 365 + 1000);
	for (size_t i = 0; i < g_baseAssemblies.size(); i++)
		g_baseAssemblies[i].buildDate.write(
		    (g_baseAssemblies[i].buildDate.read() + 1) % 365 + 1000);
	expli::TM<int>::end();
	return {0, true};
}
...
static OpResult op_lt5()
{
	...
	for (size_t i = 0; i < g_connections.size(); i++)
		s += g_connections[i].id + g_connections[i].fromAtomicPartId +
		     g_connections[i].toAtomicPartId + g_connections[i].type.read();
	expli::TM<int>::end();
	return {(uint64_t)s, false};
}
...
static OpResult op_st1()
{
	...
	for (int l = 0; l < TREE_LEVELS - 1 && (size_t)ci < g_complexAssemblies.size() &&
	                !g_complexAssemblies[ci].childAssemblyIds.empty();
	     l++) {
		ci = g_complexAssemblies[ci].childAssemblyIds[0];
		s += g_complexAssemblies[ci].id;
	}
	if ((size_t)ci < g_complexAssemblies.size() &&
	    !g_complexAssemblies[ci].childBaseAssemblyIds.empty()) {
...
{
	expli::TM<int>::begin();
	if (doc_idx < 0 || (size_t)doc_idx >= g_documents.size()) {
		expli::TM<int>::end();
		return {0, false};
	}
...
{
	expli::TM<int>::begin();
	if (ap_idx < 0 || (size_t)ap_idx >= g_atomicParts.size()) {
		expli::TM<int>::end();
		return {0, false};
	}
	int64_t s = g_atomicParts[ap_idx].x.read() + g_atomicParts[ap_idx].y.read() +
	            g_atomicParts[ap_idx].z.read();
	for (size_t i = 0; i < g_atomicParts[ap_idx].connectionIds.size(); i++) {
		...
		if ((size_t)cid < g_connections.size()) {
			int nb = (g_connections[cid].fromAtomicPartId == ap_idx)
			             ? g_connections[cid].toAtomicPartId
			             : g_connections[cid].fromAtomicPartId;
...
{
	expli::TM<int>::begin();
	if (ca_idx < 0 || (size_t)ca_idx >= g_complexAssemblies.size()) {
		expli::TM<int>::end();
		return {0, true};
	}
	g_complexAssemblies[ca_idx].buildDate.write(
	    (g_complexAssemblies[ca_idx].buildDate.read() + 1) % 365 + 1000);
	expli::TM<int>::end();
	return {1, true};
}
...
{
	expli::TM<int>::begin();
	if (cp_idx < 0 || (size_t)cp_idx >= g_compositeParts.size()) {
		expli::TM<int>::end();
		return {0, false};
	}
	int max_w = 0;
...
{
	expli::TM<int>::begin();
	if (ba_idx < 0 || (size_t)ba_idx >= g_baseAssemblies.size()) {
		expli::TM<int>::end();
		return {0, true};
	}
	g_baseAssemblies[ba_idx].buildDate.write(
	    (g_baseAssemblies[ba_idx].buildDate.read() + 1) % 365 + 1000);
	expli::TM<int>::end();
	return {1, true};
}
...
static OpResult op_st10(int cp_idx)
{
	...
	if (di >= 0 && (size_t)di < g_documents.size())
		g_documents[di].buildDate.write((g_documents[di].buildDate.read() + 1) % 365 +
		                                1000);
	expli::TM<int>::end();
	return {1, true};
}
...
static OpResult op_op1(int id)
{
	...
	if (idx && *idx >= 0 && (size_t)*idx < g_atomicParts.size())
		r = (uint64_t)(g_atomicParts[*idx].x.read() + g_atomicParts[*idx].y.read() +
		               g_atomicParts[*idx].z.read());
	expli::TM<int>::end();
	return {r, false};
}
...
static OpResult op_op3(int id)
{
	...
	if (idx && *idx >= 0 && (size_t)*idx < g_documents.size())
		r = (uint64_t)(g_documents[*idx].buildDate.read() +
		               g_documents[*idx].type.read());
	expli::TM<int>::end();
	return {r, false};
}
...
static OpResult op_op6(int ap_idx)
{
	...
	uint64_t r = (uint64_t)(g_atomicParts[ap_idx].x.read() +
	                        g_atomicParts[ap_idx].y.read() +
	                        g_atomicParts[ap_idx].z.read());
	expli::TM<int>::end();
	return {r, false};
}
...
static OpResult op_sm1(int new_id)
{
	...
	CompositePart cp;
	cp.id = new_id;
	cp.buildDate.write(2000);
	cp.documentId = doc_idx;
	cp.rootAtomicPartId = first_ap;
	...
	Document doc;
	doc.id = doc_idx;
	doc.type.write(new_id % 3);
	doc.buildDate.write(2000);
	...
	for (int j = 0; j < AP_PER_CP; j++) {
		AtomicPart ap;
		ap.id = (int)g_atomicParts.size();
		ap.x.write(j % 100);
		ap.y.write((j / 100) % 100);
		ap.z.write(j / 10000);
		ap.buildDate.write(2000);
		ap.weight.write(10);
...
static OpResult op_sm5(int from_ap, int to_ap, int typ)
{
	...
	if (to_ap < 0 || (size_t)to_ap >= g_atomicParts.size()) {
		expli::TM<int>::end();
		return {0, true};
	}
	Connection c;
...
{
	expli::TM<int>::begin();
	if (ci < 0 || (size_t)ci >= g_connections.size()) {
		expli::TM<int>::end();
		return {0, true};
	}
...
{
	expli::TM<int>::begin();
	if (parent_ca_idx < 0 || (size_t)parent_ca_idx >= g_complexAssemblies.size()) {
		expli::TM<int>::end();
		return {0, true};
	}
	if (g_complexAssemblies[parent_ca_idx].level != TREE_LEVELS - 1) {
		expli::TM<int>::end();
		return {0, true};
	}
...
static OpResult wrap_op11(std::mt19937 &r)
{
	return op_op11(pick_ap(r), r() % 100, r() % 100);
}
static OpResult wrap_op12(std::mt19937 &r) { return op_op12(pick_ap(r), (r() % 50) + 1); }
static OpResult wrap_op13(std::mt19937 &r)
{
	return op_op13(pick_doc(r), 1000 + (r() % 365));
}
static OpResult wrap_op14(std::mt19937 &r)
{
	return op_op14(pick_cp(r), 1000 + (r() % 365));
}
static OpResult wrap_op15(std::mt19937 &r)
{
	return op_op15(pick_ba(r), 1000 + (r() % 365));
}
...
static OpResult wrap_sm5(std::mt19937 &r)
{
	return op_sm5(pick_ap(r), pick_ap(r), r() % 3);
}
...
template <typename F> static auto run_tx(F &&body) -> decltype(body())
{
...
static void test_cli_flags()
{
	...
	g_num_threads = save_t;
	g_duration_ms = save_d;
	if (test_result() != 0)
		exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	printf("  Long trav:    %d (%.1f%%)\n",
	       g_count_by_cat[0].load(),
	       total > 0 ? g_count_by_cat[0].load() * 100.0 / total : 0);
	printf("  Short trav:   %d (%.1f%%)\n",
	       g_count_by_cat[1].load(),
	       total > 0 ? g_count_by_cat[1].load() * 100.0 / total : 0);
	printf("  Short ops:    %d (%.1f%%)\n",
	       g_count_by_cat[2].load(),
	       total > 0 ? g_count_by_cat[2].load() * 100.0 / total : 0);
	printf("  Struct mods:  %d (%.1f%%)\n",
	       g_count_by_cat[3].load(),
	       total > 0 ? g_count_by_cat[3].load() * 100.0 / total : 0);
	printf("  Read-only:    %llu (%.1f%%)\n",
	       (unsigned long long)ro,
	       total > 0 ? ro * 100.0 / total : 0);
	printf("  Update:       %llu (%.1f%%)\n",
	       (unsigned long long)up,
	       total > 0 ? up * 100.0 / total : 0);
