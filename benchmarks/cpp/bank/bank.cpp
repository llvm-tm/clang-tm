...
template <typename F> void run_tx(F &&body)
{
	if (g_executor) {
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; ++i) {
		if (!strcmp(argv[i], "-d") && i + 1 < argc)
			...
			nb_accounts = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-t") && i + 1 < argc)
			nb_threads = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-r") && i + 1 < argc)
			read_all_pct = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-w") && i + 1 < argc)
			write_all_pct = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-n") && i + 1 < argc)
			g_max_txns = (uint64_t)atoll(argv[++i]);
		else if (!strcmp(argv[i], "--disjoint"))
			disjoint = true;
		else if (!strcmp(argv[i], "--queue"))
			queue_mode = true;
		else if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
			printf("Usage: bank [-d ms] [-a n] [-t n] [-r pct] [-w pct] [-n txns] "
			       "[--disjoint] [--queue]\n");
			printf("  -n txns   stop after this many transactions total (0 = run for -d "
			       "ms)\n");
			return 0;
		}
	}
	...
	fprintf(stderr,
	        ...
	        final_total > expected_total ? final_total - expected_total
