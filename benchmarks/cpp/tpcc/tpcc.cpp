...
void tm_init_thread();
void tm_exit_thread();
extern void *(*tm_calloc)(size_t n, size_t sz);
extern long (*tm_read_i8)(const void *addr);
extern void (*tm_write_i8)(void *addr, long val);
extern int32_t (*tm_read_i4)(const void *addr);
extern void (*tm_write_i4)(void *addr, int32_t val);
extern float (*tm_read_f4)(const void *addr);
extern void (*tm_write_f4)(void *addr, float val);
...
template <typename F> static void tx_run(F &&body)
{
...
static void parse_args(int argc, char *argv[])
{
	for (int i = 1; i < argc; i++) {
		if (strcmp(argv[i], "-t") == 0 && i + 1 < argc)
			...
			g_num_warehouses = atoi(argv[++i]);
		else if (strcmp(argv[i], "--new-order") == 0 && i + 1 < argc)
			g_pct_new_order = atoi(argv[++i]);
		else if (strcmp(argv[i], "--payment") == 0 && i + 1 < argc)
			g_pct_payment = atoi(argv[++i]);
		else if (strcmp(argv[i], "--status") == 0 && i + 1 < argc)
			g_pct_order_status = atoi(argv[++i]);
		else if (strcmp(argv[i], "--delivery") == 0 && i + 1 < argc)
			g_pct_delivery = atoi(argv[++i]);
		else if (strcmp(argv[i], "--stock-level") == 0 && i + 1 < argc)
			g_pct_stock_level = atoi(argv[++i]);
	}
	int sum = g_pct_new_order + g_pct_payment + g_pct_order_status + g_pct_delivery +
	          g_pct_stock_level;
		...
		for (int d = 1; d <= nd; d++) {
			...
			for (int o = 1; o <= PREPOPULATED_ORDERS; o++) {
				...
				for (int l = 1; l <= ol_cnt; l++) {
					...
					g_orderline[ol_idx].ol_delivery_d = (o <= 2100) ? (int)::time(nullptr)
					                                                : 0;
...
static int txn_new_order(int w_id,
                         int d_id,
                         int c_id,
                         int num_items,
                         int *item_ids,
...
static void txn_payment(int w_id, int d_id, int c_id, float amount)
{
	...
	fwrite(&g_customer[c_idx].c_ytd_payment,
	       fread(&g_customer[c_idx].c_ytd_payment) + amount);
...
static void txn_delivery(int w_id, int carrier_id)
{
	for (int d_id = 1; d_id <= g_num_districts; d_id++) {
		...
		int c_idx = idx_c(w_id, d_id, c_id);
		fwrite(&g_customer[c_idx].c_balance,
		       fread(&g_customer[c_idx].c_balance) + ol_total);
		iwrite(&g_customer[c_idx].c_delivery_cnt,
		       iread(&g_customer[c_idx].c_delivery_cnt) + 1);
...
static int txn_stock_level(int w_id, int d_id, int threshold)
{
	...
	for (int oid = start_o_id; oid < next_o_id; oid++) {
		int o_idx = idx_ord(w_id, d_id, oid);
		if (iread(&g_order[o_idx].o_w_id) == 0)
			continue;
...
static void run_worker(int id, int loops)
{
	...
	while (!g_done.load() && loops > 0) {
		...
		if (r < t_no) {
			...
			tx_run([&]() { txn_payment(pay_w_id, d_id, c_id, amount); });
			g_counts[1].fetch_add(1);

		} else if (r < t_os) {
			int c_id = cdist(rng);
			tx_run([&]() { txn_order_status(w_id, d_id, c_id); });
			g_counts[2].fetch_add(1);

		} else if (r < t_dl) {
			int carrier_id = (int)(rng() % 10) + 1;
			tx_run([&]() { txn_delivery(w_id, carrier_id); });
			g_counts[3].fetch_add(1);

		} else {
			int threshold = threshold_dist(rng);
			tx_run([&]() { txn_stock_level(w_id, d_id, threshold); });
...
static void test_cli_flags()
{
	...
	int save_no = g_pct_new_order, save_pay = g_pct_payment, save_os = g_pct_order_status,
	    save_dl = g_pct_delivery, save_sl = g_pct_stock_level;
	...
	g_pct_new_order = save_no;
	g_pct_payment = save_pay;
	g_pct_order_status = save_os;
	g_pct_delivery = save_dl;
...
static void test_logic()
{
	...
	g_district = (District *)tm_calloc((size_t)(g_num_warehouses * g_num_districts),
	                                   sizeof(District));
	g_customer = (Customer *)tm_calloc((size_t)(g_num_warehouses * g_num_districts *
	                                            g_num_customers),
	                                   sizeof(Customer));
