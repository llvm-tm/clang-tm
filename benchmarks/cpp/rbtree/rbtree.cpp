...
static bool insert_serial(BSTNode **root, BSTNode *z)
{
	...
	for (;;) {
		...
		if (key < k) {
			if (!cur->left.peek()) {
				cur->left.poke(reinterpret_cast<uint64_t>(z));
				return true;
			}
			cur = reinterpret_cast<BSTNode *>(cur->left.peek());
		} else {
			if (!cur->right.peek()) {
				cur->right.poke(reinterpret_cast<uint64_t>(z));
				return true;
			}
...
static BSTNode *search_serial(BSTNode *root, int64_t key)
{
	while (root) {
		...
		if (key == k)
			return root;
		root = reinterpret_cast<BSTNode *>((key < k) ? root->left.peek()
		                                             : root->right.peek());
	}
	return nullptr;
}
...
static bool delete_serial(BSTNode **root, int64_t key)
{
	...
	if (!l)
		...
		BSTNode *succ_p = cur;
		BSTNode *succ = r;
		while (succ->left.peek()) {
			succ_p = succ;
			succ = reinterpret_cast<BSTNode *>(succ->left.peek());
		}
		...
		if (!parent)
			*root = succ;
			...
			parent->left.poke(reinterpret_cast<uint64_t>(succ));
		else
			parent->right.poke(reinterpret_cast<uint64_t>(succ));
		tm_free(cur);
		return true;
	}
	...
	if (!parent)
		*root = child;
		...
		parent->left.poke(reinterpret_cast<uint64_t>(child));
	else
		parent->right.poke(reinterpret_cast<uint64_t>(child));
	tm_free(cur);
	return true;
}
...
static BSTNode *tm_search(BSTNode *root, int64_t key)
{
	while (root) {
		...
		if (key == k)
			return root;
		root = reinterpret_cast<BSTNode *>((key < k) ? root->left.read()
		                                             : root->right.read());
	}
	return nullptr;
}
	...
	if (!l)
		...
		if (!parent)
			*root = succ;
			...
			set_left(parent, succ);
		else
			set_right(parent, succ);
		tm_free(cur);
		return true;
	}
...
static int tree_count(BSTNode *root)
{
	if (!root)
		return 0;
	return 1 + tree_count((BSTNode *)root->left.peek()) +
	       tree_count((BSTNode *)root->right.peek());
}
...
static bool tree_validate(BSTNode *root, int64_t lo, int64_t hi)
{
	...
	if (k < lo || k > hi) {
		fprintf(stderr,
		        "BST violation: key %lld out of range [%lld,%lld]\n",
		        (long long)k,
		        (long long)lo,
		        (long long)hi);
		return false;
	...
	if (l && l->key.peek() >= k) {
		fprintf(stderr,
		        "BST violation: left child key %lld >= parent %lld\n",
		        (long long)l->key.peek(),
		        (long long)k);
		ok = false;
	}
	if (r && r->key.peek() <= k) {
		fprintf(stderr,
		        "BST violation: right child key %lld <= parent %lld\n",
		        (long long)r->key.peek(),
		        (long long)k);
		ok = false;
	}
...
static void worker(ThreadData &d)
{
	...
	while (!g_stop.load()) {
		...
		if (op < 6) { // 60% insert
			...
			expli::TM<int>::transaction([&]() { tm_insert(&g_root, z); });
		} else if (op < 8) { // 20% lookup
			expli::TM<int>::transaction([&]() { tm_search(g_root, key); });
		} else { // 20% delete
			expli::TM<int>::transaction([&]() { tm_delete(&g_root, key); });
		}
		d.ops++;
	}
...
{
	if (argc > 1)
		g_duration_ms = atoi(argv[1]);
	if (argc > 2)
		g_nthreads = atoi(argv[2]);
	if (argc > 3)
		g_initial_keys = atoi(argv[3]);
	...
	printf("Tree size: %d  Total ops: %d  Ops/sec: %d\n",
	       final_count,
	       total_ops,
	       (int)(total_ops * 1000 / (double)g_duration_ms));
