/**
 * EigenBench - Full Specification Implementation
 *
...
TM long read_shared_array1(int idx) {
    return g_array1[idx];
}

TM void write_shared_array1(int idx, long val) {
    g_array1[idx] = val;
}

TM long read_thread_array2(int idx) {
    return g_array2[idx];
}

TM void write_thread_array2(int idx, long val) {
    g_array2[idx] = val;
}

TM long read_thread_array3(int idx) {
    return g_array3[idx];
}

TM void write_thread_array3(int idx, long val) {
    g_array3[idx] = val;
}

TX long txn_read_shared(int idx) {
    return read_shared_array1(idx);
}

TX void txn_write_shared(int idx, long val) {
    write_shared_array1(idx, val);
}

TX long txn_read_local(int idx) {
    return read_thread_array2(idx);
}

TX void txn_write_local(int idx, long val) {
    write_thread_array2(idx, val);
}
    ...
    for (int i = 0; i < w->loops && !done.load(std::memory_order_relaxed); i++) {
        ...
        if (g_config.mode == 0 || g_config.mode == 1) {
            ...
            if (g_config.enable_R2 && R2 > 0) {
                for (int j = 0; j < R2; j++) {
                    val += txn_read_local(t * g_config.A2 + w->td.indices_R2[j % w->td.indices_R2.size()]);
                }
                for (int j = 0; j < W2; j++) {
                    txn_write_local(t * g_config.A2 + w->td.indices_W2[j % w->td.indices_W2.size()], val);
                }
            }
        }
...
void print_usage(const char* prog) {
    std::cout << "EigenBench - Full Specification Implementation\n";
    std::cout << "==============================================\n\n";
    ...
    std::cout << "  --mode <n>    Mode: 0=all, 1=shared only, 2=local only (default: 0)\n\n";
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "EigenBench (Full Specification)\n";
    std::cout << "==============================\n";
    std::cout << "Configuration:\n";
    std::cout << "  Threads:        " << g_config.threads << "\n";
    std::cout << "  Duration:       " << g_config.duration << " ms\n";
    std::cout << "  R1/W1 (shared): " << g_config.R1 << "/" << g_config.W1 << " (contention=" << g_config.contention << ")\n";
    std::cout << "  R2/W2 (local):  " << g_config.R2 << "/" << g_config.W2 << " (locality=" << g_config.locality << ", density=" << g_config.density << ")\n";
    ...
    std::cout << "  2. Transaction Length:   " << (g_config.R1 + g_config.W1) << " ops/tx\n";
    std::cout << "  3. Working Set Size:      A2=" << g_config.A2 << ", A3=" << g_config.A3 << "\n";
    std::cout << "  4. Temporal Locality:     " << g_config.locality << "\n";
    std::cout << "  5. Pollution:             " << (g_config.locality < 0.5 ? "high (random)" : "low (sequential)") << "\n";
    std::cout << "  6. Contention:            " << g_config.contention << "\n";
    std::cout << "  7. Predominance:          R/W=" << g_config.R1 << "/" << g_config.W1 << " (" << (g_config.R1 * 100.0 / (g_config.R1 + g_config.W1)) << "% read)\n";
    ...
    std::cout << "Read ratio:   " << (reads + writes > 0 ? reads * 100.0 / (reads + writes) : 0) << "%\n";
    std::cout << std::endl;
