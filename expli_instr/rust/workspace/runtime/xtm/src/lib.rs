...
pub fn tm_begin() {
    ...
    TX.with(|tx| {
        let mut t = Box::new(TxState::new(id));
...
pub fn tm_commit() -> bool {
    ...
    // Validate: check all observed versions are unchanged in version table
    for r in &tx.read_set {
        let idx = version_index(r.addr);
    ...
    // Write-back all entries BEFORE updating version table.
    // This way a concurrent reader sees either (old ver, old data) or
    // (old ver after write-back, then new ver) — never (new ver, old data).
